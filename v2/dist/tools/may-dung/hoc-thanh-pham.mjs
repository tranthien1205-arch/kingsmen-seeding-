...
export default async function hoc({ app, goiApp, lenh, dir, log, script }) {
  ...
  const nhanDuong = (duong) => { const b = String(duong || "").toUpperCase(), b0 = boDau(duong); const ax = AX.find((x) => b0.includes(boDau(x.chua)));
    return { muc_dich: /ECOMMERCE|BÁN HÀNG|BAN HANG|SALE|TEASER/.test(b) ? "BAN_HANG" : /CREATIVE|BRAND|ĐỊNH VỊ|DINH VI/.test(b) ? "BRAND" : null, dong: ax ? ax.dong : AX.length ? null : /FINEX/.test(b) ? "Finex F300" : /TERRAZ/.test(b) ? "Terrazy" : null }; };
  // ---- 2+3. từng video
