...
# Làm gì: tải bản mới may-dung.mjs / BAT-DAU.bat / CHAY-NEN.bat vào C:may-dung, ghi PATH hiện tại (node, ffmpeg, ollama) vào
...
if (-not (Test-Path (Join-Path $DIR 'may-dung.json'))) { Write-Host "  X May nay chua ghep app ($DIRmay-dung.json) - chay cai-may-hoc.ps1 va ghep truoc." -ForegroundColor Red; return }
...
Register-ScheduledTask -TaskName $TEN -Action $act -Trigger $trg -Principal $pri -Settings $set -Description 'May con Kingsmen Content OS chay khi bat may, khong can dang nhap (S4U, khong luu mat khau). Nhat ky: C:may-dungmay-dung.log' -Force | Out-Null
...
Write-Host "  Kiem: Get-ScheduledTaskInfo '$TEN'  |  nhat ky: Get-Content $DIRmay-dung.log -Tail 20"
