...
function ViecCuaToi({go}){
  ...
  return <div className="space-y-4">
    ...
    {laXemMkt(me) && <Card pad="p-3" className="cursor-pointer hover:border-brand" onClick={()=>{ window.__tabMay='ai'; window.__tabNao='daymay'; go('may'); }}><div className="flex items-center gap-3 flex-wrap"><span className="text-2xl">🎓</span><div className="flex-1 min-w-[200px]"><div className="text-sm font-semibold text-ink">Dạy máy hiểu video</div><div className="text-[11px] text-ink-muted">Máy đưa từng khung hình, đoạn footage, câu thoại; anh/chị bấm một phím xác nhận đúng hay sửa. Mỗi thẻ vài giây, tối đa 20 phút/ngày.</div></div><Btn variant="brand" className="!py-1.5 !px-3 text-xs" onClick={(e)=>{ e.stopPropagation(); window.__tabMay='ai'; window.__tabNao='daymay'; go('may'); }}>Bắt đầu →</Btn></div></Card>}
    {laXemMkt(me) && <div>
