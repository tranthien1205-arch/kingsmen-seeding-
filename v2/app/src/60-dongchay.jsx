...
function DongChayNoiDung(){
  ...
  // Mô phỏng: thẻ giả. Thật (ADR-002): mục nội dung của tháng này (đợt 3 mới có soạn/duyệt/đăng trên thẻ)
  const pTen=id=>((db.pillars||[]).find(p=>p.id===id)||{}).ten||'—';
  const the=db.mo_phong?MP.the:(db.muc_noi_dung||[]).filter(m=>m.thang===db.hang_so.thang_nay).map(m=>({ id:m.id, gd:m.giai_doan, ten:m.tieu_de, dd:(DINH_DANG_ICON[m.dinh_dang]||'📄').split(' ')[0], pillar:pTen(m.pillar_id), ai:m.tao_boi==='AGENT'?'máy tạo từ kế hoạch/ý tưởng':('người tạo · '+m.created_by_name), ngay:m.ngay_dang||('tuần '+(m.tuan||'?')), that:true }));
  return <div className="space-y-4">
    ...
    {mo && mo.that && <Modal open onClose={()=>setMo(null)} title={mo.dd+' '+mo.ten}><div className="text-sm text-ink-muted">Pillar {mo.pillar} · {mo.ai} · {mo.ngay}. Soạn / duyệt / dựng / đăng trên thẻ mở ở đợt 3 (ADR-003). Sửa mục ở <b>Chiến lược & Kế hoạch › Tuần & mục</b>.</div></Modal>}
    {mo && !mo.that && <Modal open onClose={()=>setMo(null)} title={mo.dd+' '+mo.ten} wide>
