...
function BanMoi(){
  ...
  return <>
    {ban && <div className="fixed right-2 bottom-16 sm:bottom-2 z-40 text-[11px] text-ink-muted bg-white/80 border border-line rounded-full px-2 py-0.5 pointer-events-none tabular-nums" title="phiên bản đang chạy">v {String(moi?banKhoi.current:ban.commit).slice(0,7)} · {gio}</div>}
...
function Shell(){
  ...
  const NavBtn=({m, doc})=><button onClick={()=>setPage(m.key)} className={(doc?'w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm ':'flex-1 flex flex-col items-center py-1.5 text-[11px] ')+"font-semibold transition "+(page===m.key?(doc?'bg-brand text-white shadow-brand':'text-brand'):(doc?'text-white/70 hover:bg-white/10 hover:text-white':'text-ink-muted'))}>
    <span className={doc?'text-base':'text-lg leading-none'}>{m.icon}</span><span className={doc?'flex-1 text-left':''}>{doc?m.ten:m.ngan}</span>
    {m.key==='viec'&&soViec>0 && <span className={"rounded-full px-1.5 text-[11px] "+(page===m.key&&doc?'bg-white text-brand':'bg-rose-500 text-white')}>{soViec}</span>}</button>;
  return <div className="min-h-screen sm:flex">
    ...
    <aside className="hidden sm:flex sm:flex-col w-60 shrink-0 bg-ink text-white p-4 sticky top-0 h-screen">
      <div className="flex items-center gap-2 mb-6"><div className="w-9 h-9 rounded-xl bg-brand grid place-items-center font-display font-black">K</div><div><div className="font-display font-extrabold leading-tight">Kingsmen</div><div className="text-[11px] text-white/60">Content OS · đợt {db.dot}</div></div></div>
      <nav className="space-y-1 flex-1">{mans.map(m=><NavBtn key={m.key} m={m} doc/>)}</nav>
      <button onClick={()=>setHoSo(true)} className="flex items-center gap-2 p-2 rounded-xl hover:bg-white/10 text-left"><div className="w-8 h-8 rounded-full bg-brand-light/30 grid place-items-center text-sm font-bold">{(me.ho_ten||'?').slice(0,1)}</div><div className="min-w-0"><div className="text-sm font-semibold truncate">{me.ho_ten}</div><div className="text-[11px] text-white/60">{ROLE_LABEL[me.vai_tro]||me.vai_tro}</div></div></button>
    </aside>
