...
function KetQuaTab(){
  ...
  return <div className="space-y-3">
    ...
    <div className="grid sm:grid-cols-2 gap-2">
      <Card pad="p-3"><SectionTitle className="mb-1"><Pill cls={MUC_TIEU_CLS.BRAND}>Brand</Pill> nội dung xây dựng thương hiệu</SectionTitle><div className="grid grid-cols-4 gap-1 text-center">{[['tiep_can','Tiếp cận'],['luot_xem','Xem'],['chia_se','Chia sẻ'],['tuong_tac','Tương tác']].map(([f,l])=><div key={f}><div className="font-display font-extrabold text-ink">{fmtSo(sum(brand,f))}</div><div className="text-[11px] text-ink-muted">{l}</div></div>)}</div></Card>
      <Card pad="p-3"><SectionTitle className="mb-1"><Pill cls={MUC_TIEU_CLS.BAN_HANG}>Bán hàng</Pill> nội dung dẫn tới đơn</SectionTitle><div className="grid grid-cols-4 gap-1 text-center">{[['luot_xem','Xem'],['click','Click']].map(([f,l])=><div key={f}><div className="font-display font-extrabold text-ink">{fmtSo(sum(banHang,f))}</div><div className="text-[11px] text-ink-muted">{l}</div></div>)}<div><div className="font-display font-extrabold text-emerald-700">{fmtSo(sum(muc('TRUC_TIEP'),'so_don'))}</div><div className="text-[11px] text-ink-muted">đơn trực tiếp</div></div><div><div className="font-display font-extrabold text-amber-700">{fmtSo(sum(muc('GIAN_TIEP'),'so_don'))}</div><div className="text-[11px] text-ink-muted">đơn gián tiếp</div></div></div></Card>
    </div>
...
function DeXuatTab(){
  ...
  return <div className="space-y-3">
    ...
    {ds.length===0?<Empty>Chưa có đề xuất nào. Máy rút đề xuất ngày 2 hằng tháng (sau báo cáo tháng) khi B12 ở mức AI gợi ý.</Empty>:<div className="grid lg:grid-cols-2 gap-2">{ds.map(d=><Card key={d.id} pad="p-3">
      <div className="flex items-center gap-2 flex-wrap"><Pill>{d.loai}</Pill><Pill cls={d.trang_thai==='DUYET'?'bg-emerald-100 text-emerald-800':d.trang_thai==='BO'?'bg-slate-100 text-ink-muted':'bg-amber-100 text-amber-800'}>{d.trang_thai==='CHO'?'chờ G4':d.trang_thai==='DUYET'?'đã áp dụng':'đã bỏ'}</Pill><span className="text-[11px] text-ink-muted">kỳ {d.ky}</span></div>
      <div className="text-sm font-semibold text-ink mt-1">{d.tieu_de}</div><div className="text-[11px] text-ink-soft mt-0.5">{d.ly_do}</div>
      <div className="text-[11px] text-ink-muted mt-1">Bằng chứng: {Object.entries(d.bang_chung||{}).map(([k,v])=>k+': '+(typeof v==='object'?Object.entries(v).map(([a,b])=>a+'='+b).join(' '):v)).join(' · ')}</div>
      ...
      {d.trang_thai!=='CHO'&&<div className="text-[11px] text-ink-muted mt-1">{d.quyet_boi} · {fmtDate(d.quyet_at)}{d.ly_do_nguoi?(' · '+d.ly_do_nguoi):''}{d.ap_dung&&d.ap_dung.ghi_chu?(' · '+d.ap_dung.ghi_chu):''}</div>}
    </Card>)}</div>}
  </div>;
}
