...
function NguonHoc({ a }) {
  ...
  const theoDoi = async (g, o, tb) => { setBusy(true); const r = await goi('/nguon-hoc/theo-doi', { method: 'POST', body: { key: g.key, loai: g.loai, nap: g.nap, ten: g.ten, ...o } }); setBusy(false); if (r.ok) { notify(tb + (r.vua_hoc && r.vua_hoc.so ? ' · đã giao học ngay' : '')); tai(); } else notify(r.msg, 'err'); };
  ...
  const Nut = ({ g }) => g.dang_hoc ? <Pill cls="bg-sky-100 text-sky-800" className="!text-[11px]">⏳ {g.dang_hoc === 'CHO_TRAM' ? 'đang xếp hàng' : g.dang_hoc === 'CHO' ? 'chờ máy nhận' : 'máy đang học'}</Pill> : <div className="flex gap-1.5 flex-wrap items-center">{g.nap && duoc && (g.loai !== 'FOOTAGE' || g.kho_dong) && <button className={nutCls + ' !py-0.5'} disabled={busy} onClick={() => hocThem(g)} title={'nạp lại: ' + g.nap}>{g.loai === 'FOOTAGE' ? '＋ Nạp thêm footage mới' : '＋ Học thêm video mới'}</button>}
    ...
    {duoc && g.nap && g.loai !== 'KALODATA' && g.loai !== 'LOCAL' && <label className="inline-flex items-center gap-1 text-[11px]" title="app tự học thêm video mới của nguồn này mỗi tuần"><input type="checkbox" disabled={busy} checked={!!(g.theo_doi && g.theo_doi.tu_dong)} onChange={(e) => theoDoi(g, { tu_dong: e.target.checked }, e.target.checked ? 'Bật tự học mỗi tuần cho ' + g.ten : 'Tắt tự học ' + g.ten)} /> tự học mỗi tuần{g.theo_doi && g.theo_doi.lan_hoc ? <span className="text-ink-muted"> · lần cuối {fmtDate(g.theo_doi.lan_hoc)}</span> : null}</label>}
