...
// hàng đề xuất: lọc theo trường / dòng, chọn nhiều để duyệt / bỏ một lượt; điện thoại xếp thẻ
function HangDeXuat({ bn, busy, lam, trTen, duoc }) {
  const { goi, notify } = useApp(); const { dat } = useBoNhan(); const pc = useRong(760);
  const [loc, setLoc] = useState(''); const [dong, setDong] = useState(''); const [chon, setChon] = useState(() => new Set()); const [n, setN] = useState(40); const [xac, setXac] = useState(null); const [dang, setDang] = useState(false);
  const all = bn.de_xuat; const demTr = {}; all.forEach((x) => { demTr[x.truong] = (demTr[x.truong] || 0) + 1; });
  const ds = all.filter((x) => (!loc || x.truong === loc) && (!dong || (dong === '(chung)' ? !x.dong : x.dong === dong))); const dongs = [...new Set(all.map((x) => x.dong || '(chung)'))];
  const hl = async (hanh) => { setDang(true); const r = await goi('/bo-nhan/hang-loat', { method: 'POST', body: { ids: [...chon], hanh } }); setDang(false); setXac(null); if (r.ok) { dat(r); setChon(new Set()); notify((hanh === 'duyet' ? 'Đã duyệt ' : 'Đã bỏ ') + r.so + ' nhãn' + (r.loi && r.loi.length ? ' · ' + r.loi.length + ' lỗi: ' + r.loi[0] : '')); } else notify(r.msg, 'err'); };
  const tg = (id) => { const s = new Set(chon); s.has(id) ? s.delete(id) : s.add(id); setChon(s); };
  const nutQuyet = (v) => <div className="flex gap-1 items-center flex-wrap"><button className={nutCls + ' !bg-[#15803D] !border-[#15803D] text-white !px-2 !py-0.5'} disabled={busy || !duoc} onClick={() => lam('/bo-nhan/' + v.id + '/duyet', {}, () => 'Đã duyệt “' + v.ten + '” thành nhãn chuẩn')}>Duyệt</button>
    <select className="border border-line rounded-lg px-1.5 py-0.5 text-xs bg-white min-w-0 max-w-[150px]" value="" disabled={busy || !duoc} onChange={(e) => { const d = e.target.value; if (d) lam('/bo-nhan/' + v.id + '/gop', { vao: d }, (r) => 'Đã gộp “' + v.ten + '” vào “' + d + '” — ' + r.mau_chuyen + ' mẫu chuyển theo'); }}><option value="">Gộp vào…</option>{giaTri18(bn, v.truong, v.dong).map((x) => <option key={x} value={x}>{tenGt18(bn, v.truong, x)}</option>)}</select>
    <button className={nutCls + ' !px-2 !py-0.5'} disabled={busy || !duoc} onClick={() => lam('/bo-nhan/' + v.id + '/bo', {}, () => 'Đã bỏ “' + v.ten + '”')}>Bỏ</button></div>;
  const nguon = (v) => v.nguon === 'THAY' ? <Pill cls="bg-[#E3EEFA] text-[#1F5FA8]" className="!text-[11px]">thầy</Pill> : <Pill cls="bg-[#FDF3D6] text-[#8A6410]" className="!text-[11px]">người</Pill>;
  const nhanDx = (v) => <span className="border border-dashed border-[#E3B23C] bg-[#FFF9E8] rounded-md px-1.5 text-[11px]">{v.ten}</span>;
  const th = 'text-[10px] tracking-[.06em] uppercase text-ink-muted bg-[#F5F8F9] px-2 py-1.5 font-bold text-left'; const td = 'border-t border-[#EEF3F4] px-2 py-1.5 align-middle';
  return <Card pad="p-3" className="flex flex-col gap-3 !rounded-[14px] !shadow-none">
    <div className="flex items-center gap-2 flex-wrap"><span className="font-bold text-sm">Hàng đề xuất</span><span className="text-xs text-ink-muted">{all.length} nhãn mới từ thầy và người, chưa được dùng chính thức. Duyệt thì thành nhãn chuẩn; gộp thì mọi mẫu đang dùng chuyển sang nhãn đích. Bước / bài test gắn theo dòng.</span></div>
    {all.length > 0 && <div className="flex gap-1.5 flex-wrap items-center">
      ...
      {Object.entries(demTr).sort((a, b) => b[1] - a[1]).map(([k, c]) => <button key={k} onClick={() => { setLoc(k); setChon(new Set()); }} className={'rounded-full border px-2.5 py-0.5 text-xs bg-white ' + (loc === k ? 'border-ink font-bold' : 'border-line')}>{trTen(k)} · {c}</button>)}
      <select className="ml-auto border border-line rounded-lg px-2 py-1 text-xs bg-white min-w-0" value={dong} onChange={(e) => { setDong(e.target.value); setChon(new Set()); }}><option value="">mọi dòng</option>{dongs.map((d) => <option key={d} value={d}>{d}</option>)}</select></div>}
    {duoc && ds.length > 0 && <div className="flex gap-2 items-center flex-wrap text-xs"><label className="flex items-center gap-1.5 cursor-pointer"><input type="checkbox" checked={chon.size > 0 && ds.every((x) => chon.has(x.id))} onChange={(e) => setChon(e.target.checked ? new Set(ds.map((x) => x.id)) : new Set())} /> chọn cả {ds.length} đang lọc</label>
      {chon.size > 0 && (xac ? <><span className="font-semibold">{xac === 'duyet' ? 'Duyệt' : 'Bỏ'} {chon.size} nhãn?</span><button className={nutCls + (xac === 'duyet' ? ' !bg-[#15803D] !border-[#15803D] text-white' : ' !bg-[#A3372B] !border-[#A3372B] text-white')} disabled={dang} onClick={() => hl(xac)}>{dang ? 'Đang làm…' : 'Chắc chắn'}</button><button className={nutCls} onClick={() => setXac(null)}>Thôi</button></>
        : <><button className={nutCls + ' !bg-[#15803D] !border-[#15803D] text-white'} onClick={() => setXac('duyet')}>Duyệt {chon.size} đã chọn</button><button className={nutCls} onClick={() => setXac('bo')}>Bỏ {chon.size} đã chọn</button></>)}</div>}
    {!all.length ? <div className="text-xs text-ink-muted">Không còn đề xuất nào.</div> : pc ? <div className="overflow-x-auto"><table className="w-full border-collapse text-xs min-w-[680px]"><thead><tr>{duoc && <th className={th + ' w-6'}></th>}<th className={th}>Trường · dòng</th><th className={th}>Nhãn đề xuất</th><th className={th}>Nguồn</th><th className={th + ' !text-right'}>Mẫu dùng</th><th className={th}>Quyết</th></tr></thead>
      <tbody>{ds.slice(0, n).map((v) => <tr key={v.id}>{duoc && <td className={td}><input type="checkbox" checked={chon.has(v.id)} onChange={() => tg(v.id)} /></td>}<td className={td}>{trTen(v.truong)}{v.dong ? <span className="text-ink-muted"> · {v.dong}</span> : null}</td><td className={td}>{nhanDx(v)}</td><td className={td}>{nguon(v)}</td><td className={td + ' text-right tabular-nums'}>{v.so_mau}</td><td className={td}>{nutQuyet(v)}</td></tr>)}</tbody></table></div>
      : <div className="flex flex-col divide-y divide-[#EEF3F4] border border-[#EEF3F4] rounded-xl">{ds.slice(0, n).map((v) => <div key={v.id} className="p-2 flex flex-col gap-1.5 text-xs"><div className="flex items-center gap-2">{duoc && <input type="checkbox" checked={chon.has(v.id)} onChange={() => tg(v.id)} />}{nhanDx(v)}{nguon(v)}<span className="ml-auto tabular-nums text-ink-muted">{v.so_mau} mẫu</span></div><div className="text-[11px] text-ink-muted">{trTen(v.truong)}{v.dong ? ' · ' + v.dong : ' · chung'}</div>{nutQuyet(v)}</div>)}</div>}
    {ds.length > n && <button className={nutCls + ' self-start'} onClick={() => setN(n + 60)}>Xem thêm {Math.min(60, ds.length - n)} / còn {ds.length - n}</button>}
  </Card>;
}
// ===================== TỔNG QUAN =====================
