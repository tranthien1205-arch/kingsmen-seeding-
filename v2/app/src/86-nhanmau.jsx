...
function NguonHoc({ a }) {
  ...
  const TheNguon = ({ g }) => { const [t, c] = trangThai(g); const nhan = pct(g.thay, g.hinh); const chuaDong = g.dong['(chưa có)'] || 0; const laFt = g.loai === 'FOOTAGE';
    return <div className="rounded-2xl border border-line bg-white p-3 flex flex-col gap-2 min-w-0">
      <div className="flex items-start gap-2.5"><div className="w-9 h-9 rounded-xl bg-[#F2F7F8] grid place-items-center text-lg shrink-0">{(LOAI_NGUON[g.loai] || ['•'])[0]}</div>
        ...
        <span className={'shrink-0 rounded-full border px-2 py-0.5 text-[10px] font-semibold whitespace-nowrap ' + c}>{t}</span></div>
      <div className="flex gap-1 flex-wrap">{Object.entries(g.dong).sort((x, y) => y[1] - x[1]).map(([k, n]) => <span key={k} className={'rounded-md px-1.5 py-0.5 text-[10px] font-semibold ' + (k === '(chưa có)' ? 'bg-[#FFF9E8] text-[#8A6410]' : 'bg-[#F6EDD6] text-[#8A6410]')}>{k === '(chưa có)' ? 'chưa có dòng' : k}{n ? ' · ' + n : ''}</span>)}</div>
      <div className="grid grid-cols-3 gap-2 text-center"><div className="rounded-lg bg-[#F7FAFB] py-1.5"><b className="block text-[16px] tabular-nums">{g.so_video}</b><span className="text-[10px] text-ink-muted">{laFt ? 'clip' : 'video'}</span></div>
        <div className="rounded-lg bg-[#F7FAFB] py-1.5"><b className="block text-[16px] tabular-nums">{g.hinh}</b><span className="text-[10px] text-ink-muted">đoạn hình</span></div>
        <div className="rounded-lg bg-[#F7FAFB] py-1.5"><b className="block text-[16px] tabular-nums">{laFt ? (g.cuoi ? fmtDate(g.cuoi).split(' ')[1] || fmtDate(g.cuoi) : '—') : (g.loi || 0)}</b><span className="text-[10px] text-ink-muted">{laFt ? 'nạp cuối' : 'câu lời'}</span></div></div>
      {g.hinh > 0 && <div className="flex flex-col gap-1"><div className="flex justify-between text-[11px]"><span className="text-ink-muted">Nhãn thầy</span><b className="tabular-nums">{nhan}%</b></div><Thanh v={nhan} />{!laFt && g.loi > 0 && <div className="text-[10px] text-ink-muted">lời nghe sai {pct(g.nghe_sai, g.loi)}% · có ảnh {pct(g.anh, g.hinh)}% · bản xem {pct(g.ban_xem, g.so_video)}%</div>}</div>}
      {chuaDong > 0 && <div className="text-[11px] text-[#8A6410]">⚠ {chuaDong} video chưa có dòng — gán ở ⋯</div>}
      <div className="flex items-center gap-2 flex-wrap pt-1 border-t border-[#EEF3F4] mt-auto">
        ...
        <span className="flex-1" /><span className="text-[10px] text-ink-muted">{g.cuoi ? fmtDate(g.cuoi) : ''}</span>
      ...
      {menu === g.key && <div className="rounded-xl bg-[#F7FAFB] p-2 flex flex-col gap-2 text-xs">
        ...
        {mo === g.key && <div className="rounded-lg bg-white border border-line p-2 max-h-64 overflow-auto">{g.video.map((v) => <div key={v.id} className="flex items-center gap-2 py-1 border-b border-[#EEF3F4] last:border-0"><span className="flex-1 min-w-0 truncate">{v.ten}</span><span className="text-ink-muted tabular-nums whitespace-nowrap">{v.hinh} đoạn</span></div>)}</div>}
  ...
  return <div className="flex flex-col gap-3">
    <Card pad="p-4" className="!rounded-[16px] !shadow-none flex flex-col gap-3">
      ...
      <div className="grid grid-cols-2 min-[760px]:grid-cols-4 gap-2">{[[tong.v, 'video thành phẩm đã học'], [clip, 'clip footage gốc trong kho'], [(pct(tong.th, tong.h) ?? 0) + '%', 'đoạn hình có nhãn thầy'], [d.nguon.length, 'nguồn · ' + tuHoc + ' tự học tuần']].map(([x, t], i) => <div key={i} className="rounded-xl bg-[#F7FAFB] px-3 py-2"><b className="text-[20px] block tabular-nums leading-tight">{x}</b><span className="text-[11px] text-ink-muted">{t}</span></div>)}</div>
      <div className={'rounded-lg px-2.5 py-1.5 text-[11px] flex gap-3 flex-wrap ' + (maySong.length ? 'bg-[#F2F7F8] text-ink-muted' : 'bg-[#FBE4E1] text-[#A3372B]')}><b>Máy học</b>{d.may.map((m) => <span key={m.ten}>{m.song ? '🟢' : '⚪'} {m.ten}{m.song ? '' : ' · im từ ' + (m.nhan_luc ? fmtDate(m.nhan_luc) : '—')}</span>)}</div>
    </Card>
    ...
    {tab === 'tp' && <>
      <div className="flex gap-1.5 flex-wrap items-center text-xs">{[['', 'Tất cả'], ...Object.entries(NEN).filter(([k]) => tp.some((g) => g.loai === k))].map(([k, t]) => <button key={k} onClick={() => setNen(k)} className={'rounded-full border px-2.5 py-1 ' + (nen === k ? 'border-[#0E7C8C] text-[#0E7C8C] font-semibold bg-[#E0F2F4]' : 'border-line bg-white')}>{t} · {k ? tp.filter((g) => g.loai === k).length : tp.length}</button>)}
        ...
        <select className="border border-line rounded-full px-2 py-1 bg-white" value={xep} onChange={(e) => setXep(e.target.value)}><option value="moi">mới học trước</option><option value="nhieu">nhiều video trước</option><option value="can">cần xử lý trước</option></select></div>
      <div className="grid grid-cols-1 min-[760px]:grid-cols-2 min-[1280px]:grid-cols-3 gap-3">{dsTp.map((g) => <TheNguon key={g.key} g={g} />)}</div>
    ...
    {tab === 'ft' && <><div className="text-[12px] text-ink-muted px-1">Footage gốc là nguyên liệu để máy dựng video mới: mỗi dòng một kho, thư mục con theo cảnh / góc quay được giữ làm nhãn. Nạp thêm bằng ＋ Thêm nguồn › thư mục Drive › Footage gốc.</div>
      <div className="grid grid-cols-1 min-[760px]:grid-cols-2 min-[1280px]:grid-cols-3 gap-3">{ft.map((g) => <TheNguon key={g.key} g={g} />)}</div>{!ft.length && <Card pad="p-4"><Empty>Chưa có kho footage nào.</Empty></Card>}</>}
    {tab === 'kd' && d.kalodata && <KalodataNguon kd={d.kalodata} duoc={duoc} dongs={d.dongs} onDoi={tai} />}
    {tab === 'lenh' && <Card pad="p-3" className="!rounded-[14px] !shadow-none"><div className="flex flex-col gap-1.5">{!d.lenh.length && <span className="text-xs text-ink-muted">Chưa giao máy việc học nào.</span>}{d.lenh.map((l) => { const [t, c] = trangThaiLenh(l.trang_thai); const ts = l.tham_so || {}; return <div key={l.id} className="flex gap-2 items-start text-[12px] rounded-lg border border-line/70 px-2.5 py-1.5"><Pill cls={c} className="!text-[10px] shrink-0">{t}</Pill><span className="flex-1 min-w-0 break-words">{l.viec === 'hoc_thanh_pham' ? (ts.chi_proxy ? 'Tạo bản xem + ảnh bù' : 'Học video ' + (ts.nguon || '') + (ts.kenh ? ' ' + ts.kenh : '')) : l.viec === 'phan_tich_footage' ? 'Đọc footage' : l.viec === 'nap_drive' ? 'Nạp footage Drive' : 'Trạm tải / quét'}{ts.so ? ' · ' + ts.so + ' video' : ''}{l.ket_qua ? <span className="text-ink-muted"> — {String(l.ket_qua).slice(0, 160)}</span> : null}</span><span className="text-ink-muted whitespace-nowrap text-[11px]">{fmtDate(l.created_at)}</span></div>; })}</div></Card>}
  </div>;
}
