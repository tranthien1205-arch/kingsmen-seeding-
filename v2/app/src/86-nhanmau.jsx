...
function KhoMau18({ onDoi }) {
  ...
  const dongMau = (x) => { const t = x.tt; const tieuDe = kn === 'K4' ? '“' + (x.text || '') + '”' : kn === 'K2' ? x.ten : ((x.nguoi && x.nguoi.mo_ta) || (x.thay && x.thay.mo_ta) || (x.mo && x.mo.mo_ta) || x.mo_ta || '—');
    return <div key={x.id}>
      <div onClick={() => setChon(chon === x.id && !rong ? null : x.id)} title={x.ten + ' · đoạn ' + ((x.i || 0) + 1) + ' · giây ' + x.tu + '–' + x.den + (x.dong ? ' · ' + x.dong : '') + ' · ' + ((TT18[t] || {}).ten || '')} className={'grid grid-cols-[16px_44px_minmax(0,1fr)_10px] gap-2 items-center px-2 py-[6px] border-t border-[#EEF3F4] first:border-t-0 cursor-pointer ' + (chon === x.id ? 'bg-[#EEF7F8] shadow-[inset_3px_0_0_#0E7C8C]' : 'hover:bg-[#F7FAFB]')}>
        ...
        <div className="min-w-0 truncate text-[13px] text-ink">{tieuDe}</div>
  ...
  return <Card pad="p-3" className="flex flex-col gap-3 !rounded-[14px] !shadow-none">
    ...
    <div className="grid gap-3 items-start min-[900px]:grid-cols-[300px_minmax(0,1fr)]">
