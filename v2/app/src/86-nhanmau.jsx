...
function NguonHoc({ a }) {
  ...
  const hocThem = async (g) => { setBusy(true); const r = await goi('/lop-hoc/nap', { method: 'POST', body: { nap: g.nap, toi_da: 30 } }); setBusy(false); if (r.ok) { notify(r.trung ? 'Máy đã có lệnh này rồi' : 'Đã giao máy học thêm video mới của ' + g.ten); tai(); } else notify(r.msg, 'err'); };
  ...
  const Nut = ({ g }) => g.dang_hoc ? <Pill cls="bg-sky-100 text-sky-800" className="!text-[11px]">⏳ {g.dang_hoc === 'CHO' ? 'chờ máy nhận' : 'máy đang học'}</Pill> : <div className="flex gap-1.5 flex-wrap items-center">{g.nap && duoc && <button className={nutCls + ' !py-0.5'} disabled={busy} onClick={() => hocThem(g)} title={'nạp lại: ' + g.nap}>＋ Học thêm video mới</button>}
    ...
    <button className={nutCls + ' !py-0.5'} onClick={() => setMo(mo === g.key ? null : g.key)}>{mo === g.key ? 'Ẩn video' : 'Xem ' + g.so_video + ' video'}</button></div>;
