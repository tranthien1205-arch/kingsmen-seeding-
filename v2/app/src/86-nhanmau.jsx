...
function NguonHoc({ a }) {
  ...
  const Nut = ({ g }) => g.dang_hoc ? <Pill cls="bg-sky-100 text-sky-800" className="!text-[11px]">⏳ {g.dang_hoc === 'CHO_TRAM' ? 'đang xếp hàng' : g.dang_hoc === 'CHO' ? 'chờ máy nhận' : 'máy đang học'}</Pill> : <div className="flex gap-1.5 flex-wrap items-center">{g.nap && duoc && <button className={nutCls + ' !py-0.5'} disabled={busy} onClick={() => hocThem(g)} title={'nạp lại: ' + g.nap}>＋ Học thêm video mới</button>}
