...
function BoNaoAI(){
  ...
  return <div className="space-y-3">
    ...
    <Tabs size="sm" active={tab} onChange={setTab} tabs={[{key:'ban',label:'🎯 Bàn huấn luyện'},{key:'daymay',label:'🎓 Dạy máy',count:(soQuyet+soThe)||null},{key:'dinhtuyen',label:'Định tuyến',count:(a.dinh_tuyen||[]).filter(d=>d.de_nghi).length||null},{key:'mohinh',label:'Mô hình',count:(a.mo_hinh||[]).length||null},{key:'khoa',label:'🔑 Khoá API',count:(db.khoa_api||[]).filter(t=>!t.co).length||null},{key:'hoc',label:'Huấn luyện',count:(a.phien_ban||[]).filter(x=>x.trang_thai==='CHO_DUYET').length||null},{key:'lophoc',label:'🎓 Lớp học (đơn giản)',count:(a.ky_nang||[]).filter(k=>k.trang_thai==='MOI').length||null},{key:'chiphi',label:'Chi phí theo mô hình'}]}/>
    {tab==='ban'&&<BanHuanLuyen a={a} setTab={setTab} dem={dem} soQuyet={soQuyet}/>}{tab==='dinhtuyen'&&<DinhTuyenAI a={a}/>}{tab==='mohinh'&&<MoHinhAI a={a}/>}{tab==='hoc'&&<HuanLuyenAI a={a}/>}{tab==='lophoc'&&<LopHoc a={a}/>}{tab==='daymay'&&<DayMay a={a} setTab={setTab} dem={dem} taiDem={taiDem}/>}{tab==='chiphi'&&<ChiPhiMoHinh a={a}/>}{tab==='khoa'&&<KhoaAPI/>}
  </div>;
}
...
function NapMotO({a, duoc}){
  ...
  return <Card pad="p-4"><SectionTitle className="mb-1">1 · Đưa video vào cho máy học</SectionTitle>
...
function KyNang({a, duoc, db}){
  ...
  return <div className="space-y-2"><div className="flex items-center gap-2 flex-wrap"><SectionTitle>2 · Bốn kỹ năng máy đang học</SectionTitle><span className="text-[11px] text-ink-muted">mỗi thẻ trả lời: đang làm bằng gì · học được bao nhiêu · có gì mới để bật</span>{duoc&&<Btn variant="ghost" className="!py-0.5 !px-2 text-[11px] ml-auto" onClick={tuHoc} disabled={busy}>Học ngay nếu đủ mẫu</Btn>}</div>
...
function HuanLuyenAI({a}){
  ...
  return <div className="space-y-3">
    ...
    <NapDuLieu a={a} duoc={duoc} dongs={dongs}/>
    <TienTrinh a={a} db={db}/>
    <KhoMau a={a} duoc={duoc} dongs={dongs}/>
    <Card pad=""><div className="px-3 py-2 border-b border-line"><SectionTitle>5 · Phiên bản mô hình (người duyệt mới bật — G4)</SectionTitle></div>
...
function KhoMau({a, duoc, dongs}){
  ...
  return <Card pad="p-3"><SectionTitle className="mb-1">4 · Kho mẫu ({kho.length} video thành phẩm)</SectionTitle>
