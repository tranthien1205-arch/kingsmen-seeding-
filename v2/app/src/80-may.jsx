...
function May(){
  const { db, me } = useApp();
  const [tab,setTab]=useState(()=>{ const t=window.__tabMay||'buoc'; delete window.__tabMay; return t; });
...
function DinhTuyenAI({a}){
  ...
  return <Card pad=""><div className="overflow-x-auto"><table className="w-full text-xs min-w-[640px]"><thead><tr className="text-left text-ink-muted border-b border-line"><th className="px-3 py-2">Tính năng</th><th className="px-3 py-2">Chính (API)</th><th className="px-3 py-2">Dự phòng</th><th className="px-3 py-2">Mô hình mở</th><th className="px-3 py-2">Mức</th><th className="px-3 py-2">Điểm mở / mẫu</th><th className="px-3 py-2">Máy đề nghị</th></tr></thead><tbody>
    {(a.dinh_tuyen||[]).map(d=><tr key={d.tinh_nang} className="border-b border-line/60 align-top"><td className="px-3 py-2"><div className="font-semibold text-ink">{d.ten}</div><div className="text-[10px] text-ink-muted">{LOAI_MH[d.loai]} · {d.tinh_nang} · ngưỡng {d.nguong}/100, ≥ {d.min_mau} mẫu</div></td>
      ...
      <td className="px-3 py-2 text-[11px]">{d.de_nghi?<span className={d.de_nghi==='LEN'?'text-emerald-700':'text-rose-700'}>{d.de_nghi==='LEN'?'⬆ nâng':'⬇ hạ'} — {d.de_nghi_ly_do}</span>:<span className="text-ink-muted">—</span>}</td></tr>)}</tbody></table></div>
...
// ADR-012 — MÀN HUẤN LUYỆN (mô phỏng docs/huan-luyen-mo-phong.html, duyệt 24/09): 1 mô hình đang học · 2 bốn cửa nạp có nhãn dòng/mục đích
// · 3 tiến trình Trạm → máy dựng · 4 kho mẫu (xem, gắn nhãn, loại) · 5 phiên bản chờ duyệt (G4). Bản chung + bản riêng theo phạm vi.
...
// ADR-013 — LỚP HỌC CỦA MÁY (mô phỏng docs/lop-hoc-mo-phong.html, duyệt 24/09): một ô nạp → hỏi sản phẩm → bốn kỹ năng nói tiếng người
// (máy tự học khi đủ mẫu, xem thử rồi bật) → nhật ký → kho video. Kỹ thuật (mức, phạm vi, phiên bản, lệnh) gấp ở dưới = màn Huấn luyện cũ.
function LopHoc({a}){
  const { me, goi, notify, db } = useApp(); const duoc=laGat(me);
  return <div className="space-y-3">
...
function NapMotO({a, duoc}){
  const { goi, notify, db } = useApp(); const [nap,setNap]=useState(''); const [nhan,setNhan]=useState(null); const [dong,setDong]=useState(''); const [md,setMd]=useState(''); const [toiDa,setToiDa]=useState(20); const [busy,setBusy]=useState(false);
  const dongs=a.dong_san_pham||[]; const tramSong=!!((db.tram||{}).trang_thai||{}).song; const mayDung=(db.may_ghep||[]).filter(m=>m.song&&(m.kha_nang||[]).includes('dung_video'));
  const nhanRa=async()=>{ if(!nap.trim()) return; setBusy(true); const r=await goi('/lop-hoc/nap',{method:'POST',body:{nap:nap.trim(), chi_nhan:true}}); setBusy(false); if(r.ok) setNhan(r); else notify(r.msg,'err'); };
  const hoc=async()=>{ setBusy(true); const r=await goi('/lop-hoc/nap',{method:'POST',body:{nap:nap.trim(), dong:dong||undefined, muc_dich:md||undefined, toi_da:toiDa}}); setBusy(false); if(!r.ok) return notify(r.msg,'err');
    notify(r.loai==='KALODATA'?('Đã xin Trạm quét ngành '+r.nganh+' — video tìm được sẽ tự tải và học'):r.loai==='TIKTOK'?('Đã xin Trạm tải kênh '+(r.kenh||nap)):(r.trung?'Máy dựng đã có lệnh này':'Máy dựng bắt đầu học')); setNap(''); setNhan(null); setDong(''); setMd(''); };
  const Chip=({on,children,onClick})=><button type="button" onClick={onClick} className={"px-2.5 py-1 rounded-full text-xs border "+(on?'bg-brand/10 border-brand text-brand font-semibold':'border-line text-ink-muted')}>{children}</button>;
  return <Card pad="p-4"><SectionTitle className="mb-1">Đưa video vào cho máy học</SectionTitle>
    <div className="flex gap-2 flex-wrap items-center"><Input className="flex-1 min-w-[260px] !py-2" placeholder="Dán link thư mục Drive · thư mục trên máy dựng (D:\…) · @kênh TikTok · tên ngành hàng Kalodata" value={nap} onChange={e=>{ setNap(e.target.value); setNhan(null); }} onKeyDown={e=>{ if(e.key==='Enter') nhanRa(); }}/>{duoc&&<Btn variant="brand" onClick={nhanRa} disabled={busy||!nap.trim()}>Nạp</Btn>}</div>
    <div className="text-[11px] text-ink-muted mt-1">Video đã dựng tay là quý nhất; thư mục con tên <code>goc</code> chứa clip quay gốc thì máy học được cả cách cắt. Máy dựng đang bật: {mayDung.length?mayDung.map(m=>m.ten).join(', '):<b className="text-rose-700">không</b>} · Trạm: {tramSong?'sống':<b className="text-rose-700">im</b>}.</div>
    {nhan&&<div className="mt-3 rounded-xl border border-dashed border-line bg-paper p-3 space-y-2 text-xs">
      <div><b>Máy nhận ra: {nhan.ten}.</b> Video này về sản phẩm nào? (bỏ qua cũng được)</div>
      <div className="flex gap-1.5 flex-wrap items-center">{dongs.map(d=><Chip key={d} on={dong===d} onClick={()=>setDong(dong===d?'':d)}>{d}</Chip>)}<Input className="!py-1 !px-2 text-xs !w-40" placeholder="+ sản phẩm khác" value={dongs.includes(dong)?'':dong} onChange={e=>setDong(e.target.value)}/></div>
      <div className="flex gap-1.5 flex-wrap items-center"><Chip on={md==='BAN_HANG'} onClick={()=>setMd(md==='BAN_HANG'?'':'BAN_HANG')}>Để bán hàng</Chip><Chip on={md==='BRAND'} onClick={()=>setMd(md==='BRAND'?'':'BRAND')}>Làm thương hiệu</Chip>{nhan.loai!=='KALODATA'&&<span className="text-ink-muted ml-2">tối đa <Input type="number" min="1" max="60" className="!py-0.5 !px-1 text-xs !w-14 inline-block" value={toiDa} onChange={e=>setToiDa(+e.target.value||20)}/> video</span>}</div>
      <div className="flex gap-2 items-center"><Btn variant="brand" className="!py-1 text-xs" onClick={hoc} disabled={busy}>Bắt đầu học</Btn><span className="text-ink-muted">{nhan.loai==='KALODATA'?'Trạm quét ngành này trên Kalodata (tên phải đúng như Kalodata gọi, vd "Vật tư xây dựng"), tải top video theo doanh thu rồi máy dựng học':nhan.loai==='TIKTOK'?'Trạm tải video kèm lượt xem rồi máy dựng học':'máy dựng cắt shot, nghe lời thoại, khoảng 1 phút mỗi video'}</span></div>
    </div>}
  </Card>;
}
function KyNang({a, duoc, db}){
  const { goi, notify } = useApp(); const [busy,setBusy]=useState(false); const [xem,setXem]=useState(null);
  const ds=a.ky_nang||[]; const ten=id=>((db.tai_san||[]).find(t=>t.id===id)||{}); const dongs=a.dong_san_pham||[];
  const bat=async(k)=>{ setBusy(true); const r=await goi('/lop-hoc/bat',{method:'POST',body:{tinh_nang:k.tinh_nang_chinh, phien_ban_id:k.ban_moi?k.ban_moi.id:undefined}}); setBusy(false); if(r.ok){ notify('Đã bật máy nhà cho '+k.ten); setXem(null); } else notify(r.msg,'err'); };
  const tat=async(k)=>{ setBusy(true); const r=await goi('/lop-hoc/tat',{method:'POST',body:{tinh_nang:k.tinh_nang_chinh}}); setBusy(false); if(r.ok) notify('Đã tắt, quay về AI ngoài'); else notify(r.msg,'err'); };
  const tuHoc=async()=>{ setBusy(true); const r=await goi('/lop-hoc/tu-hoc',{method:'POST'}); setBusy(false); if(r.ok) notify((r.hoc||[]).length?('Máy bắt đầu học: '+r.hoc.join(' · ')):'Chưa có kỹ năng nào đủ mẫu mới để học'); else notify(r.msg,'err'); };
  const pill=k=>{ const m={MOI:['bg-emerald-100 text-emerald-800','có bản mới'],NHA:['bg-brand text-white','máy nhà'],DANG_HOC:['bg-amber-100 text-amber-800','đang học'],DU_MAU:['bg-amber-100 text-amber-800','đủ mẫu, chờ máy học'],GOM:['bg-slate-100 text-ink','đang gom ví dụ']}[k.trang_thai]||['bg-slate-100 text-ink',k.trang_thai]; return <Pill cls={m[0]}>{k.muc==='API'&&k.trang_thai==='GOM'?'AI ngoài':m[1]}</Pill>; };
  const loiKhuyen=k=>{ if(k.trang_thai==='MOI') return 'Bản mới khớp người '+k.ban_moi.diem+'/100 trên '+k.ban_moi.n_kiem+' mẫu kiểm (quy tắc cũ '+k.ban_moi.diem_truoc+'). '+(k.ban_moi.diem>=k.ban_moi.diem_truoc+5?'Hơn quy tắc cũ, xem thử rồi bật.':'Chưa hơn quy tắc cũ đủ 5 điểm, nên chờ máy học thêm.'); if(k.trang_thai==='NHA') return 'Máy nhà đang làm'+(k.dang_dung?(', bản '+k.dang_dung.phien_ban+' khớp người '+k.dang_dung.diem+'/100'):'')+'. AI ngoài dự phòng khi máy im.'; if(k.trang_thai==='DANG_HOC') return 'Máy đang học, vài phút nữa có bản mới để xem thử.'; if(k.trang_thai==='DU_MAU') return 'Đủ ví dụ. Máy tự học ở lượt tới (mỗi giờ), hoặc bấm "Học ngay".';
    const thieu=Math.max(0,k.can-k.so_mau); if(k.id==='cat_ghep') return 'Đang dùng luật 2,8 giây mỗi shot. Cần thêm '+thieu+' video đã dựng tay. Muốn học cả "chọn đoạn nào trong clip" thì nạp kèm thư mục goc.'; if(k.id==='viet') return 'Claude đang viết, có '+(k.so_kich_ban||0)+' kịch bản bán tốt làm tham chiếu. Mỗi bài anh duyệt hoặc trả kèm lý do là một ví dụ. Đủ '+k.can+' thì máy 3070 Ti học bản riêng.'; if(k.id==='giong') return 'Google đang đọc. Bật máy nhà (Piper) để tiết kiệm phí; nghe thử ở video nháp.'; return 'AI ngoài đang chọn. Cần thêm '+thieu+' ví dụ (chấm cảnh ở thẻ video, hoặc nạp video đã dựng).'; };
  return <div className="space-y-2"><div className="flex items-center gap-2 flex-wrap"><SectionTitle>Kỹ năng máy đang học</SectionTitle><span className="text-[11px] text-ink-muted">mỗi thẻ trả lời: đang làm bằng gì · học được bao nhiêu · có gì mới để bật</span>{duoc&&<Btn variant="ghost" className="!py-0.5 !px-2 text-[11px] ml-auto" onClick={tuHoc} disabled={busy}>Học ngay nếu đủ mẫu</Btn>}</div>
    <div className="grid md:grid-cols-2 xl:grid-cols-4 gap-3">{ds.map(k=><Card key={k.id} pad="p-3"><div className="flex items-center justify-between gap-2"><b className="text-ink text-sm">{k.icon} {k.ten}</b>{pill(k)}</div>
      <div className="mt-1"><span className="text-2xl font-bold text-ink">{k.trang_thai==='MOI'?k.ban_moi.diem:k.trang_thai==='NHA'&&k.dang_dung?k.dang_dung.diem:k.so_mau}</span><span className="text-[11px] text-ink-muted ml-1">{k.trang_thai==='MOI'||(k.trang_thai==='NHA'&&k.dang_dung)?('/100 khớp người'+(k.trang_thai==='MOI'?(' · trước '+k.ban_moi.diem_truoc):'')):('/'+k.can+' '+(k.id==='cat_ghep'?'video thành phẩm':k.id==='viet'?'kịch bản mẫu':'ví dụ'))}</span></div>
      <div className="h-1.5 rounded-full bg-slate-100 overflow-hidden mt-1"><div className="h-full bg-brand" style={{width:Math.min(100,Math.round((k.trang_thai==='MOI'?k.ban_moi.diem:k.trang_thai==='NHA'&&k.dang_dung?k.dang_dung.diem:k.so_mau/Math.max(1,k.can)*100)))+'%'}}/></div>
      <div className="text-[11px] text-ink-muted mt-1.5">{loiKhuyen(k)}{k.ban_rieng.length>0&&<> Có bản riêng cho {k.ban_rieng.map(b=>TEN_PV(b.pham_vi)+' ('+b.diem+')').join(', ')}.</>}{k.theo_dong.length>0&&k.trang_thai!=='MOI'&&<> Theo sản phẩm: {k.theo_dong.map(x=>x.dong+' '+x.n).join(' · ')}.</>}</div>
      {duoc&&<div className="flex gap-1.5 items-center mt-2 flex-wrap">{k.trang_thai==='MOI'&&<><Btn variant="brand" className="!py-1 !px-2 text-[11px]" onClick={()=>bat(k)} disabled={busy}>Bật bản mới</Btn>{(k.ban_moi.vi_du||[]).length>0&&<Btn variant="ghost" className="!py-1 !px-2 text-[11px]" onClick={()=>setXem(xem===k.id?null:k.id)}>{xem===k.id?'Đóng':'Xem thử'}</Btn>}</>}
        {k.trang_thai==='NHA'&&<Btn variant="ghost" className="!py-1 !px-2 text-[11px]" onClick={()=>tat(k)} disabled={busy}>Tắt, về AI ngoài</Btn>}
        {k.trang_thai!=='MOI'&&k.trang_thai!=='NHA'&&k.id==='giong'&&<Btn variant="soft" className="!py-1 !px-2 text-[11px]" onClick={()=>bat(k)} disabled={busy}>Bật máy nhà</Btn>}</div>}
      {xem===k.id&&k.ban_moi&&<div className="mt-2 space-y-1.5 text-[11px]">{k.ban_moi.vi_du.map((v,i)=><div key={i} className="rounded-lg border border-line p-1.5"><div className="text-ink-muted truncate" title={v.text}>💡 {v.hinh||v.text}</div><div className="grid grid-cols-3 gap-1 mt-1">{[['Người chọn',v.nguoi],['Bản mới',v.moi],['Quy tắc cũ',v.cu]].map(([t,id])=><div key={t} className={"rounded border p-1 "+(id===v.nguoi?'border-emerald-300 bg-emerald-50':'border-line')}><div className="aspect-[9/16] max-h-16 bg-slate-100 rounded overflow-hidden">{ten(id).media_type==='IMAGE'&&ten(id).media_url?<img src={ten(id).media_url} alt="" className="w-full h-full object-cover"/>:null}</div><div className="text-[10px] text-ink-muted mt-0.5">{t}</div><div className="text-[10px] text-ink truncate" title={ten(id).ten||id}>{ten(id).ten||id}</div></div>)}</div></div>)}</div>}
    </Card>)}</div></div>;
}
function NhatKyHoc({a}){
  const ds=a.nhat_ky_hoc||[]; const tt=a.tien_trinh||[];
  const dong=[...ds.map(x=>({t:x.at, m:(x.user_name||'')+' · '+x.hanh_dong+(x.ghi_chu?(': '+x.ghi_chu):''), loi:/lỗi|hỏng/i.test(x.ghi_chu||'')})), ...tt.filter(l=>['XONG','HONG'].includes(l.trang_thai)).map(l=>({t:l.xong_at||l.created_at, m:(l.viec==='hoc_thanh_pham'?'Máy dựng học thành phẩm':l.viec==='huan_luyen'?'Máy học '+(l.tham_so.tinh_nang||''):l.viec==='chay_agent'?('Trạm '+(l.tham_so.viec==='kalodata_video'?'quét Kalodata':'tải TikTok')):l.viec)+(l.ket_qua?(' — '+l.ket_qua):''), loi:l.trang_thai==='HONG'}))].sort((x,y)=>String(y.t).localeCompare(String(x.t))).slice(0,12);
  return <Card pad="p-3"><SectionTitle className="mb-1">3 · Nhật ký</SectionTitle>{dong.length===0?<div className="text-xs text-ink-muted">Chưa có gì.</div>:<div className="space-y-0.5 text-[11px]">{dong.map((x,i)=><div key={i} className="flex gap-2"><span className="text-slate-400 shrink-0 w-24">{fmtDate(x.t)}</span><span className={x.loi?'text-rose-700':'text-ink-2'}>{x.m}</span></div>)}</div>}</Card>;
}
function HuanLuyenAI({a, phan}){
  const { me, goi, notify, db } = useApp(); const duoc=laGat(me); const [busy,setBusy]=useState(false); const [ly,setLy]=useState({}); const [pvChon,setPvChon]=useState({});
  const mayHoc=(db.may_ghep||[]).filter(m=>m.song&&(m.kha_nang||[]).includes('huan_luyen')); const mayDung=(db.may_ghep||[]).filter(m=>m.song&&(m.kha_nang||[]).includes('dung_video'));
  const dongs=a.dong_san_pham||[]; const tk=a.mau_thong_ke||[]; const pvtk=a.pham_vi_thong_ke||[]; const pbs=a.phien_ban||[];
  const hoc=async(tn,pv)=>{ setBusy(true); const r=await goi('/ai/huan-luyen',{method:'POST',body:{tinh_nang:tn, pham_vi:pv||'chung'}}); setBusy(false); if(r.ok) notify(r.trung?'Lệnh huấn luyện đang chờ máy':('Đã giao máy huấn luyện '+TEN_PV(pv)+' với '+r.so_mau+' mẫu')); else notify(r.msg,'err'); };
  const quyet=async(p,q)=>{ setBusy(true); const r=await goi('/ai/phien-ban/'+p.id+'/'+q,{method:'POST',body:{ly_do:ly[p.id]||''}}); setBusy(false); if(r.ok) notify(q==='duyet'?'Đã bật phiên bản':'Đã từ chối'); else notify(r.msg,'err'); };
  const dt=(a.dinh_tuyen||[]).filter(d=>['NHIN','NGON_NGU','TTS'].includes(d.loai)&&d.mo_hinh_mo);
  const mucPill=m=><Pill cls={m==='MO'?'bg-emerald-100 text-emerald-800':m==='BONG'?'bg-amber-100 text-amber-800':'bg-slate-100 text-ink'}>{m==='MO'?'MỞ':m==='BONG'?'BÓNG':'API'}</Pill>;
  const soMau=(tn,pv)=>{ if(!pv||pv==='chung') return (tk.find(t=>t.tinh_nang===tn)||{}).da_cham||0; const [k,v]=pv.startsWith('dong:')?['dong',pv.slice(5)]:['muc_dich',pv.slice(9)]; return pvtk.filter(x=>x.tinh_nang===tn&&x[k]===v).reduce((s,x)=>s+(x.da_cham||0),0); };
  const banRieng=tn=>pbs.filter(p=>p.tinh_nang===tn&&p.trang_thai==='DUYET'&&p.pham_vi&&p.pham_vi!=='chung');
  const soMo=dt.filter(d=>d.muc==='MO').length, soBong=dt.filter(d=>d.muc==='BONG').length, thieu=dt.filter(d=>soMau(d.tinh_nang)<(d.min_mau||30)).length, choDuyet=pbs.filter(p=>p.trang_thai==='CHO_DUYET').length;
  return <div className="space-y-3">
    <Card pad="p-3"><SectionTitle className="mb-1">1 · Mô hình đang học</SectionTitle>
      <div className="text-[11px] text-ink-muted mb-2">API = thuê AI ngoài · BÓNG = mô hình mở chạy song song để so · MỞ = mô hình mở tự làm, API dự phòng. Mỗi tính năng có bản <b>chung</b>; dòng sản phẩm đủ mẫu thì huấn luyện thêm <b>bản riêng</b>, máy chỉ dùng bản riêng khi điểm kiểm cao hơn bản chung ≥ 3. Máy huấn luyện đang bật: {mayHoc.length?mayHoc.map(m=>m.ten).join(', '):(mayDung.length?mayDung.map(m=>m.ten).join(', ')+' (chỉ chọn đoạn/ghép)':'không')}.</div>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-2">{[['đầu đã MỞ',soMo,'text-emerald-700'],['đầu đang BÓNG',soBong,'text-amber-700'],['đầu thiếu mẫu',thieu,'text-rose-700'],['phiên bản chờ duyệt',choDuyet,'text-ink']].map(([t,n,c])=><div key={t} className="rounded-xl border border-line p-2"><div className={"text-2xl font-bold "+c}>{n}</div><div className="text-[11px] text-ink-muted">{t}</div></div>)}</div>
      <div className="overflow-x-auto"><table className="w-full text-xs min-w-[640px]"><thead><tr className="text-left text-ink-muted"><th className="px-2 py-1">Tính năng</th><th className="px-2 py-1">Mô hình mở</th><th className="px-2 py-1">Mức</th><th className="px-2 py-1 text-right">Mẫu có / cần</th><th className="px-2 py-1 text-right">Điểm</th><th className="px-2 py-1">Bản riêng</th><th className="px-2 py-1">Huấn luyện</th></tr></thead>
        <tbody>{dt.map(d=>{ const n=soMau(d.tinh_nang); const can=d.min_mau||30; const rieng=banRieng(d.tinh_nang); const pv=pvChon[d.tinh_nang]||'chung'; const nPv=soMau(d.tinh_nang,pv); const laNhin=d.loai==='NHIN';
          return <tr key={d.tinh_nang} className="border-t border-line/60"><td className="px-2 py-1"><b className="text-ink">{d.ten}</b><div className="text-[10px] text-ink-muted">{d.loai==='NHIN'?'nhìn':d.loai==='TTS'?'giọng đọc':'ngôn ngữ'}{d.de_nghi?(' · máy đề nghị '+d.de_nghi):''}</div></td><td className="px-2 py-1">{d.mo_hinh_mo}</td><td className="px-2 py-1">{mucPill(d.muc)}</td><td className={"px-2 py-1 text-right tabular-nums "+(n<can?'text-rose-700 font-semibold':'')}>{n} / {can}</td><td className="px-2 py-1 text-right tabular-nums">{d.diem||'—'}</td>
            <td className="px-2 py-1">{rieng.length?rieng.map(p=><Pill key={p.id} cls="bg-sky-100 text-sky-800 mr-1">{TEN_PV(p.pham_vi)} · {p.danh_gia.diem}</Pill>):<span className="text-ink-muted">chỉ bản chung</span>}</td>
            <td className="px-2 py-1">{duoc&&laNhin?<div className="flex gap-1 items-center"><Select className="!py-0.5 !px-1 text-[10px] !w-36" value={pv} onChange={e=>setPvChon({...pvChon,[d.tinh_nang]:e.target.value})}><option value="chung">chung ({n})</option>{dongs.map(x=><option key={x} value={'dong:'+x}>{x} ({soMau(d.tinh_nang,'dong:'+x)})</option>)}<option value="muc_dich:BAN_HANG">Bán hàng ({soMau(d.tinh_nang,'muc_dich:BAN_HANG')})</option><option value="muc_dich:BRAND">Brand ({soMau(d.tinh_nang,'muc_dich:BRAND')})</option></Select><Btn variant="brand" className="!py-0.5 !px-2 text-[10px]" onClick={()=>hoc(d.tinh_nang,pv)} disabled={busy||nPv<10} title={nPv<10?'cần ≥ 10 mẫu đã chấm trong phạm vi này':''}>🎓</Btn></div>:d.loai==='NGON_NGU'?<span className="text-[10px] text-ink-muted">LoRA trên GPU thuê (xuất tập mẫu bên dưới)</span>:<span className="text-[10px] text-ink-muted">không huấn luyện, chỉ chọn bản</span>}</td></tr>; })}</tbody></table></div>
      <div className="mt-2 text-[11px] text-ink-muted"><b>Ngôn ngữ mở (LoRA, ADR-009c):</b> trên máy con chạy <code>node may-dung.mjs xuat-tap-mau soan_nhap_agent</code> (thêm <code>--pham_vi=dong:Keo</code> để xuất riêng một dòng) → GPU thuê chạy <a className="underline" href="/tools/may-dung/huan-luyen-ngon-ngu.py" download>huan-luyen-ngon-ngu.py</a> → GGUF → <code>ollama create kingsmen-qwen:vN</code> → <code>node may-dung.mjs phien-ban kingsmen-qwen:vN</code> → duyệt ở mục 5. Kịch bản bán tốt trong kho (mục 4) đã nằm sẵn trong tập mẫu. Tính năng ngôn ngữ được gạt MỞ: {(a.mo_ho_tro||[]).join(', ')}.</div>
    </Card>
    {phan!=='mo_hinh'&&<><NapDuLieu a={a} duoc={duoc} dongs={dongs}/>
    <TienTrinh a={a} db={db}/>
    <KhoMau a={a} duoc={duoc} dongs={dongs}/></>}
    {phan!=='mo_hinh'&&<Card pad=""><div className="px-3 py-2 border-b border-line"><SectionTitle>5 · Phiên bản mô hình (người duyệt mới bật — G4)</SectionTitle></div>
      {pbs.length===0?<Empty>Chưa có phiên bản nào.</Empty>:<div className="divide-y divide-line">{pbs.map(p=><div key={p.id} className="px-3 py-2 text-xs flex items-center gap-2 flex-wrap"><Pill cls={p.trang_thai==='DUYET'?'bg-emerald-100 text-emerald-800':p.trang_thai==='TU_CHOI'?'bg-rose-100 text-rose-700':'bg-amber-100 text-amber-800'}>{p.trang_thai==='CHO_DUYET'?'chờ duyệt':p.trang_thai==='DUYET'?'đang dùng':'từ chối'}</Pill><span className="font-semibold text-ink">{p.mo_hinh_id} · {p.phien_ban}</span>{p.pham_vi&&p.pham_vi!=='chung'&&<Pill cls="bg-sky-100 text-sky-800">bản riêng: {TEN_PV(p.pham_vi)}</Pill>}<span className="text-ink-muted">{p.tinh_nang} · khớp người <b>{p.danh_gia.diem}</b>/100 trên {p.danh_gia.n_kiem} mẫu kiểm (quy tắc {p.danh_gia.diem_truoc}) · học {p.danh_gia.n_hoc} · {p.may} · {fmtDate(p.created_at)}{p.duyet_boi?(' · '+p.duyet_boi+(p.ly_do?(': '+p.ly_do):'')):''}</span>
...
// 2 · bốn cửa nạp — mỗi cửa hỏi: ở đâu · dòng sản phẩm nào · mục đích gì
function NapDuLieu({a, duoc, dongs}){
  const { goi, notify, db } = useApp(); const tramSong=!!((db.tram||{}).trang_thai||{}).song; const mayDung=(db.may_ghep||[]).filter(m=>m.song&&(m.kha_nang||[]).includes('dung_video'));
  const kd=a.kalodata||{}; const cho=a.tai_tiktok_cho||[]; const kho=a.kho_thanh_pham||[];
  const [busy,setBusy]=useState(false);
  const [drive,setDrive]=useState({link:'',dong:'',md:'',toi_da:20}); const [local,setLocal]=useState({link:'',dong:'',md:'',toi_da:20}); const [tt,setTt]=useState({kenh:'',dong:'',md:'',toi_da:20});
  const [kl,setKl]=useState({nganh:(kd.nganh||[]).join(', '), top_n:kd.top_n||10, tu_dong:kd.tu_dong!==false, dong:kd.dong||'', md:kd.muc_dich||'BAN_HANG'});
  useEffect(()=>{ setKl({nganh:(kd.nganh||[]).join(', '), top_n:kd.top_n||10, tu_dong:kd.tu_dong!==false, dong:kd.dong||'', md:kd.muc_dich||'BAN_HANG'}); },[JSON.stringify(kd.nganh),kd.top_n,kd.tu_dong,kd.dong,kd.muc_dich]);
  const nap=async(x,nguon)=>{ const link=(nguon==='TIKTOK'?x.kenh:x.link).trim(); if(!link) return; setBusy(true); const r=await goi('/kho-thanh-pham/nap',{method:'POST',body:{link, toi_da:x.toi_da, nguon:nguon==='TIKTOK'?'TIKTOK':undefined, dong:x.dong||undefined, muc_dich:x.md||undefined}}); setBusy(false); if(!r.ok) return notify(r.msg,'err'); notify(nguon==='TIKTOK'?('Đã xin Trạm tải kênh '+r.kenh):(r.trung?'Máy dựng đã có lệnh này':'Đã giao máy dựng cắt shot & học')); };
  const luuKl=async()=>{ setBusy(true); const r=await goi('/kalodata',{method:'PUT',body:{nganh:kl.nganh, top_n:kl.top_n, tu_dong:kl.tu_dong, dong:kl.dong||undefined, muc_dich:kl.md}}); setBusy(false); if(r.ok) notify('Đã lưu cấu hình Kalodata'); else notify(r.msg,'err'); };
  const quet=async()=>{ setBusy(true); const r=await goi('/kalodata/quet',{method:'POST'}); setBusy(false); if(r.ok) notify(r.trung?'Trạm đã có lệnh quét':'Đã xin Trạm quét Kalodata'); else notify(r.msg,'err'); };
  const lanCuoi=(ng)=>{ const v=kho.filter(t=>t.nguon===ng); return v.length?(v.length+' video · mới nhất '+fmtDate(v[0].created_at)):'chưa nạp lần nào'; };
  const tkKalo=(db.tram||{}).tai_khoan_kalodata; const kq=kd.ket_qua_cuoi;
  const The=({title,children,mau})=><div className={"rounded-xl border border-line p-3 space-y-2 border-t-4 "+(mau||'border-t-brand')}><div className="font-semibold text-ink text-sm">{title}</div>{children}</div>;
  return <Card pad="p-3"><SectionTitle className="mb-1">2 · Nạp dữ liệu học</SectionTitle>
    <div className="text-[11px] text-ink-muted mb-2">Nhãn <b>dòng sản phẩm</b> và <b>mục đích</b> đi theo từng video vào kho (để trống = chung). Máy dựng đang bật: {mayDung.length?mayDung.map(m=>m.ten).join(', '):<b className="text-rose-700">không</b>} · Trạm: {tramSong?'🟢 sống':<b className="text-rose-700">im</b>}.</div>
    <div className="grid md:grid-cols-2 xl:grid-cols-4 gap-3">
      <The title="📁 Drive"><div className="text-[11px] text-ink-muted">Video đã dựng tay. Thư mục con <code>goc</code> chứa clip quay gốc → học thêm <i>chọn đoạn</i>.</div>
        <Input className="!py-1.5 text-xs" placeholder="link thư mục Drive (ai có link cũng xem)" value={drive.link} onChange={e=>setDrive({...drive,link:e.target.value})}/>
        <div className="flex gap-1 flex-wrap items-center"><NhanPhamVi dong={drive.dong} setDong={v=>setDrive({...drive,dong:v})} mucDich={drive.md} setMucDich={v=>setDrive({...drive,md:v})} dongs={dongs}/><Input type="number" min="1" max="60" className="!py-1.5 text-xs !w-16" value={drive.toi_da} onChange={e=>setDrive({...drive,toi_da:+e.target.value||20})}/></div>
        {duoc&&<Btn variant="brand" className="!py-1.5 text-xs" onClick={()=>nap(drive,'DRIVE')} disabled={busy||!/drive\.google\.com\/drive\/(u\/\d+\/)?folders\//.test(drive.link)}>Nạp & học</Btn>}<div className="text-[11px] text-ink-muted">{lanCuoi('DRIVE')}</div></The>
      <The title="💻 Thư mục máy dựng"><div className="text-[11px] text-ink-muted">Đường dẫn trên máy dựng (vd <code>D:\Video da dung</code>). Cùng luật thư mục <code>goc</code>.</div>
        <Input className="!py-1.5 text-xs" placeholder="D:\thu-muc" value={local.link} onChange={e=>setLocal({...local,link:e.target.value})}/>
        <div className="flex gap-1 flex-wrap items-center"><NhanPhamVi dong={local.dong} setDong={v=>setLocal({...local,dong:v})} mucDich={local.md} setMucDich={v=>setLocal({...local,md:v})} dongs={dongs}/><Input type="number" min="1" max="60" className="!py-1.5 text-xs !w-16" value={local.toi_da} onChange={e=>setLocal({...local,toi_da:+e.target.value||20})}/></div>
        {duoc&&<Btn variant="brand" className="!py-1.5 text-xs" onClick={()=>nap(local,'LOCAL')} disabled={busy||!/^[A-Za-z]:[\\/]|^\//.test(local.link.trim())}>Nạp & học</Btn>}<div className="text-[11px] text-ink-muted">{lanCuoi('LOCAL')}</div></The>
      <The title="🎵 Kênh TikTok"><div className="text-[11px] text-ink-muted">Trạm tải video kèm lượt xem bằng tài khoản TikTok đã đăng nhập. Xem cao hơn trung vị kênh → mẫu nặng hơn.</div>
        <Input className="!py-1.5 text-xs" placeholder="@tenkenh hoặc link tiktok.com/@tenkenh" value={tt.kenh} onChange={e=>setTt({...tt,kenh:e.target.value})}/>
        <div className="flex gap-1 flex-wrap items-center"><NhanPhamVi dong={tt.dong} setDong={v=>setTt({...tt,dong:v})} mucDich={tt.md} setMucDich={v=>setTt({...tt,md:v})} dongs={dongs}/><Input type="number" min="1" max="60" className="!py-1.5 text-xs !w-16" value={tt.toi_da} onChange={e=>setTt({...tt,toi_da:+e.target.value||20})}/></div>
        {duoc&&<Btn variant="brand" className="!py-1.5 text-xs" onClick={()=>nap(tt,'TIKTOK')} disabled={busy||!/tiktok\.com\/@|^@[A-Za-z0-9_.]{2,}$/.test(tt.kenh.trim())} title={!tramSong?'Trạm văn phòng đang im':''}>Xin Trạm tải</Btn>}
        <div className="text-[11px] text-ink-muted">{lanCuoi('TIKTOK')}{cho.filter(c=>!c.nguon||c.nguon==='TIKTOK').length>0&&<div>Trạm đang chờ tải: {cho.filter(c=>!c.nguon||c.nguon==='TIKTOK').map(c=>c.kenh+' ('+c.toi_da+')').join(', ')}</div>}</div></The>
      <The title="📈 Kalodata" mau="border-t-amber-500"><div className="text-[11px] text-ink-muted">Video TikTok Shop bán chạy theo ngành. Doanh thu thay lượt xem làm trọng số; lời thoại thành kịch bản tham chiếu. Cần tài khoản Kalodata đã đăng nhập ở Trạm › Tài khoản.</div>
        <Input className="!py-1.5 text-xs" placeholder="ngành hàng Kalodata, cách nhau dấu phẩy" value={kl.nganh} onChange={e=>setKl({...kl,nganh:e.target.value})}/>
        <div className="flex gap-1 flex-wrap items-center"><NhanPhamVi dong={kl.dong} setDong={v=>setKl({...kl,dong:v})} mucDich={kl.md} setMucDich={v=>setKl({...kl,md:v})} dongs={dongs}/><Input type="number" min="3" max="50" className="!py-1.5 text-xs !w-16" value={kl.top_n} onChange={e=>setKl({...kl,top_n:+e.target.value||10})}/><label className="text-[11px] text-ink-muted flex items-center gap-1"><input type="checkbox" checked={kl.tu_dong} onChange={e=>setKl({...kl,tu_dong:e.target.checked})}/> tự quét tuần</label></div>
        {duoc&&<div className="flex gap-2"><Btn variant="soft" className="!py-1.5 text-xs" onClick={luuKl} disabled={busy}>Lưu</Btn><Btn variant="brand" className="!py-1.5 text-xs" onClick={quet} disabled={busy||!(kd.nganh||[]).length} title={!tramSong?'Trạm văn phòng đang im':''}>Quét ngay</Btn></div>}
        <div className="text-[11px] text-ink-muted">{(kd.nganh||[]).length?('ngành: '+kd.nganh.join(' · ')+' · top '+(kd.top_n||10)):'chưa đặt ngành hàng'}{kd.lan_cuoi?(' · quét lần cuối '+fmtDate(kd.lan_cuoi)):''}{kq?(' → '+kq.nganh+': '+kq.so+' video'+(kq.loi?(' · lỗi: '+kq.loi):'')):''}{kd.quet_ngay?' · đang chờ Trạm quét':''}{cho.filter(c=>c.nguon==='KALODATA').length>0&&(' · Trạm đang tải '+cho.filter(c=>c.nguon==='KALODATA').map(c=>c.nganh+' ('+c.toi_da+')').join(', '))}</div></The>
    </div></Card>;
}
// 3 · tiến trình — mỗi lệnh nạp/học là một dòng, trạng thái và lỗi nguyên văn từ máy
function TienTrinh({a, db}){
  const ds=a.tien_trinh||[]; const mays=db.may_ghep||[]; const ten=id=>id==='tram'?'Trạm':((mays.find(m=>m.id===id)||{}).ten||id||'máy');
  const moTa=l=>{ const t=l.tham_so||{}; if(l.viec==='chay_agent') return t.viec==='kalodata_video'?'Trạm quét Kalodata':'Trạm tải kênh TikTok'; if(l.viec==='hoc_thanh_pham') return 'Máy dựng học thành phẩm · '+(t.nguon||'')+(t.kenh?(' '+t.kenh):'')+(t.dong?(' · '+t.dong):''); if(l.viec==='huan_luyen') return 'Huấn luyện '+(t.tinh_nang||'')+' · '+TEN_PV(t.pham_vi); if(l.viec==='phan_tich_footage') return 'Phân tích footage'; if(l.viec==='nap_drive') return 'Nạp footage Drive'; return l.viec; };
  const pill=s=><Pill cls={s==='XONG'?'bg-emerald-100 text-emerald-800':s==='HONG'?'bg-rose-100 text-rose-700':s==='DA_GUI'?'bg-sky-100 text-sky-800':'bg-slate-100 text-ink'}>{s==='CHO'?'chờ máy':s==='DA_GUI'?'đang chạy':s==='XONG'?'xong':s==='HONG'?'lỗi':s}</Pill>;
  return <Card pad="p-3"><SectionTitle className="mb-1">3 · Việc đang chạy</SectionTitle><div className="text-[11px] text-ink-muted mb-2">Trạm quét/tải → máy dựng cắt shot, nghe lời, xếp cỡ cảnh → app ghi mẫu → sẵn sàng huấn luyện. Kết quả và lỗi ghi nguyên văn từ máy.</div>
    {ds.length===0?<div className="text-xs text-ink-muted">Chưa có việc nào.</div>:<div className="space-y-1">{ds.map(l=><div key={l.id} className="flex gap-2 items-start text-[11px] rounded-lg border border-line/70 px-2 py-1"><span className="text-slate-400 shrink-0 w-24">{fmtDate(l.created_at)}</span><span className="text-ink font-semibold shrink-0">{moTa(l)}</span><span className="text-ink-muted">{ten(l.may_id)}{l.ket_qua?(' — '+l.ket_qua):''}</span><span className="ml-auto shrink-0">{pill(l.trang_thai)}</span></div>)}</div>}</Card>;
}
// 4 · kho mẫu — video thành phẩm: xem, gắn nhãn, loại
function KhoMau({a, duoc, dongs}){
  const { goi, notify } = useApp(); const [loc,setLoc]=useState({nguon:'',dong:'',loi:false}); const [busy,setBusy]=useState(false); const [sua,setSua]=useState(null);
  const kho=a.kho_thanh_pham||[]; const nguonTen={DRIVE:'Drive',LOCAL:'Máy dựng',TIKTOK:'TikTok',REELS:'Reels',KALODATA:'Kalodata'};
  const ds=kho.filter(t=>(!loc.nguon||t.nguon===loc.nguon)&&(!loc.dong||(loc.dong==='_'?!t.dong:t.dong===loc.dong))&&(!loc.loi||t.kich_ban));
  const ganNhan=async(t)=>{ setBusy(true); const r=await goi('/kho-thanh-pham/'+t.id,{method:'PATCH',body:{dong:sua.dong||null, muc_dich:sua.md||null}}); setBusy(false); if(r.ok){ notify('Đã gắn nhãn — mẫu của video này đổi theo'); setSua(null); } else notify(r.msg,'err'); };
  const loai=async(t)=>{ if(sua&&sua.id===t.id&&sua.xoa){ setBusy(true); const r=await goi('/kho-thanh-pham/'+t.id,{method:'DELETE'}); setBusy(false); if(r.ok){ notify('Đã loại khỏi kho cùng mẫu của nó'); setSua(null); } else notify(r.msg,'err'); } else setSua({id:t.id, xoa:true}); };
  const Chip=({on,children,onClick})=><button onClick={onClick} className={"px-2 py-0.5 rounded-full text-[11px] border "+(on?'bg-brand text-white border-brand':'border-line text-ink-muted')}>{children}</button>;
  return <Card pad="p-3"><SectionTitle className="mb-1">Video thành phẩm đã học ({kho.length}) · sửa dòng, mục đích, gán nhãn từng video</SectionTitle>
    <div className="flex gap-1 flex-wrap mb-2"><Chip on={!loc.nguon} onClick={()=>setLoc({...loc,nguon:''})}>Tất cả {kho.length}</Chip>{['KALODATA','TIKTOK','DRIVE','LOCAL'].filter(n=>kho.some(t=>t.nguon===n)).map(n=><Chip key={n} on={loc.nguon===n} onClick={()=>setLoc({...loc,nguon:n})}>{nguonTen[n]} {kho.filter(t=>t.nguon===n).length}</Chip>)}<span className="w-2"/>{dongs.filter(d=>kho.some(t=>t.dong===d)).map(d=><Chip key={d} on={loc.dong===d} onClick={()=>setLoc({...loc,dong:loc.dong===d?'':d})}>{d} {kho.filter(t=>t.dong===d).length}</Chip>)}{kho.some(t=>!t.dong)&&<Chip on={loc.dong==='_'} onClick={()=>setLoc({...loc,dong:loc.dong==='_'?'':'_'})}>chung {kho.filter(t=>!t.dong).length}</Chip>}<Chip on={loc.loi} onClick={()=>setLoc({...loc,loi:!loc.loi})}>🗣 có lời thoại {kho.filter(t=>t.kich_ban).length}</Chip></div>
    {ds.length===0?<div className="text-xs text-ink-muted">Chưa có video thành phẩm nào{kho.length?' khớp bộ lọc':''}. Nạp ở mục 2.</div>:<div className="divide-y divide-line/70">{ds.map(t=><div key={t.id} className="py-2 text-xs">
      <div className="flex items-center gap-2 flex-wrap"><b className="text-ink truncate max-w-[280px]" title={t.thu_muc}>{t.link?<a className="underline" href={t.link} target="_blank" rel="noreferrer">{t.kenh?t.kenh+' · ':''}{t.ten}</a>:t.ten}</b><Pill cls={t.nguon==='KALODATA'?'bg-amber-100 text-amber-800':t.nguon==='TIKTOK'?'bg-emerald-100 text-emerald-800':'bg-slate-100 text-ink'}>{nguonTen[t.nguon]||t.nguon}</Pill>{t.dong&&<Pill cls="bg-sky-100 text-sky-800">{t.dong}</Pill>}{t.muc_dich&&<Pill cls="bg-slate-100 text-ink">{MUC_DICH_TEN[t.muc_dich]}</Pill>}
        {duoc&&<span className="ml-auto flex gap-1">{sua&&sua.id===t.id&&!sua.xoa?<><NhanPhamVi dong={sua.dong} setDong={v=>setSua({...sua,dong:v})} mucDich={sua.md} setMucDich={v=>setSua({...sua,md:v})} dongs={dongs}/><Btn variant="ok" className="!py-0.5 !px-2 text-[10px]" onClick={()=>ganNhan(t)} disabled={busy}>Lưu</Btn><Btn variant="ghost" className="!py-0.5 !px-2 text-[10px]" onClick={()=>setSua(null)}>Huỷ</Btn></>:<><Btn variant="ghost" className="!py-0.5 !px-2 text-[10px]" onClick={()=>setSua({id:t.id, dong:t.dong||'', md:t.muc_dich||''})}>Gắn nhãn</Btn><Btn variant="ghost" className={"!py-0.5 !px-2 text-[10px] "+(sua&&sua.id===t.id&&sua.xoa?'!bg-rose-600 !text-white':'')} onClick={()=>loai(t)} disabled={busy}>{sua&&sua.id===t.id&&sua.xoa?'Bấm lần nữa để loại':'Loại'}</Btn></>}</span>}</div>
      <div className="text-[11px] text-ink-muted tabular-nums">{t.doanh_thu!=null&&<>doanh thu <b>{fmtVND(t.doanh_thu)}</b>{t.luot_ban!=null?(' · '+t.luot_ban.toLocaleString('vi-VN')+' lượt bán'):''} · </>}{t.luot_xem!=null?(t.luot_xem.toLocaleString('vi-VN')+' xem · '):''}{t.dai}s · {t.so_shot} shot · nhịp {t.nhip}s{t.co_goc?(' · '+t.co_goc+' shot khớp clip gốc'):''}{t.san_pham?(' · '+t.san_pham):''} · {t.ngay_dang||fmtDate(t.created_at)}</div>
      {t.kich_ban&&<div className="text-[11px] text-ink-2 italic truncate" title={t.kich_ban}>🗣 "{t.kich_ban}"</div>}
    </div>)}</div>}
  </Card>;
}
function ChiPhiMoHinh({a}){
  ...
  <Card pad=""><div className="px-3 py-2 border-b border-line"><SectionTitle>Chi phí tháng này theo mô hình — {tong.toFixed(2)} USD</SectionTitle></div>
    {ds.length===0?<Empty>Chưa có lượt gọi nào tháng này.</Empty>:<div className="overflow-x-auto"><table className="w-full text-xs min-w-[560px]"><thead><tr className="text-left text-ink-muted"><th className="px-3 py-1">Mô hình</th><th className="px-3 py-1">Nhà cung cấp</th><th className="px-3 py-1">Lượt</th><th className="px-3 py-1">Token vào / ra</th><th className="px-3 py-1">USD</th></tr></thead><tbody>{ds.map((x,i)=><tr key={i} className="border-t border-line/60"><td className="px-3 py-1 font-semibold text-ink">{x.mo_hinh}</td><td className="px-3 py-1">{x.provider}</td><td className="px-3 py-1">{x.so_lan}</td><td className="px-3 py-1">{fmtSo(x.vao)} / {fmtSo(x.ra)}</td><td className="px-3 py-1">{(x.usd||0).toFixed(3)}{x.provider==='may_ghep'&&<span className="text-emerald-700"> (mở, 0 đ)</span>}</td></tr>)}</tbody></table></div>}
