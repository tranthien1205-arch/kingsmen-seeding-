...
export function taoMau(H) {
  ...
  async function nguonHoc(env) { await dam(env);
    ...
    for (const t of theoDoi) { const g = nhomTheoKey[t.key] || (t.loai === 'FOOTAGE' ? ds.find((x) => x.loai === 'FOOTAGE' && t.key.indexOf('FOOTAGE|' + x.muc_id + '|') === 0) : null);
      if (g) { g.theo_doi = t; if (!g.nap) g.nap = t.nap; } else ds.push({ key: t.key, loai: t.loai, ten: t.ten, phu: (t.tk_tram ? 'tài khoản Trạm ' + t.tk_tram + ' · ' : '') + 'đã đăng ký, chưa học video nào', nap: t.nap, theo_doi: t, so_video: 0, dong: t.dong ? { [t.dong]: 0 } : {}, hinh: 0, anh: 0, thay: 0, nguoi: 0, loi: 0, nghe_sai: 0, ban_xem: 0, cuoi: t.them_luc || '', video: [] }); }
