...
export function taoMau(H) {
  ...
  async function tuGanDong(env) { await dam(env); const rows = (await env.DB.prepare(`SELECT doi_tuong_id, ten, text, nhan_thay, nhan_nguoi FROM mau_doan WHERE hieu_luc=1 AND nguon='THANH_PHAM'`).all()).results; const phieu = {}; const hienTai = {};
  ...
  async function nguonHoc(env) { await dam(env);
    ...
    for (const v of (await env.DB.prepare(`SELECT id, ten, nguon, kenh, thu_muc, dong, proxy_url, created_at FROM kho_thanh_pham`).all()).results) { const tm = String(v.thu_muc || ''); let key, base;
      ...
      else if (v.nguon === 'TIKTOK' || v.nguon === 'REELS') { const k = v.kenh || '@' + (tm.split(/[\\/]/).pop() || '?'); key = 'TIKTOK|' + k; base = { loai: 'TIKTOK', ten: k, phu: 'kênh TikTok', nap: /^@/.test(k) ? k : null }; }
    ...
    for (const v of (await env.DB.prepare(`SELECT t.id, t.ten, t.created_at, t.muc_id, mu.tieu_de, sp.dong FROM tai_san t LEFT JOIN muc_noi_dung mu ON mu.id=t.muc_id LEFT JOIN san_pham sp ON sp.id=mu.san_pham_id WHERE t.loai='FOOTAGE' AND t.media_type='VIDEO'`).all()).results)
      cong('FOOTAGE|' + (v.muc_id || ''), { key: 'FOOTAGE|' + (v.muc_id || ''), loai: 'FOOTAGE', ten: v.tieu_de || 'footage chưa gắn mục', phu: 'footage quay · dòng theo sản phẩm của mục', nap: null, muc_id: v.muc_id }, v, v.dong, v.created_at, true);
    ...
    return { nguon: ds, lenh, may, dongs: (await dsDongChuan(env)).map((x) => x.dong) }; }
