...
export function taoMau(H) {
  ...
  async function tuGanDong(env) { await dam(env); const rows = (await env.DB.prepare(`SELECT doi_tuong_id, ten, text, nhan_thay, nhan_nguoi FROM mau_doan WHERE hieu_luc=1 AND nguon='THANH_PHAM' AND (dong IS NULL OR dong='')`).all()).results; const phieu = {};
    ...
    const gan = []; for (const [v, p] of Object.entries(phieu)) { const ds = Object.entries(p).sort((a, b) => b[1] - a[1]); const tong = ds.reduce((a, x) => a + x[1], 0); if (ds[0][1] >= 2 && ds[0][1] / tong >= 0.7) gan.push([v, ds[0][0]]); }
    const now = nowISO(); for (const [v, d] of gan) { await env.DB.prepare(`UPDATE kho_thanh_pham SET dong=? WHERE id=? AND (dong IS NULL OR dong='')`).bind(d, v).run(); await env.DB.prepare(`UPDATE mau_doan SET dong=?, buoc_thu=0, updated_at=? WHERE doi_tuong_id=? AND (dong IS NULL OR dong='')`).bind(d, now, v).run(); }
