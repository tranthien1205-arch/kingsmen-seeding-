...
async function bootstrap(env, u){
  ...
  return {
    ...
    ai_nao: xem ? await (async()=>{ const th=thangHienTai(); return {
      ...
      kho_thanh_pham: await all(`SELECT id,ten,nguon,thu_muc,dai,so_shot,nhip,co_goc,created_at,luot_xem,luot_thich,ngay_dang,link,kenh FROM kho_thanh_pham ORDER BY COALESCE(luot_xem,-1) DESC, created_at DESC LIMIT 80`),
...
async function handleApi(request, env){
  ...
  if(path.startsWith('/hub/')){
    ...
    if(path==='/hub/tiktok-da-tai' && method==='POST'){ const kenh=chuoi(body.kenh,60), thu=chuoi(body.thu_muc,300); if(!kenh||!thu) return json({error:'Thiếu kênh/thư mục'},400);
      ...
      const r=await taoLenhTram(env,'hoc_thanh_pham',{nguon:'TIKTOK', kenh, duong_dan:thu, toi_da:Math.max(1,Math.min(60,so(body.so,20)))}, MAY('Trạm'), mayId); return json({ ok:true, giao:true, lenh_id:r.id }); }
    ...
    if(path==='/hub/thanh-pham' && method==='POST'){ const ten=chuoi(body.ten,200); const nid=chuoi(body.nguon_id,120)||ten; if(!ten) return json({error:'Thiếu tên'},400); if(await env.DB.prepare(`SELECT id FROM kho_thanh_pham WHERE nguon_id=?`).bind(nid).first()) return json({ok:true, trung:true});
      ...
      const luotXem=body.luot_xem==null?null:Math.max(0,Math.round(so(body.luot_xem))), kenh=chuoi(body.kenh,80)||null;
      const id=uid('tp'); await env.DB.prepare(`INSERT INTO kho_thanh_pham (id,ten,nguon,nguon_id,thu_muc,dai,so_shot,nhip,co_goc,phan_tich,created_at,luot_xem,luot_thich,ngay_dang,link,kenh) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(id, ten, ['DRIVE','LOCAL','TIKTOK','REELS'].includes(chuoi(body.nguon,10))?chuoi(body.nguon,10):'DRIVE', nid, chuoi(body.thu_muc,200), +dai.toFixed(2), shots.length, +(dai/shots.length).toFixed(2), coGoc, JSON.stringify({shots:shots.map(x=>({...x, goc:x.goc?{...x.goc, doan:undefined}:null}))}), nowISO(), luotXem, body.luot_thich==null?null:Math.round(so(body.luot_thich)), chuoi(body.ngay_dang,30)||null, chuoi(body.link,300)||null, kenh).run();
      const soLieu={luot_xem:luotXem, kenh, ngay_dang:chuoi(body.ngay_dang,30)||null};
    ...
    if(path==='/hub/tap-mau' && method==='GET'){ const tn=chuoi(url.searchParams.get('tinh_nang'),40)||'chon_canh'; const goc=env.APP_BASE_URL||url.origin; const tuyet=u=>/^https?:/.test(u||'')?u:goc+u;
      ...
      return json({ tinh_nang:tn, mau: ds.map(r=>{ const dv=docJSON(r.dau_vao,{}); if(tn==='chon_doan'||tn==='ghep_canh') return { id:r.id, tap:r.tap, dau_vao:dv, nhan:docJSON(r.nhan,{}) }; return { id:r.id, tap:r.tap, luot_xem:dv.luot_xem??null, kenh:dv.kenh||null, hinh:dv.hinh, text:dv.text, ung_vien:(dv.ung_vien||[]).map(u=>({...u, media_url:tuyet(u.media_url)})), nhan:docJSON(r.nhan,{}) }; }) }); }
