  ...
  return {
    ...
    ai_nao: xem ? await (async()=>{ const th=thangHienTai(); return {
      ...
      do_chinh_xac_hinh: await (async()=>{ const ds=(await all(`SELECT dau_vao, dau_ra, nhan, dong, created_at FROM mau_hoc_ai WHERE tinh_nang='nhan_khung'`)).map(x=>({v:docJSON(x.dau_vao,{})||{}, c:docJSON(x.dau_ra,{})||{}, n:docJSON(x.nhan,{})||{}, dong:x.dong, luc:x.created_at}));
        ...
        return { so_nhan:ds.length, dung_nhom:ds.filter(dN).length, dung_chi_tiet:ds.filter(dC).length, ngau_nhien:dem(ds.filter(x=>x.v.ngau_nhien)), can_xac_nhan:dem(ds.filter(x=>x.v.can_xac_nhan_may)), theo_nguon:{ FOOTAGE:dem(ds.filter(x=>x.v.nguon!=='THANH_PHAM')), THANH_PHAM:dem(ds.filter(x=>x.v.nguon==='THANH_PHAM')) }, theo_nhom, nham, muc_tin }; })(),
...
async function handleApi(request, env){
  ...
  if(path==='/nhan-hinh/hang' && method==='GET'){ if(!isStaff(me)) return json({error:'Không có quyền'},403);
    ...
    const them=(nguon,row,dong,qt,bt,anhCua)=>{ const pt=docJSON(row.phan_tich,{})||{}; (Array.isArray(pt.timeline)?pt.timeline:[]).forEach((d,i)=>{ tong++; if(d.nguoi){ daGan++; return; } if(locDong&&dong!==locDong) return; const k=nguon+':'+row.id+':'+i; if(bo.has(k)) return;
      ds.push({ k, nguon, id:row.id, i, ten:row.ten, dong:dong||null, tu:d.tu, den:d.den, nhom:d.nhom, buoc:d.buoc||null, bai_test:d.bai_test||null, tham_my:d.tham_my, tu_tin:d.tu_tin, can_xac_nhan:!!d.can_xac_nhan, mo_ta:d.mo_ta||'', hanh_dong:d.hanh_dong||'', khung_url:d.khung_url||anhCua(d, pt)||null, media_url:nguon==='FOOTAGE'?row.media_url:null, link:row.link||null, quy_trinh:dsDongSP(qt), bai_test_ds:dsDongSP(bt) }); }); };
