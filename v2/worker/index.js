...
async function thayDocDoan(env, x){ const key=env.ANTHROPIC_API_KEY; if(!key) return {ok:false, loi:'chưa có ANTHROPIC_API_KEY'};
  ...
  const t0=Date.now(); let res, j; try{ res=await fetch('https://api.anthropic.com/v1/messages',{method:'POST', headers:{'content-type':'application/json','x-api-key':key,'anthropic-version':'2023-06-01'}, body:JSON.stringify({ model, max_tokens:300, messages:[{role:'user', content:[...anh, {type:'text', text:loi}]}] })}); j=await res.json().catch(()=>({})); }catch(e){ return {ok:false, loi:String(e.message||e).slice(0,120)}; }
  const vao=so(j.usage&&j.usage.input_tokens), ra=so(j.usage&&j.usage.output_tokens);
  await ghiAIUsage(env,{provider:'anthropic', model, tinh_nang:'hoc_nhan_khung', tokens_vao:vao, tokens_ra:ra, ok:!!res.ok, ms:Date.now()-t0, loi:res.ok?null:String((j.error&&j.error.message)||res.status).slice(0,200), muc:'API'});
...
async function handleApi(request, env){
  ...
  if(path.startsWith('/hub/')){
    ...
    if(path==='/hub/viec/doc_khung' && method==='GET'){ const mid=chuoi(url.searchParams.get('muc_id'),40), tid=chuoi(url.searchParams.get('tai_san_id'),40), lai=url.searchParams.get('lai')==='1'; const goc=env.APP_BASE_URL||url.origin;
      ...
      const viDu=(dong)=>[...suaNguoi.filter(x=>dong&&x.dong===dong), ...suaNguoi.filter(x=>!dong||x.dong!==dong)].slice(0,10).map(x=>'Cảnh "'+String(x.dv.mo_ta||'').slice(0,80)+'" là '+x.nh.nhom+(x.nh.buoc?(' / '+x.nh.buoc):'')+(x.nh.bai_test?(' / '+x.nh.bai_test):'')+((x.dv.cu&&(x.dv.cu.nhom!==x.nh.nhom||x.dv.cu.buoc!==x.nh.buoc))?(' (máy từng đọc nhầm là '+x.dv.cu.nhom+(x.dv.cu.buoc?(' / '+x.dv.cu.buoc):'')+')'):''));
    ...
    if(path==='/hub/thay-doc' && method==='POST'){ const cfg=(await docCauHinh(env)).ai||{}; if(cfg.thay_nhin===false) return json({ok:false, tat:true, loi:'Thầy (Claude) đang tắt ở Cấu hình'});
      ...
      const chay=async()=>{ while(k<ds.length){ const i=k++; kq[i]=await thayDocDoan(env,{ anh:ds[i].anh, quy_trinh:body.quy_trinh, bai_test:body.bai_test, san_pham:body.san_pham, ngu_canh:body.ngu_canh }).catch(e=>({ok:false, loi:String(e.message||e).slice(0,100)})); if(kq[i].vuot_ngan_sach){ for(let q=k;q<ds.length;q++) kq[q]={ok:false, loi:kq[i].loi, vuot_ngan_sach:true}; k=ds.length; } } };
