...
async function handleApi(request, env){
  ...
  if(path.startsWith('/hub/')){
    ...
    if(path==='/hub/thanh-pham' && method==='POST'){ const ten=chuoi(body.ten,200); const nid=chuoi(body.nguon_id,120)||ten; if(!ten) return json({error:'Thiếu tên'},400); const daCo=await env.DB.prepare(`SELECT id FROM kho_thanh_pham WHERE nguon_id=?`).bind(nid).first(); if(daCo&&!body.lam_lai) return json({ok:true, trung:true});
      ...
      { const dongTP=chuoi(body.dong,80)||null, linkTP=chuoi(body.link,300)||null; await MAU().upsertHinh(env,{ nguon:'THANH_PHAM', doi_tuong_id:id, dong:dongTP, ten, link:linkTP, timeline:tlSach(body.timeline), khungCua:(d)=>{ const g=(d.tu+d.den)/2; const co=shots.filter(x=>x.khung_url); const sh=co.find(x=>g>=x.t0&&g<x.t1)||co.sort((a,b)=>Math.abs((a.t0+a.t1)/2-g)-Math.abs((b.t0+b.t1)/2-g))[0]; return sh?sh.khung_url:null; } }); const tlTP=tlSach(body.timeline); await MAU().upsertLoi(env,{ doi_tuong_id:id, dong:dongTP, ten, link:linkTP, shots:shots.map(s=>{ if(s.nhom) return s; const g=(s.t0+s.t1)/2; const d=tlTP.find(x=>g>=x.tu&&g<x.den); return d?{...s, hinh:d.thay||d.mo||{nhom:d.nhom, buoc:d.buoc, bai_test:d.bai_test}}:s; }) }); }   // ADR-018: đoạn hình + câu thoại vào kho mẫu
