...
// một video TikTok một link (26/09: Trạm/Kalodata gửi 30 link cho 10 video — mỗi lần lặp máy học tốn ~6 phút đọc hình, 0 mẫu mới); giữ link đầu
const motVideoMotLink = (ds) => { const da=new Set(); return ds.filter(v=>{ const k=(String(v.link||'').match(/\/video\/(\d+)/)||[])[1]||String(v.link||''); if(da.has(k)) return false; da.add(k); return true; }); };
...
const CONFIG_MAC_DINH = {
  ...
  huan_luyen:{ anh_xa_dong:[{chua:'FINEX',dong:'Finex F300'},{chua:'TERRAZY',dong:'Terrazy'},{chua:'KEO CHIT MACH',dong:'Keo chít mạch'},{chua:'KEO CHÍT MẠCH',dong:'Keo chít mạch'}], tran_phut_ngay:20, k1_gom_chung:300, k1_gom_dong:100, k1_vang_chung:200, k1_vang_dong:60, k2_gom:100, k2_vang:50, k4_gom:150, k4_vang:100, thay_loi:true, thay_loi_model:'claude-haiku-4-5-20251001',
...
// ============================================================
// Schema + dữ liệu mẫu chạy MỘT LẦN cho mỗi phiên bản mã (26/09: chạy lại ~105 câu tạo bảng / thêm cột + seed ở MỌI request,
// mỗi câu một lượt tới D1 → API prod mất ~22 s mỗi lần gọi). Mã băm = nội dung hàm schema + seed + hằng seed; ghi ở module_config.schema_ban;
// mỗi isolate nhớ theo env.DB (không tốn câu nào), isolate mới kiểm 1 câu. Sửa hàm schema / seed → mã băm đổi → tự chạy lại một lần.
const DA_SCHEMA=new WeakMap(); let SCHEMA_BAN=null;
function banSchema(){ if(SCHEMA_BAN) return SCHEMA_BAN; const t=ensureSchemaDayDu.toString()+seedNeuTrong.toString()+seedBoNaoAI.toString()+JSON.stringify([MO_HINH_SEED, DINH_TUYEN_SEED, BUOC]);
  let h=0; for(let i=0;i<t.length;i++) h=(Math.imul(h,31)+t.charCodeAt(i))|0; SCHEMA_BAN=(h>>>0).toString(36)+'.'+t.length; return SCHEMA_BAN; }
async function ensureSchema(env){ const ban=banSchema(); if(DA_SCHEMA.get(env.DB)===ban) return;
  try{ const r=await env.DB.prepare(`SELECT cau_hinh FROM module_config WHERE id='schema_ban'`).first(); if(r&&(docJSON(r.cau_hinh,{})||{}).ban===ban){ DA_SCHEMA.set(env.DB, ban); return; } }catch(e){}
  await ensureSchemaDayDu(env);
  await env.DB.prepare(`INSERT INTO module_config (id,cau_hinh,updated_at,updated_by_name) VALUES ('schema_ban',?,?,'Máy') ON CONFLICT(id) DO UPDATE SET cau_hinh=excluded.cau_hinh, updated_at=excluded.updated_at`).bind(JSON.stringify({ban}), nowISO()).run();
  DA_SCHEMA.set(env.DB, ban); }
async function ensureSchemaDayDu(env){
  const q=[
  ...
  // ADR-006 (chạy SAU khi tạo bảng — trước đây nằm trước, chỉ thành công nhờ request sau chạy lại): điểm N ngày gần nhất + đề nghị của máy (LEN | XUONG | null) cho từng bước
...
// G1 (ADR-021b, chủ chọn 27/09 "Chốt từ Hồ sơ"): bản chốt = nguyên hồ sơ (lõi + ý đồ) + tỷ trọng pillar + định hướng giai đoạn.
// Mã băm cho biết hồ sơ đã đổi sau lần chốt gần nhất; so sánh từng phần để người chốt thấy mình sắp chốt cái gì.
const boNoi=hs=>{ const {_luu,_cap_nhat,_boi,phien,...o}=hs||{}; return o; };
function maChuoi(t){ let h=0; const x=String(t); for(let i=0;i<x.length;i++) h=(Math.imul(h,31)+x.charCodeAt(i))|0; return (h>>>0).toString(36)+'.'+x.length; }
const maBanChot=(hs, pillars, dh)=>maChuoi(JSON.stringify([boNoi(hs), (pillars||[]).map(p=>[p.ten, so(p.ty_trong)]).sort((a,b)=>String(a[0]).localeCompare(String(b[0]))), chuoi(dh,600)]));
async function banChot(env){ const hs=boNoi(await docHoSo(env)); const pillars=(await env.DB.prepare(`SELECT id,ten,ty_trong,muc_tieu FROM pillars WHERE active=1 ORDER BY thu_tu`).all()).results; const cl=await env.DB.prepare(`SELECT dinh_huong FROM chien_luoc WHERE id=1`).first()||{};
  const dh=chuoi(cl.dinh_huong,600); return { ho_so:hs, pillars, dinh_huong:dh, ma:maBanChot(hs, pillars, dh) }; }
function tomTatChot(hs, pillars, dh){ const yd=(hs.y_do||[]).filter(y=>y.active!==false); const theoDong={}; yd.forEach(y=>{ const k=y.dong||'Mọi dòng'; theoDong[k]=(theoDong[k]||0)+1; });
  return { tagline:(hs.thuong_hieu||{}).tagline||'', dong:(hs.dong||[]).map(d=>({ dong:d.dong, cau_dinh_vi:d.cau_dinh_vi||'', trang_thai:d.trang_thai||'' })), y_do_bat:yd.length, y_do_theo_dong:theoDong,
    pillars:(pillars||[]).map(p=>({ ten:p.ten, ty_trong:so(p.ty_trong), so_y_do:yd.filter(y=>chuanHoaTen(y.pillar)===chuanHoaTen(p.ten)).length })), dinh_huong:dh||'' }; }
const NHAN_PHAN={ dinh_vi_chinh:'định vị chính', su_that:'sự thật', dong_con:'dòng con', doi_tuong:'khách hàng', chon_sku:'chọn sản phẩm', rao_can:'rào cản', chuan_thi_cong:'chuẩn thi công', bao_hanh:'bảo hành', lo_trinh:'lộ trình', quy_tac_claim:'quy tắc claim', luu_y_phap_che:'pháp chế', kenh:'kênh', noi_dung_kenh:'nội dung kênh', can_bo_sung:'cần bổ sung', thong_so_chung:'thông số chung', dan_dat:'cách dẫn dắt', loi_the:'lợi thế', doi_thu:'đối thủ', ghi_chu_doi_tuong:'ghi chú khách hàng', trang_thai:'trạng thái' };
function soSanhHoSo(cu, moi){ const ra=[]; const them=(loai, noi_dung)=>ra.push({loai, noi_dung}); const J=x=>JSON.stringify(x==null?null:x);
  const thC=cu.ho_so.thuong_hieu||{}, thM=moi.ho_so.thuong_hieu||{}; for(const [k,ten] of [['tagline','tagline'],['la_ai','Kingsmen là ai'],['cuoc_choi','cuộc chơi'],['tru_cuoc_choi','ba trụ cuộc chơi'],['tam_nhin','tầm nhìn'],['su_menh','sứ mệnh'],['niem_tin','niềm tin'],['tinh_than','tinh thần'],['tong_giong','tông giọng'],['la','Kingsmen là'],['khong_la','Kingsmen không phải']]) if(J(thC[k])!==J(thM[k])) them('~','Thương hiệu mẹ: '+ten);
  const dC=new Map((cu.ho_so.dong||[]).map(d=>[d.dong,d])), dM=new Map((moi.ho_so.dong||[]).map(d=>[d.dong,d]));
  for(const [ten,d] of dM){ const c=dC.get(ten); if(!c){ them('+','Dòng '+ten); continue; }
    if((c.cau_dinh_vi||'')!==(d.cau_dinh_vi||'')) them('~',ten+': câu định vị'); if((c.loi_hua||'')!==(d.loi_hua||'')) them('~',ten+': lời hứa');
    const tC=new Map((c.tru_cot||[]).map(t=>[t.ten,t])), tM=new Map((d.tru_cot||[]).map(t=>[t.ten,t]));
    for(const [tt,t] of tM){ const x=tC.get(tt); if(!x) them('+',ten+': trụ "'+tt+'"'); else if(J(x)!==J(t)) them('~',ten+': trụ "'+tt+'"'); } for(const tt of tC.keys()) if(!tM.has(tt)) them('-',ten+': trụ "'+tt+'"');
    const khac=Object.keys(NHAN_PHAN).filter(k=>J(c[k])!==J(d[k])); if(khac.length) them('~',ten+': '+khac.map(k=>NHAN_PHAN[k]).join(', ')); }
  for(const ten of dC.keys()) if(!dM.has(ten)) them('-','Dòng '+ten);
  const yC=new Map((cu.ho_so.y_do||[]).map(y=>[y.id,y])), yM=new Map((moi.ho_so.y_do||[]).map(y=>[y.id,y]));
  for(const [id,y] of yM){ const c=yC.get(id); if(!c){ them('+','Ý đồ "'+y.ten+'"'+(y.active===false?' (đang tắt)':'')); continue; } const bC=c.active!==false, bM=y.active!==false;
    if(bC&&!bM) them('-','Tắt ý đồ "'+y.ten+'"'); else if(!bC&&bM) them('+','Bật lại ý đồ "'+y.ten+'"'); else if(J(c)!==J(y)) them('~','Ý đồ "'+y.ten+'"'); }
  for(const [id,c] of yC) if(!yM.has(id)) them('-','Bỏ ý đồ "'+c.ten+'"');
  const pC=new Map((cu.pillars||[]).map(p=>[p.ten,so(p.ty_trong)])), pM=new Map((moi.pillars||[]).map(p=>[p.ten,so(p.ty_trong)]));
  for(const [t,v] of pM){ if(!pC.has(t)) them('+','Pillar "'+t+'" '+v+'%'); else if(pC.get(t)!==v) them('~','Pillar "'+t+'" '+pC.get(t)+'% → '+v+'%'); } for(const t of pC.keys()) if(!pM.has(t)) them('-','Pillar "'+t+'"');
  if((cu.dinh_huong||'')!==(moi.dinh_huong||'')) them('~','Định hướng giai đoạn');
  return ra; }
// đối tượng chung (bài chưa rõ dòng): từng dòng → các nhóm khách trong hồ sơ
const doiTuongChung=hs=>(hs.dong||[]).filter(d=>(d.doi_tuong||[]).length).map(d=>d.dong+': '+d.doi_tuong.map(x=>x.ten).join(', ')).join(' | ');
async function dinhHuongBai(env, thang){ const cl=await env.DB.prepare(`SELECT dinh_huong FROM chien_luoc WHERE id=1`).first()||{}; const kh=await env.DB.prepare(`SELECT dinh_huong FROM ke_hoach_thang WHERE thang=?`).bind(thang||thangHienTai()).first()||{};
  return (cl.dinh_huong?('\nĐỊNH HƯỚNG GIAI ĐOẠN (ưu tiên khi chọn góc): '+chuoi(cl.dinh_huong,600)):'')+(kh.dinh_huong?('\nĐỊNH HƯỚNG THÁNG: '+chuoi(kh.dinh_huong,600)):''); }
...
async function tomTatDinhVi(env, cl){ const hs=await docHoSo(env); const th=hs.thuong_hieu||{}; const L=[(th.tagline||'Kingsmen')+(th.la_ai?(' — '+th.la_ai):'')+(th.cuoc_choi?(' Cuộc chơi: '+th.cuoc_choi+'.'):'')];
  ...
  if(cl&&cl.dinh_huong) L.push('Định hướng giai đoạn: '+chuoi(cl.dinh_huong,600)); return L.join('\n'); }
...
async function promptNoiDung(env, b){
  ...
  const muc=b.muc_id?await env.DB.prepare(`SELECT dong, y_do, thang FROM muc_noi_dung WHERE id=?`).bind(b.muc_id).first():null;
  ...
  const usr='MỤC TIÊU BÀI: '+(b.muc_tieu==='BAN_HANG'?'BÁN HÀNG (dẫn tới hỏi mua/inbox, nói rõ sản phẩm & lợi ích thật)':'XÂY DỰNG BRAND (để được xem, chia sẻ, nhớ tên; không ép mua)')+'\nPILLAR: '+(pl?(pl.ten+(pl.mo_ta?(' — '+pl.mo_ta):'')):'(chưa)')+'\nFRAMEWORK: '+(fw?(fw.ten+(fw.mo_ta?(' — '+fw.mo_ta):'')):'(tự chọn cấu trúc)')+khoiYDo
    ...
    +dinhVi+(await dinhHuongBai(env, muc&&muc.thang))+'\nTÔNG GIỌNG: '+(((hs.thuong_hieu||{}).tong_giong)||cl.tong_giong||'(chưa đặt)')+'\nĐỐI TƯỢNG: '+(hd&&(hd.doi_tuong||[]).length?hd.doi_tuong.map(d=>d.ten+(d.can?(' — '+d.can):'')).join('; '):(doiTuongChung(hs)||cl.doi_tuong||'(chưa đặt)'))+'\nCỤM TỪ CẤM: '+JSON.stringify(claims.map(c=>c.cum_tu))
...
// KHO FOOTAGE THEO DÒNG (26/09, chủ gửi thư mục Drive footage gốc "1. KEO CHÍT MẠCH CHUYÊN DỤNG", "TERRAZY x ĐẠI LÝ 2026" chia theo cảnh / góc quay):
// mỗi dòng một mục đặc biệt (giai_doan 'KHO', không lên kanban) giữ footage gốc; viết kịch bản và dựng video lấy thêm clip từ kho cùng dòng.
const khoFootageId=(dong)=>'kho_ft_'+String(dong||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/đ/g,'d').replace(/[^a-z0-9]+/g,'_').replace(/^_|_$/g,'');
async function damKhoFootage(env, dong, me){ const id=khoFootageId(dong); if(!(await env.DB.prepare(`SELECT id FROM muc_noi_dung WHERE id=?`).bind(id).first())) await env.DB.prepare(`INSERT INTO muc_noi_dung (id,thang,tieu_de,muc_tieu,dinh_dang,giai_doan,tao_boi,ghi_chu,created_at,created_by_name,updated_at,dong) VALUES (?,?,?,'BRAND',NULL,'KHO','AGENT',?,?,?,?,?)`).bind(id, thangHienTai(), '📦 Kho footage — '+dong, 'Footage gốc của dòng '+dong+' (quay thô, chia theo cảnh / góc quay). Viết kịch bản và dựng video của dòng này lấy clip từ đây.', nowISO(), (me&&me.ho_ten)||'Máy', nowISO(), dong).run(); return id; }
// ADR-020 đợt 3: cách viết theo từng giá trị trục — trợ lý đọc khi mục kế hoạch đã chọn tổ hợp
const VIET_THEO_TRUC={ cau_truc:{ VAN_DE_GIAI_PHAP:'nêu vấn đề khách gặp → giải pháp là sản phẩm → bằng chứng (thi công / kết quả) → chốt', TRUOC_SAU:'cảnh đầu là hiện trạng xấu, giữa là thi công, cuối là kết quả đặt cạnh cảnh đầu để so', TUNG_BUOC:'đánh số từng bước thi công theo quy trình, mỗi cảnh một bước', THU_NGHIEM:'dựng một phép thử (nước, cào, va đập, lau bẩn…) và cho thấy kết quả', KE_CHUYEN:'kể một tình huống có nhân vật (chủ nhà / thợ / đại lý) từ khó khăn tới kết quả', DANH_SACH:'dạng "3 lý do / 5 lỗi …", mỗi cảnh một ý' },
  mo_dau:{ CAU_HOI:'mở bằng một câu hỏi đánh vào nỗi đau khách', KET_QUA_TRUOC:'mở bằng cảnh kết quả đẹp nhất, sau đó mới quay lại cách làm', LOI_HAY_GAP:'mở bằng lỗi hay gặp khiến khách giật mình', CON_SO:'mở bằng một con số cụ thể có thật trong tài liệu (không bịa)', HANH_DONG_MANH:'mở bằng một hành động mạnh trên hình (đổ, đập, xối nước…)', CAM_XUC:'mở bằng cảm xúc của chủ nhà / thợ' },
  phong_cach:{ NGUOI_NOI:'lời như một người nói thẳng vào camera, ngôi thứ nhất', LONG_TIENG:'lời bình đọc đè lên hình, ngắn gọn', CHU_VA_NHAC:'rất ít lời, chủ yếu chữ trên hình + nhạc (text ngắn)', AM_THANH_THAT:'ít lời, để tiếng thi công thật, câu ngắn', TREND:'bắt nhịp trend, câu ngắn, nhịp nhanh' },
  muc_dich:{ QUANG_CAO:'chốt bằng lời kêu gọi mua / nhắn tin rõ', THUONG_HIEU:'nhấn uy tín, chất lượng thương hiệu, chốt mềm', BAN_HANG_HANG_NGAY:'nhắc sản phẩm cụ thể, chốt đơn nhanh', HUONG_DAN:'rõ ràng, dạy được, chốt bằng lời mời tư vấn', CHUNG_MINH:'mọi câu phải có hình chứng minh', PHAN_HOI:'dựa trên công trình / khách thật' } };
async function angleFootage(env, mucId, dinhDang, goc){ if(!mucId||dinhDang!=='VIDEO') return goc||''; const mm=await env.DB.prepare(`SELECT dong, truc FROM muc_noi_dung WHERE id=?`).bind(mucId).first(); const kho=mm&&mm.dong?khoFootageId(mm.dong):'-';
  const tr=docJSON(mm&&mm.truc,null); let huongTruc=''; if(tr){ const T=MAU().TRUC; const dong=Object.keys(T).filter(k=>tr[k]&&VIET_THEO_TRUC[k]&&VIET_THEO_TRUC[k][tr[k]]).map(k=>'- '+T[k].ten+': '+T[k].gt[tr[k]]+' — '+VIET_THEO_TRUC[k][tr[k]]);
    const vd=tr.cau_truc?(await env.DB.prepare(`SELECT kich_ban, luot_xem FROM kho_thanh_pham WHERE kich_ban IS NOT NULL AND length(kich_ban)>80 AND json_extract(truc,'$.cau_truc')=? AND (?='' OR dong=?) ORDER BY COALESCE(doanh_thu,0) DESC, COALESCE(luot_xem,0) DESC LIMIT 2`).bind(tr.cau_truc, (mm&&mm.dong)||'', (mm&&mm.dong)||'').all()).results:[];
    if(dong.length) huongTruc='\n\nTỔ HỢP BẮT BUỘC (ADR-020 — mục kế hoạch đã chọn):\n'+dong.join('\n')+(vd.length?'\nVÍ DỤ CÙNG CẤU TRÚC ĐÃ CHẠY TỐT (chỉ học cách dựng ý, không chép câu):\n'+vd.map((x,i)=>'Ví dụ '+(i+1)+(x.luot_xem?' ('+x.luot_xem+' xem)':'')+': '+String(x.kich_ban).slice(0,500)).join('\n'):''); }
  goc=(goc||'')+huongTruc;
  const ft=(await env.DB.prepare(`SELECT ten,mo_ta FROM tai_san WHERE (muc_id=? OR muc_id=?) AND loai IN ('FOOTAGE','ANH') AND COALESCE(mo_ta,'')<>'' AND COALESCE(nguon,'')<>'THU_NGHIEM' ORDER BY (muc_id=?) DESC, created_at LIMIT 40`).bind(mucId, kho, mucId).all()).results;
...
const LOAI_LENH_THAY_CHU=['chay_agent','dung','gui_duyet','tra_lai','nap_footage','hoc_tiktok','hoc_reels'];
async function chayThayChu(env){ const c=await cfgThayChu(env); if(!c.bat) return {bo_qua:'chế độ AI thay chủ đang tắt'}; const kq=[];
  ...
  for(const v of ds.slice(0,20)){ try{
    ...
    else if(v.loai==='nap_footage'){ const fm=String(v.link||'').match(/folders\/([A-Za-z0-9_-]{10,})/); const dong=chuoi(v.dong,80); if(!fm||!dong||!(await MAU().dsDongChuan(env)).some(x=>x.dong===dong)){ kq.push([v.link,'bỏ: cần link thư mục Drive + dòng có trong Bộ nhãn']); continue; } const kho=await damKhoFootage(env, dong, AI_THAY_CHU); const c=await chonMayDung(env,{}); if(!c){ kq.push([fm[1],'bỏ: không có máy dựng đang bật']); continue; } const r=await taoLenhTram(env,'nap_drive',{muc_id:kho, folder_id:fm[1], toi_da:Math.max(1,Math.min(300,so(v.toi_da,200))), kho_dong:dong}, AI_THAY_CHU, c.may.id); await logAudit(env,AI_THAY_CHU,'nạp footage gốc vào kho','muc_noi_dung',kho,fm[1]+' · '+dong); kq.push([fm[1],'nạp footage · '+(r.trung?'đã có lệnh ':'')+r.id+' · '+c.may.ten]); }
    else if(v.loai==='hoc_reels'){ const dong=chuoi(v.dong,80); if(dong&&!(await MAU().dsDongChuan(env)).some(x=>x.dong===dong)){ kq.push([v.link,'bỏ: dòng không có trong Bộ nhãn']); continue; } const x=await hangTaiReels(env, {link:v.link, toi_da:v.toi_da, dong, muc_dich:v.muc_dich}, AI_THAY_CHU); kq.push([v.link, x.ok?('học Reels · '+x.trang+' · '+(x.trung?'đã có lệnh ':'')+x.lenh_id+' · Trạm đọc'):('bỏ: '+x.loi)]); }
    else if(v.loai==='hoc_tiktok'){ const dong=chuoi(v.dong,80); if(dong&&!(await MAU().dsDongChuan(env)).some(x=>x.dong===dong)){ kq.push([v.kenh,'bỏ: dòng không có trong Bộ nhãn']); continue; } const x=await hangTaiTikTok(env, {link:v.kenh, toi_da:v.toi_da, dong, muc_dich:v.muc_dich}, AI_THAY_CHU); kq.push([v.kenh, x.ok?('học kênh · '+(x.trung?'đã có lệnh ':'')+x.lenh_id+' · máy học tự liệt kê + tải'):('bỏ: '+x.loi)]); }
...
// Xếp một kênh TikTok cho Trạm tải (phiên đăng nhập trên Trạm) rồi máy học — một hàm cho nút của người và lệnh AI thay chủ 'hoc_tiktok'
async function hangTaiTikTok(env, {link, toi_da, dong, muc_dich}, me){
  // 26/09 (chủ: "không cần đăng nhập tiktok… nhiều ứng dụng tải bình thường"): máy học tự liệt kê kênh bằng yt-dlp giả trình duyệt rồi tải — không qua Trạm,
  // không cần tài khoản chủ kênh. Trạm (TikTok Studio) chỉ còn là đường dự phòng cho kênh riêng tư.
  const kenh=String(link||'').trim().replace(/^https?:\/\/(www\.)?tiktok\.com\//,'').replace(/[?#].*$/,'').replace(/\/.*$/,'').replace(/^@?/,'@'); if(!/^@[A-Za-z0-9_.]{2,40}$/.test(kenh)) return {ok:false, loi:'Nhập kênh TikTok dạng @tenkenh hoặc link tiktok.com/@tenkenh'};
  const mayId=await mayManhNhat(env,'dung_video'); if(!mayId) return {ok:false, ma:409, loi:'Không có máy học nào đang bật'};
...
async function hangTaiReels(env, {link, toi_da, dong, muc_dich}, me){ let u=String(link||'').trim();
  if(/facebook\.com\/share\//.test(u)){ try{ const r=await fetch(u,{redirect:'manual', headers:{'user-agent':'Mozilla/5.0'}}); const loc=r.headers.get('location'); if(loc) u=loc; }catch(e){} }
  const m=u.match(/facebook\.com\/(?:profile\.php\?id=(\d+)|([A-Za-z0-9.\-]{3,80}))/); const trang=m?(m[1]?('profile.php?id='+m[1]):m[2]):null; if(!trang||/^(share|reel|watch|groups|pages)$/i.test(trang)) return {ok:false, loi:'Dán link fanpage Facebook (vd facebook.com/KeoKingsmen hoặc link chia sẻ trang)'};
  const tt=await docTramTrangThai(env); if(!tt||!tt.song) return {ok:false, ma:409, loi:'Trạm văn phòng đang im — Reels đọc qua phiên Facebook trên Trạm'};
...
async function napLoTram(env, b){
  ...
  if(bang==='doi_thu_tin'){ const kq=await gomYTuong(env, dong.map(d=>({ ten:chuoi(d.tieu_de||d.ten||d.title,200), mo_ta:chuoi(d.noi_dung||d.mo_ta||d.tom_tat,500), link:chuoi(d.link||d.url,500), nguon:'DOI_THU' })).filter(x=>x.ten), 'Trạm'); moi=kq.them; }
      ...
      const tsId=uid('ts'); await env.DB.prepare(`INSERT INTO tai_san (id,loai,ten,mo_ta,media_url,media_type,muc_id,noi_dung_id,nguon,created_at,created_by_name,ma_video) VALUES (?,'VIDEO_XUAT',?,?,?,'VIDEO',?,?,'TRAM',?,?,?)`).bind(tsId,'Video nháp máy dựng: '+ten, chuoi(d.mo_ta,400)+(thieu.length?(' · thiếu hình: '+thieu.join('; ')):''), chuoi(d.media_url,500), nd.muc_id, nd.id, nowISO(), nguon, maVideo(tsId, nowISO())).run();
...
// KHO VIDEO — thông tin theo dõi (26/09, chủ: "kho video app sx cần gắn các thông tin tracking theo dòng nội dung để quản lý sử dụng")
// ma_video: mã cố định mỗi bản nháp (V + ngày VN yymmdd + 4 ký tự cuối id) — dán vào caption / mã theo dõi kết quả để đo đúng video.
// muc_noi_dung.dong: dòng sản phẩm của mục (một nguồn cho mọi video của mục); tai_san.su_dung: CHON (chọn dùng) | BO (loại) | null (chưa quyết).
const maVideo=(id, luc)=>'V'+new Date(Date.parse(luc||nowISO())+7*3600e3).toISOString().slice(2,10).replace(/-/g,'')+'-'+String(id).replace(/[^a-z0-9]/gi,'').slice(-4).toUpperCase();
const doanDong=(t)=>{ t=String(t||''); if(/terrazy/i.test(t)) return 'Terrazy'; if(/finex|\bf ?300\b/i.test(t)) return 'Finex F300'; if(/keo chít mạch|chít mạch|keo ron|chà ron|ron gạch|mạch gạch/i.test(t)) return 'Keo chít mạch'; return null; };
// ADR-020: footage gốc — góc quay + loại cảnh lấy từ tên thư mục con (người quay đã chia sẵn), không tốn thầy
const trucThuMuc=(tm)=>{ const t=String(tm||'').toUpperCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/Đ/g,'D'); return { goc_quay:/POV/.test(t)?'POV':/CAN\b.*TRUNG|CAN ?& ?TRUNG/.test(t)?'CAN_CHI_TIET':/\bCAN\b|CAN CANH|CLOSE/.test(t)?'CAN_CHI_TIET':/TRUNG/.test(t)?'TRUNG':/TOAN/.test(t)?'TOAN_CANH':/TREN XUONG|TOP ?DOWN/.test(t)?'TREN_XUONG':null,
  nhom_canh:/VAN DE/.test(t)?'VAN_DE':/THI CONG/.test(t)?'THI_CONG':/HOAN THIEN/.test(t)?'HOAN_THIEN':/TEST|THU NGHIEM/.test(t)?'THU_NGHIEM':/QUAY SAN PHAM|SAN PHAM/.test(t)?'SAN_PHAM':/CTA|OUTRO/.test(t)?'CHOT':null }; };
async function dienVideo(env){ try{
  for(const r of (await env.DB.prepare(`SELECT id, mo_ta FROM tai_san WHERE loai='FOOTAGE' AND goc_quay IS NULL AND nhom_canh IS NULL AND mo_ta LIKE '%thư mục%' LIMIT 300`).all()).results){ const x=trucThuMuc((String(r.mo_ta).match(/thư mục ([^·]+)/)||[])[1]); if(x.goc_quay||x.nhom_canh) await env.DB.prepare(`UPDATE tai_san SET goc_quay=?, nhom_canh=? WHERE id=?`).bind(x.goc_quay, x.nhom_canh, r.id).run(); else await env.DB.prepare(`UPDATE tai_san SET nhom_canh='' WHERE id=?`).bind(r.id).run(); }   /* mỗi lần bootstrap: 2 truy vấn nhỏ, chỉ đụng dòng còn thiếu */
  for(const r of (await env.DB.prepare(`SELECT id, created_at FROM tai_san WHERE loai='VIDEO_XUAT' AND ma_video IS NULL LIMIT 500`).all()).results) await env.DB.prepare(`UPDATE tai_san SET ma_video=? WHERE id=?`).bind(maVideo(r.id, r.created_at), r.id).run();
  for(const m of (await env.DB.prepare(`SELECT id, tieu_de, ghi_chu FROM muc_noi_dung WHERE dong IS NULL LIMIT 500`).all()).results){ const d=doanDong(m.tieu_de+' '+m.ghi_chu); if(d) await env.DB.prepare(`UPDATE muc_noi_dung SET dong=? WHERE id=?`).bind(d, m.id).run(); }
}catch(e){} }
async function bootstrap(env, u){ await dienVideo(env);
  const all=async(s,...a)=>(await env.DB.prepare(s).bind(...a).all()).results;
  ...
  const [pb, khR, mucR, ytR] = xem ? await Promise.all([
    all(`SELECT id,phien_ban,pillars,ghi_chu,chot_boi,chot_at,dinh_huong,ma FROM chien_luoc_phien_ban ORDER BY phien_ban DESC LIMIT 10`),
  ...
  return {
    ...
    y_do: await docHoSo(env).then(hs=>{ hsBoot=hs; return hs; }).then(hs=>(hs.y_do||[]).filter(y=>y.active!==false).map(y=>({ id:y.id, ten:y.ten, dong:y.dong||'', pillar:y.pillar||'', tru_cot:y.tru_cot||'', thong_diep:y.thong_diep||'', uu_tien:y.uu_tien||2, muc_tieu:y.muc_tieu||'BRAND', dinh_dang:y.dinh_dang||[], truc:y.truc||{}, goi_y:y.goi_y||[] }))).catch(()=>[]),
    ...
    chien_luoc: { ...(cl||{}), da_doi: !!(cl&&so(cl.phien_ban)>0&&hsBoot&&xem&&cl.chot_ma!==maBanChot(hsBoot, pillars.filter(p=>p.active===1||p.active===true), cl.dinh_huong)) },
...
async function handleApi(request, env){
  ...
  if(path.startsWith('/hub/')){
    ...
    if(path==='/hub/viec/tai_reels' && method==='GET'){ const cfg=await docCauHinh(env); return json({ viec:((cfg.tai_reels||{}).hang)||[], da_co:(await env.DB.prepare(`SELECT nguon_id FROM kho_thanh_pham WHERE nguon='REELS'`).all()).results.map(x=>x.nguon_id) }); }
    if(path==='/hub/reels-da-tai' && method==='POST'){ const trang=chuoi(body.trang,120); if(!trang) return json({error:'Thiếu trang'},400);
      const cfg=await docCauHinh(env); const cu=(((cfg.tai_reels||{}).hang)||[]).find(x=>x.trang===trang)||{}; await env.DB.prepare(`UPDATE module_config SET cau_hinh=? WHERE id='tai_reels'`).bind(JSON.stringify({hang:(((cfg.tai_reels||{}).hang)||[]).filter(x=>x.trang!==trang)})).run();
      const thay=new Set(); const ds=(Array.isArray(body.links)?body.links:[]).slice(0,60).map(v=>({link:chuoi(v.link,300), meta:(v.meta&&typeof v.meta==='object')?{...v.meta, kenh:'fb/'+trang}:{kenh:'fb/'+trang}})).filter(v=>{ const id=(v.link.match(/facebook\.com\/(?:reel|reels)\/(\d+)/)||[])[1]; if(!id||thay.has(id)) return false; thay.add(id); return true; });
      if(!ds.length) return json({ok:true, giao:false, loi:'Trạm gửi danh sách Reels rỗng cho '+trang+(body.loi?(' — '+chuoi(body.loi,200)):'')});
      const mayId=await mayManhNhat(env,'dung_video'); if(!mayId) return json({ok:true, giao:false, loi:'chưa ghép máy học nào'});
      const r=await taoLenhTram(env,'hoc_thanh_pham',{nguon:'REELS', kenh:'fb/'+trang, toi_da:ds.length, dong:cu.dong||null, muc_dich:cu.muc_dich||null, links:ds}, MAY('Trạm'), mayId); return json({ ok:true, giao:true, lenh_id:r.id, so:ds.length }); }
    if(path==='/hub/tiktok-da-tai' && method==='POST'){ const kenh=chuoi(body.kenh,60), thu=chuoi(body.thu_muc,300); if(!kenh||(!thu&&!Array.isArray(body.links))) return json({error:'Thiếu kênh/danh sách'},400);
      ...
      const dsLink=motVideoMotLink((Array.isArray(body.links)?body.links:[]).slice(0,60).map(v=>({link:chuoi(v.link,300), meta:(v.meta&&typeof v.meta==='object')?v.meta:{}})).filter(v=>/tiktok\.com\/.+\/video\/\d+/.test(v.link)));
    ...
    if(path==='/hub/kalodata' && method==='POST'){ const nganh=chuoi(body.nganh,80); const ds=motVideoMotLink((Array.isArray(body.video)?body.video:[]).slice(0,60).map(v=>({ link:chuoi(v.link,300), tieu_de:chuoi(v.tieu_de,300), kenh:chuoi(v.kenh,80), doanh_thu:v.doanh_thu==null?null:+so(v.doanh_thu).toFixed(2), luot_ban:v.luot_ban==null?null:Math.round(so(v.luot_ban)), luot_xem:v.luot_xem==null?null:Math.round(so(v.luot_xem)), san_pham:chuoi(v.san_pham,200), ngay_dang:chuoi(v.ngay_dang,30) })).filter(v=>/tiktok\.com\/.+\/video\/\d+/.test(v.link)));
    ...
    if(path==='/hub/viec/nap_drive' && method==='GET'){ const mid=chuoi(url.searchParams.get('muc_id'),40); const ds=(await env.DB.prepare(`SELECT ten, drive_id FROM tai_san WHERE muc_id=? AND nguon='DRIVE'`).bind(mid).all()).results; return json({ da_co: ds.filter(x=>!x.drive_id).map(x=>x.ten), da_co_id: ds.filter(x=>x.drive_id).map(x=>x.drive_id) }); }   /* 27/09: chống trùng theo mã file Drive; bản ghi cũ theo tên */
    if(path==='/hub/tai-san' && method==='POST'){ const mucId=chuoi(body.muc_id,40); const muc=mucId?await env.DB.prepare(`SELECT id,tieu_de FROM muc_noi_dung WHERE id=?`).bind(mucId).first():null; if(!muc) return json({error:'Không có mục'},404);
      ...
      const driveId=chuoi(body.drive_id,120)||null; if(await (driveId?env.DB.prepare(`SELECT id FROM tai_san WHERE muc_id=? AND nguon='DRIVE' AND (drive_id=? OR (drive_id IS NULL AND ten=?))`).bind(mucId, driveId, ten):env.DB.prepare(`SELECT id FROM tai_san WHERE muc_id=? AND ten=? AND nguon='DRIVE'`).bind(mucId, ten)).first()) return json({ ok:true, trung:true });
      ...
      const pt=phanTichSach(body.phan_tich, khungDs, mt2.co_canh); const id=uid('ts'); await env.DB.prepare(`INSERT INTO tai_san (id,loai,ten,mo_ta,media_url,media_type,muc_id,noi_dung_id,nguon,created_at,created_by_name,phan_tich,drive_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(id, mt==='IMAGE'?'ANH':'FOOTAGE', ten, moTa.slice(0,500), media_url, mt, mucId, nd?nd.id:null, chuoi(body.nguon,20)==='LOCAL'?'LOCAL':'DRIVE', nowISO(), MAY('Máy (nạp Drive)').ho_ten, pt?JSON.stringify(pt):null, driveId).run();
    ...
    if(path==='/hub/viec/dung_video' && method==='GET'){ const chi=chuoi(url.searchParams.get('noi_dung_id'),40); const ds=(await env.DB.prepare(`SELECT n.id, n.tieu_de, n.hook, n.sections, n.cta, n.chi_tiet, n.muc_id, m.tieu_de muc_tieu_de, m.dong muc_dong FROM noi_dung n LEFT JOIN muc_noi_dung m ON m.id=n.muc_id WHERE n.trang_thai='DUYET' AND n.dinh_dang='VIDEO' AND m.giai_doan='SAN_XUAT' ${chi?'AND n.id=?':''} ORDER BY n.updated_at DESC LIMIT 20`).bind(...(chi?[chi]:[])).all()).results;
      ...
      const out=[]; for(const nd of ds){ const ct=docJSON(nd.chi_tiet,{}); if(ct.video_url&&!chi) continue; const ts=(await env.DB.prepare(`SELECT id,media_url,media_type,loai,ten,mo_ta,phan_tich FROM tai_san WHERE (muc_id=? OR noi_dung_id=? OR muc_id=?) AND loai IN ('FOOTAGE','ANH') AND COALESCE(nguon,'')<>'THU_NGHIEM_LOAI' ORDER BY (muc_id=? OR noi_dung_id=?) DESC, created_at LIMIT 200`).bind(nd.muc_id, nd.id, nd.muc_dong?khoFootageId(nd.muc_dong):'-', nd.muc_id, nd.id).all()).results.map(t=>({...t, phan_tich:docJSON(t.phan_tich,null)})); const tlm=await MAU().timelineCua(env, ts.map(t=>t.id)); for(const t of ts) if(tlm[t.id]) t.phan_tich={...(t.phan_tich||{}), timeline:tlm[t.id]};
  ...
  if(path==='/chien-luoc' && method==='PUT'){
    ...
    if(body.dinh_huong!==undefined){ await env.DB.prepare(`UPDATE chien_luoc SET dinh_huong=?, updated_at=? WHERE id=1`).bind(chuoi(body.dinh_huong,600), nowISO()).run(); await logAudit(env,me,'sửa định hướng giai đoạn','chien_luoc',1,chuoi(body.dinh_huong,200)); return json({ db: await bootstrap(env,me) }); }
  ...
  if(path==='/chien-luoc/chot' && method==='POST'){
    ...
    const cl=await env.DB.prepare(`SELECT * FROM chien_luoc WHERE id=1`).first(); const b=await banChot(env); if(!(b.ho_so.dong||[]).some(d=>chuoi(d.cau_dinh_vi))) return json({error:'Hồ sơ định vị chưa có câu định vị nào để chốt'},400);
    const pb=so(cl.phien_ban)+1;
    await env.DB.prepare(`INSERT INTO chien_luoc_phien_ban (id,phien_ban,dinh_vi,tong_giong,doi_tuong,pillars,ghi_chu,chot_boi,chot_at,ho_so,dinh_huong,ma) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`).bind(uid('clv'),pb,cl.dinh_vi,cl.tong_giong,cl.doi_tuong,JSON.stringify(b.pillars),chuoi(body.ghi_chu,500),me.ho_ten,nowISO(),JSON.stringify(b.ho_so),b.dinh_huong,b.ma).run();
    await env.DB.prepare(`UPDATE chien_luoc SET phien_ban=?, chot_boi=?, chot_at=?, chot_ma=? WHERE id=1`).bind(pb, me.ho_ten, nowISO(), b.ma).run();
    await logAudit(env,me,'chốt chiến lược (G1)','chien_luoc',pb,chuoi(body.ghi_chu,200)); return json({ db: await bootstrap(env,me) });
  }
  // G1 xem trước: sẽ chốt gì + đã đổi gì so với lần chốt gần nhất (so từng phần của hồ sơ, pillar, định hướng)
  if(path==='/chien-luoc/so-sanh' && method==='GET'){ if(!canXemMkt(me)) return json({error:'Không có quyền'},403); const moi=await banChot(env); const cl=await env.DB.prepare(`SELECT * FROM chien_luoc WHERE id=1`).first()||{};
    const cuR=so(cl.phien_ban)>0?await env.DB.prepare(`SELECT ho_so,pillars,dinh_huong FROM chien_luoc_phien_ban WHERE phien_ban=? ORDER BY chot_at DESC LIMIT 1`).bind(so(cl.phien_ban)).first():null; const cu=cuR&&cuR.ho_so?{ ho_so:docJSON(cuR.ho_so,{})||{}, pillars:docJSON(cuR.pillars,[]), dinh_huong:cuR.dinh_huong||'' }:null;
    return json({ phien_ban:so(cl.phien_ban), chot_boi:cl.chot_boi||null, chot_at:cl.chot_at||null, co_ban_cu:!!cu, da_doi:!cl.chot_ma||cl.chot_ma!==moi.ma, tom_tat:tomTatChot(moi.ho_so, moi.pillars, moi.dinh_huong), thay_doi:cu?soSanhHoSo(cu, moi):[] }); }
  if((m=path.match(/^\/chien-luoc\/phien-ban\/(\d+)$/)) && method==='GET'){ if(!canXemMkt(me)) return json({error:'Không có quyền'},403); const r=await env.DB.prepare(`SELECT * FROM chien_luoc_phien_ban WHERE phien_ban=? ORDER BY chot_at DESC LIMIT 1`).bind(so(m[1])).first(); if(!r) return json({error:'Không có phiên bản'},404);
    const hs=docJSON(r.ho_so,null); return json({ phien_ban:r.phien_ban, chot_boi:r.chot_boi, chot_at:r.chot_at, ghi_chu:r.ghi_chu||'', co_ho_so:!!hs, tom_tat:hs?tomTatChot(hs, docJSON(r.pillars,[]), r.dinh_huong):null, dinh_vi_cu:hs?null:(r.dinh_vi||'') }); }
  // Kế hoạch tháng
  ...
  if((m=path.match(/^\/muc\/([^/]+)\/dong$/)) && method==='POST'){ if(!isStaff(me)) return json({error:'Không có quyền'},403); const cu=await env.DB.prepare(`SELECT id,tieu_de FROM muc_noi_dung WHERE id=?`).bind(m[1]).first(); if(!cu) return json({error:'Không tìm thấy'},404);
    const dong=chuoi(body.dong,80)||null; if(dong&&!(await MAU().dsDongChuan(env)).some(x=>x.dong===dong)) return json({error:'Dòng "'+dong+'" chưa có — thêm ở Bộ nhãn › Dòng sản phẩm'},400);
    await env.DB.prepare(`UPDATE muc_noi_dung SET dong=?, updated_at=? WHERE id=?`).bind(dong, nowISO(), cu.id).run(); await logAudit(env,me,'gán dòng cho mục','muc_noi_dung',cu.id,(dong||'bỏ dòng')+' · '+cu.tieu_de); return json({ db: await bootstrap(env,me) }); }
  if((m=path.match(/^\/tai-san\/([^/]+)\/su-dung$/)) && method==='POST'){ if(!isStaff(me)) return json({error:'Không có quyền'},403); const t=await env.DB.prepare(`SELECT id,ten,loai FROM tai_san WHERE id=?`).bind(m[1]).first(); if(!t||t.loai!=='VIDEO_XUAT') return json({error:'Không có video'},404);
    const sd=['CHON','BO'].includes(body.su_dung)?body.su_dung:null; const lyDo=chuoi(body.ly_do,300); await env.DB.prepare(`UPDATE tai_san SET su_dung=? WHERE id=?`).bind(sd, t.id).run(); await logAudit(env,me,sd==='CHON'?'chọn dùng video (đạt)':sd==='BO'?'loại video (không đạt)':'bỏ đánh dấu video','tai_san',t.id,t.ten+(lyDo?' · '+lyDo:''));
    // đã đánh giá bản nháp → đóng việc "Xem & duyệt video nháp" của bài; không đạt + dựng lại → giao máy dựng lại (26/09)
    const tt=await env.DB.prepare(`SELECT noi_dung_id FROM tai_san WHERE id=?`).bind(t.id).first(); let dungLai=null;
    if(sd&&tt&&tt.noi_dung_id){ await env.DB.prepare(`UPDATE cong_viec SET trang_thai='XONG', xong_at=?, xong_boi=? WHERE loai='DUYET_VIDEO_NHAP' AND doi_tuong_id=? AND trang_thai='MO'`).bind(nowISO(), me.ho_ten+' · '+(sd==='CHON'?'video đạt':'video không đạt'), tt.noi_dung_id).run();
      if(sd==='BO'&&body.dung_lai){ const nd=await env.DB.prepare(`SELECT * FROM noi_dung WHERE id=?`).bind(tt.noi_dung_id).first(); if(nd&&nd.trang_thai==='DUYET'){ const ct=docJSON(nd.chi_tiet,{}); ct.video_bi_loai=[...(ct.video_bi_loai||[]).slice(-9), {url:ct.video_url, ly_do:lyDo, luc:nowISO()}]; delete ct.video_url; delete ct.goi_dung_url; await env.DB.prepare(`UPDATE noi_dung SET chi_tiet=?, updated_at=? WHERE id=?`).bind(JSON.stringify(ct), nowISO(), nd.id).run(); dungLai=await giaoDung(env, {...nd, chi_tiet:JSON.stringify(ct)}, {uuTienUserId:me.id, tacNhan:me}); } } }
    return json({ db: await bootstrap(env,me), dung_lai:dungLai }); }
  if((m=path.match(/^\/muc\/([^/]+)\/truc$/)) && method==='POST'){ if(!isStaff(me)) return json({error:'Không có quyền'},403); const cu=await env.DB.prepare(`SELECT id,dong,tieu_de FROM muc_noi_dung WHERE id=?`).bind(m[1]).first(); if(!cu) return json({error:'Không tìm thấy'},404);
    const T=MAU().TRUC; let tr={}; if(body.de_xuat){ const dp=await MAU().doPhuTruc(env); const hq=((dp.dong[cu.dong||'(chưa có dòng)']||{}).hieu_qua)||null; if(!hq) return json({error:'Dòng '+(cu.dong||'(chưa có)')+' chưa có video gán trục kèm lượt xem để đề xuất — chọn tay'},409);
      const th=(hq.to_hop||[])[0]; for(const k of Object.keys(T)){ const a=(hq.truc[k]||[]).filter(r=>r.n>=2); tr[k]=a[0]?a[0].gt:null; } if(th){ tr.cau_truc=th.cau_truc; tr.mo_dau=th.mo_dau; } tr.nguon='DE_XUAT'; }
    else { for(const k of Object.keys(T)) tr[k]=T[k].gt[body[k]]?body[k]:null; tr.nguon='NGUOI'; }
    await env.DB.prepare(`UPDATE muc_noi_dung SET truc=?, updated_at=? WHERE id=?`).bind(JSON.stringify(tr), nowISO(), cu.id).run(); await logAudit(env,me,body.de_xuat?'đề xuất tổ hợp trục cho mục':'chọn tổ hợp trục cho mục','muc_noi_dung',cu.id,JSON.stringify(tr)); return json({ db: await bootstrap(env,me), truc:tr }); }
  if((m=path.match(/^\/muc\/([^/]+)\/ab$/)) && method==='POST'){ if(!isStaff(me)) return json({error:'Không có quyền'},403); const cu=await env.DB.prepare(`SELECT * FROM muc_noi_dung WHERE id=?`).bind(m[1]).first(); if(!cu) return json({error:'Không tìm thấy'},404);
    const T=MAU().TRUC; const truc=docJSON(cu.truc,{})||{}; const k=body.doi==='cau_truc'?'cau_truc':'mo_dau'; const ds=Object.keys(T[k].gt); const moiGt=chuoi(body.gia_tri,40)&&T[k].gt[body.gia_tri]?body.gia_tri:ds[(ds.indexOf(truc[k])+1)%ds.length];
    const tr={...truc, [k]:moiGt, nguon:'AB', ab_cua:cu.id}; const id=uid('muc');
    await env.DB.prepare(`INSERT INTO muc_noi_dung (id,thang,tuan,ngay_dang,tieu_de,muc_tieu,pillar_id,framework_id,san_pham_id,kenh_id,dinh_dang,giai_doan,tao_boi,ghi_chu,created_at,created_by_name,updated_at,dong,truc) VALUES (?,?,?,?,?,?,?,?,?,?,?,'Y_TUONG','NGUOI',?,?,?,?,?,?)`)
      .bind(id, cu.thang, cu.tuan, cu.ngay_dang, String(cu.tieu_de).replace(/ \[B[^\]]*\]$/,'')+' [B · '+T[k].gt[moiGt]+']', cu.muc_tieu, cu.pillar_id, cu.framework_id, cu.san_pham_id, cu.kenh_id, cu.dinh_dang, (cu.ghi_chu||'')+' · Bản A/B của mục '+cu.id+': đổi '+T[k].ten.toLowerCase()+' → '+T[k].gt[moiGt]+'.', nowISO(), me.ho_ten, nowISO(), cu.dong, JSON.stringify(tr)).run();
  ...
  if((path==='/tai-san'||path==='/footage') && method==='POST'){ if(!isStaff(me)) return json({error:'Không có quyền'},403); const media_url=chuoi(body.media_url,500); if(!media_url) return json({error:'Cần file hoặc link media'},400); if(/drive\.google\.com\/drive\/(u\/\d+\/)?folders\//.test(media_url)) return json({error:'Đây là link THƯ MỤC Drive, không phải một file — dùng nút 📥 Nạp từ Drive để máy tải các clip trong thư mục'},400);
    ...
    await env.DB.prepare(`INSERT INTO tai_san (id,loai,ten,mo_ta,media_url,media_type,muc_id,noi_dung_id,nguon,created_at,created_by_name,ma_video) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`).bind(id, loai, chuoi(body.ten,200)||media_url.split('/').pop(), chuoi(body.mo_ta,500), media_url, String(body.media_type||(loai==='ANH'?'IMAGE':'VIDEO')).toUpperCase(), body.muc_id||null, body.noi_dung_id||null, chuoi(body.nguon,40)||'NGUOI', nowISO(), me.ho_ten, loai==='VIDEO_XUAT'?maVideo(id, nowISO()):null).run();
  ...
  if(path==='/lop-hoc/nap' && method==='POST'){ if(!canGat(me)) return json({error:'Chỉ Trưởng MKT/Admin'},403); const t=String(body.nap||'').trim(); if(!t) return json({error:'Dán link thư mục Drive, đường dẫn thư mục trên máy dựng, @kênh TikTok hoặc tên ngành hàng Kalodata'},400);
    const laDrive=/drive\.google\.com\/drive\/(u\/\d+\/)?folders\//.test(t), laLocal=/^[A-Za-z]:[\\/]|^\//.test(t), laTikTok=/tiktok\.com\/@|^@[A-Za-z0-9_.]{2,}$/.test(t), laFb=/facebook\.com\/|fb\.watch\//.test(t); const loai=laDrive?'DRIVE':laLocal?'LOCAL':laTikTok?'TIKTOK':laFb?'REELS':'KALODATA';
    if(body.chi_nhan) return json({ loai, ten:loai==='KALODATA'?('ngành Kalodata “'+t+'”'):loai==='TIKTOK'?('kênh TikTok '+t):loai==='REELS'?'Reels của fanpage Facebook':loai==='DRIVE'?'thư mục Drive':'thư mục trên máy dựng' });
    if(loai==='REELS'){ const x=await hangTaiReels(env, {link:t, toi_da:body.toi_da, dong:body.dong, muc_dich:body.muc_dich}, me); if(!x.ok) return json({error:x.loi}, x.ma||400); return json({ ok:true, loai, trang:x.trang, lenh_id:x.lenh_id, trung:x.trung }); }
  ...
  if((m=path.match(/^\/kho-thanh-pham\/([^/]+)$/)) && (method==='PATCH'||method==='DELETE')){ if(!canGat(me)) return json({error:'Chỉ Trưởng MKT/Admin'},403); const tp=await env.DB.prepare(`SELECT id,ten FROM kho_thanh_pham WHERE id=?`).bind(m[1]).first(); if(!tp) return json({error:'Không có video này trong kho'},404);
    ...
    if(dong!==undefined){ await env.DB.prepare(`UPDATE kho_thanh_pham SET dong=? WHERE id=?`).bind(dong, tp.id).run(); await MAU().dam(env); await env.DB.prepare(`UPDATE mau_doan SET dong=?, buoc_thu=0, updated_at=? WHERE doi_tuong_id=?`).bind(dong, nowISO(), tp.id).run(); }   /* 26/09: trước đây chỉ đổi video, mẫu giữ dòng cũ */ if(md!==undefined) await env.DB.prepare(`UPDATE kho_thanh_pham SET muc_dich=? WHERE id=?`).bind(md, tp.id).run();
  ...
  if(path==='/kho-thanh-pham/nap' && method==='POST' && String(body.nguon||'').toUpperCase()==='TIKTOK'){ if(!canGat(me)) return json({error:'Chỉ Trưởng MKT/Admin'},403);
    const x=await hangTaiTikTok(env, {link:body.link, toi_da:body.toi_da, dong:body.dong, muc_dich:body.muc_dich}, me); if(!x.ok) return json({error:x.loi}, x.ma||400);
    return json({ db: await bootstrap(env,me), lenh_id:x.lenh_id, trung:x.trung, kenh:x.kenh }); }
