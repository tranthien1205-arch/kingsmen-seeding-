...
async function ensureSchema(env){
  ...
  for(const st of [`ALTER TABLE tai_san ADD COLUMN phan_tich TEXT`,`ALTER TABLE kho_thanh_pham ADD COLUMN luot_xem INTEGER`,`ALTER TABLE kho_thanh_pham ADD COLUMN luot_thich INTEGER`,`ALTER TABLE kho_thanh_pham ADD COLUMN ngay_dang TEXT`,`ALTER TABLE kho_thanh_pham ADD COLUMN link TEXT`,`ALTER TABLE kho_thanh_pham ADD COLUMN kenh TEXT`,`ALTER TABLE kho_thanh_pham ADD COLUMN doanh_thu REAL`,`ALTER TABLE kho_thanh_pham ADD COLUMN luot_ban INTEGER`,`ALTER TABLE kho_thanh_pham ADD COLUMN san_pham TEXT`,`ALTER TABLE kho_thanh_pham ADD COLUMN kich_ban TEXT`,`ALTER TABLE bai_dang ADD COLUMN ma_theo_doi TEXT`,`ALTER TABLE tram_lenh ADD COLUMN may_id TEXT`,`ALTER TABLE ai_usage ADD COLUMN mo_hinh_id TEXT`,`ALTER TABLE ai_usage ADD COLUMN muc TEXT`,`ALTER TABLE mo_hinh_phien_ban ADD COLUMN model_id TEXT`,`ALTER TABLE lead_seeding ADD COLUMN loai TEXT`,`ALTER TABLE lead_seeding ADD COLUMN muc_do INTEGER`,`ALTER TABLE lead_seeding ADD COLUMN goi_y TEXT`,`ALTER TABLE mo_hinh ADD COLUMN base_url TEXT`,`ALTER TABLE mo_hinh ADD COLUMN khoa_env TEXT`]) try{ await env.DB.prepare(st).run(); }catch(e){}
...
async function bootstrap(env, u){
  ...
  return {
    ...
    ai_nao: xem ? await (async()=>{ const th=thangHienTai(); return {
      ...
      kho_thanh_pham: await all(`SELECT id,ten,nguon,thu_muc,dai,so_shot,nhip,co_goc,created_at,luot_xem,luot_thich,ngay_dang,link,kenh,doanh_thu,luot_ban,san_pham,substr(kich_ban,1,300) kich_ban FROM kho_thanh_pham ORDER BY COALESCE(doanh_thu,-1) DESC, COALESCE(luot_xem,-1) DESC, created_at DESC LIMIT 80`),
...
async function handleApi(request, env){
  ...
  if(path.startsWith('/hub/')){
    ...
    if(path==='/hub/phan-tich' && method==='POST'){ const t=await env.DB.prepare(`SELECT * FROM tai_san WHERE id=?`).bind(chuoi(body.tai_san_id,40)).first(); if(!t) return json({error:'Không có tài sản'},404); const khungDs=(Array.isArray(body.khung_urls)?body.khung_urls:[]).map(x=>chuoi(x,500)).filter(Boolean).slice(0,3);
      ...
      const pt=phanTichSach(body.phan_tich, khungDs.length?khungDs:(cu.khung||[]), coCanh); await env.DB.prepare(`UPDATE tai_san SET phan_tich=?, mo_ta=? WHERE id=?`).bind(JSON.stringify(pt), moTa, t.id).run(); return json({ ok:true, co_canh:pt.co_canh }); }
    ...
    if(path==='/hub/tiktok-da-tai' && method==='POST'){ const kenh=chuoi(body.kenh,60), thu=chuoi(body.thu_muc,300); if(!kenh||!thu) return json({error:'Thiếu kênh/thư mục'},400);
      const cfg=await docCauHinh(env); const hang=(((cfg.tai_tiktok||{}).hang)||[]).filter(x=>x.kenh!==kenh); await env.DB.prepare(`UPDATE module_config SET cau_hinh=? WHERE id='tai_tiktok'`).bind(JSON.stringify({hang})).run();
      const mayId=await mayChoViec(env,'mo_hinh')||await mayChoViec(env,'dung_video'); if(!mayId) return json({ok:true, giao:false, loi:'không có máy dựng đang bật — nạp lại sau'});
      const r=await taoLenhTram(env,'hoc_thanh_pham',{nguon:chuoi(body.nguon,10)==='KALODATA'?'KALODATA':'TIKTOK', kenh, duong_dan:thu, toi_da:Math.max(1,Math.min(60,so(body.so,20)))}, MAY('Trạm'), mayId); return json({ ok:true, giao:true, lenh_id:r.id }); }
    ...
    if(path==='/hub/kalodata' && method==='POST'){ const nganh=chuoi(body.nganh,80); const ds=(Array.isArray(body.video)?body.video:[]).slice(0,60).map(v=>({ link:chuoi(v.link,300), tieu_de:chuoi(v.tieu_de,300), kenh:chuoi(v.kenh,80), doanh_thu:v.doanh_thu==null?null:+so(v.doanh_thu).toFixed(2), luot_ban:v.luot_ban==null?null:Math.round(so(v.luot_ban)), luot_xem:v.luot_xem==null?null:Math.round(so(v.luot_xem)), san_pham:chuoi(v.san_pham,200), ngay_dang:chuoi(v.ngay_dang,30) })).filter(v=>/tiktok\.com\/.+\/video\/\d+/.test(v.link));
      ...
      const hang=((cfg.tai_tiktok||{}).hang||[]).filter(x=>x.kenh!==kenh); hang.push({ kenh, nguon:'KALODATA', nganh, toi_da:ds.length, thu_muc:'D:\\may-dung\\thanh-pham\\kalodata\\'+slug, links:ds.map(v=>v.link), meta:Object.fromEntries(ds.map(v=>[v.link, v])), luc:nowISO(), boi:'Kalodata' });
      await env.DB.prepare(`INSERT INTO module_config (id,cau_hinh,updated_at,updated_by_name) VALUES ('tai_tiktok',?,?,'Trạm') ON CONFLICT(id) DO UPDATE SET cau_hinh=excluded.cau_hinh, updated_at=excluded.updated_at`).bind(JSON.stringify({hang:hang.slice(-10)}), nowISO()).run();
      const r=await taoLenhTram(env,'chay_agent',{id:'content_os', viec:'tai_tiktok'}, MAY('Trạm'), 'tram'); return json({ ok:true, so:ds.length, kenh, lenh_id:r.id }); }
    ...
    if(path==='/hub/thanh-pham' && method==='POST'){ const ten=chuoi(body.ten,200); const nid=chuoi(body.nguon_id,120)||ten; if(!ten) return json({error:'Thiếu tên'},400); if(await env.DB.prepare(`SELECT id FROM kho_thanh_pham WHERE nguon_id=?`).bind(nid).first()) return json({ok:true, trung:true});
      ...
      const id=uid('tp'); await env.DB.prepare(`INSERT INTO kho_thanh_pham (id,ten,nguon,nguon_id,thu_muc,dai,so_shot,nhip,co_goc,phan_tich,created_at,luot_xem,luot_thich,ngay_dang,link,kenh,doanh_thu,luot_ban,san_pham,kich_ban) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(id, ten, ['DRIVE','LOCAL','TIKTOK','REELS','KALODATA'].includes(chuoi(body.nguon,10))?chuoi(body.nguon,10):'DRIVE', nid, chuoi(body.thu_muc,200), +dai.toFixed(2), shots.length, +(dai/shots.length).toFixed(2), coGoc, JSON.stringify({shots:shots.map(x=>({...x, goc:x.goc?{...x.goc, doan:undefined}:null}))}), nowISO(), luotXem, body.luot_thich==null?null:Math.round(so(body.luot_thich)), chuoi(body.ngay_dang,30)||null, chuoi(body.link,300)||null, kenh, doanhThu, body.luot_ban==null?null:Math.round(so(body.luot_ban)), chuoi(body.san_pham,200)||null, kichBan).run();
      ...
      const coLoi=shots.filter(x=>x.loi&&x.khung_url); if(coLoi.length>=2) for(const x of coLoi){ await ghiMauAI(env,{tinh_nang:'chon_canh', doi_tuong:'kho_thanh_pham', doi_tuong_id:id, dau_vao:{...soLieu, k:shots.indexOf(x), label:'shot', hinh:'', text:x.loi, ung_vien:coLoi.map(y=>({id:'tp_'+id+'_'+shots.indexOf(y), ten:'shot '+shots.indexOf(y), mo_ta:y.co_canh||'', media_url:y.khung_url, media_type:'IMAGE'}))}, dau_ra:null, nhan:{tai_san_id:'tp_'+id+'_'+shots.indexOf(x), nguon:'THANH_PHAM'}}); n++; }
    ...
    if(path==='/hub/tap-mau-ngon-ngu' && method==='GET'){ const tn=chuoi(url.searchParams.get('tinh_nang'),40)||'soan_nhap_agent'; const ds=(await env.DB.prepare(`SELECT m.id, m.tap, m.dau_vao, m.dau_ra, m.phan_quyet, m.doi_tuong_id, n.trang_thai nd_tt, n.tieu_de, n.hook, n.sections, n.cta, n.chi_tiet FROM mau_hoc_ai m LEFT JOIN noi_dung n ON n.id=m.doi_tuong_id WHERE m.tinh_nang=? AND (m.phan_quyet IS NOT NULL OR n.trang_thai='DUYET') ORDER BY m.created_at DESC LIMIT 3000`).bind(tn).all()).results;
      const banTot=tn==='soan_nhap_agent'?(await env.DB.prepare(`SELECT id,ten,kich_ban,doanh_thu,luot_xem,san_pham,kenh FROM kho_thanh_pham WHERE kich_ban IS NOT NULL AND length(kich_ban)>80 ORDER BY COALESCE(doanh_thu,0) DESC LIMIT 300`).all()).results:[];
      return json({ tinh_nang:tn, mau: [...ds.map(r=>{ const dv=docJSON(r.dau_vao,{}); const pq=docJSON(r.phan_quyet,{})||{}; return { id:r.id, tap:r.tap, system:dv.system||'', user:dv.user||'', may:docJSON(r.dau_ra,r.dau_ra)||'', nguoi:(r.nd_tt==='DUYET'&&(r.tieu_de||r.hook))?JSON.stringify({tieu_de:r.tieu_de, hook:r.hook, sections:docSections(r.sections), cta:r.cta, ...docJSON(r.chi_tiet,{})}):'', quyet:pq.quyet||(r.nd_tt==='DUYET'?'DUYET':''), ly_do:pq.ly_do||'' }; }),
        ...banTot.map((x,i)=>({ id:'tp_'+x.id, tap:i%5===0?'KIEM':'HOC', system:'Bạn viết lời thoại video bán hàng TikTok Shop tiếng Việt: mở đầu 3 giây giữ người xem, nêu vấn đề, bằng chứng, chốt đơn.', user:'Viết lời thoại video bán hàng cho sản phẩm: '+(x.san_pham||x.ten)+(x.kenh?(' (kênh '+x.kenh+')'):''), may:'', nguoi:String(x.kich_ban), quyet:'DUYET', ly_do:'video thành phẩm bán tốt'+(x.doanh_thu?(' · doanh thu ~$'+Math.round(x.doanh_thu)):'')+(x.luot_xem?(' · '+x.luot_xem+' lượt xem'):''), doanh_thu:x.doanh_thu, luot_xem:x.luot_xem }))] }); }
  ...
  if(path==='/kalodata' && method==='PUT'){ if(!canGat(me)) return json({error:'Chỉ Trưởng MKT/Admin'},403); const cfg=await docCauHinh(env); const nganh=(Array.isArray(body.nganh)?body.nganh:String(body.nganh||'').split(/[,;\n]/)).map(x=>chuoi(x,80).trim()).filter(Boolean).slice(0,8);
    const kd={...(cfg.kalodata||{}), nganh, top_n:Math.max(3,Math.min(50,so(body.top_n,10))), tu_dong:body.tu_dong!==false, khu_vuc:chuoi(body.khu_vuc,5)||'VN', thoi_gian:['7','30','90'].includes(String(body.thoi_gian))?String(body.thoi_gian):'30'};
  ...
  if(path==='/kho-thanh-pham/nap' && method==='POST' && String(body.nguon||'').toUpperCase()==='TIKTOK'){ if(!canGat(me)) return json({error:'Chỉ Trưởng MKT/Admin'},403);
    ...
    const cfg=await docCauHinh(env); const ds=((cfg.tai_tiktok||{}).hang||[]).filter(x=>x.kenh!==kenh); ds.push({kenh, toi_da:Math.max(1,Math.min(60,so(body.toi_da,20))), thu_muc:'D:\\may-dung\\thanh-pham\\tiktok\\'+kenh.slice(1), luc:nowISO(), boi:me.ho_ten});
  ...
  if(path==='/kho-thanh-pham/nap' && method==='POST'){ if(!canGat(me)) return json({error:'Chỉ Trưởng MKT/Admin'},403); const lk=String(body.link||'').trim();
    ...
    const r=await taoLenhTram(env,'hoc_thanh_pham',{nguon:fm?'DRIVE':'LOCAL', folder_id:fm?fm[1]:null, duong_dan:local?lk.slice(0,300):null, toi_da:Math.max(1,Math.min(60,so(body.toi_da,20)))}, me, mayId);
