...
async function ensureSchema(env){
  ...
  for(const st of [`ALTER TABLE san_pham ADD COLUMN quy_trinh TEXT`,`ALTER TABLE tai_san ADD COLUMN phan_tich TEXT`,`ALTER TABLE kho_thanh_pham ADD COLUMN luot_xem INTEGER`,`ALTER TABLE kho_thanh_pham ADD COLUMN luot_thich INTEGER`,`ALTER TABLE kho_thanh_pham ADD COLUMN ngay_dang TEXT`,`ALTER TABLE kho_thanh_pham ADD COLUMN link TEXT`,`ALTER TABLE kho_thanh_pham ADD COLUMN kenh TEXT`,`ALTER TABLE kho_thanh_pham ADD COLUMN doanh_thu REAL`,`ALTER TABLE kho_thanh_pham ADD COLUMN luot_ban INTEGER`,`ALTER TABLE kho_thanh_pham ADD COLUMN san_pham TEXT`,`ALTER TABLE kho_thanh_pham ADD COLUMN kich_ban TEXT`,`ALTER TABLE kho_thanh_pham ADD COLUMN dong TEXT`,`ALTER TABLE kho_thanh_pham ADD COLUMN muc_dich TEXT`,`ALTER TABLE mau_hoc_ai ADD COLUMN dong TEXT`,`ALTER TABLE mau_hoc_ai ADD COLUMN muc_dich TEXT`,`ALTER TABLE mo_hinh_phien_ban ADD COLUMN pham_vi TEXT DEFAULT 'chung'`,`ALTER TABLE bai_dang ADD COLUMN ma_theo_doi TEXT`,`ALTER TABLE tram_lenh ADD COLUMN may_id TEXT`,`ALTER TABLE ai_usage ADD COLUMN mo_hinh_id TEXT`,`ALTER TABLE ai_usage ADD COLUMN muc TEXT`,`ALTER TABLE mo_hinh_phien_ban ADD COLUMN model_id TEXT`,`ALTER TABLE lead_seeding ADD COLUMN loai TEXT`,`ALTER TABLE lead_seeding ADD COLUMN muc_do INTEGER`,`ALTER TABLE lead_seeding ADD COLUMN goi_y TEXT`,`ALTER TABLE mo_hinh ADD COLUMN base_url TEXT`,`ALTER TABLE mo_hinh ADD COLUMN khoa_env TEXT`]) try{ await env.DB.prepare(st).run(); }catch(e){}
...
const DANH_MUC = {
  ...
  san_pham:   { truong:['ma','ten','dong','mo_ta','thong_so','tieu_chuan','bao_hanh','huong_dan','quy_trinh','active'], batBuoc:['ten'] },
...
async function handleApi(request, env){
  ...
  if(path.startsWith('/hub/')){
    ...
    if(path==='/hub/viec/doc_khung' && method==='GET'){ const mid=chuoi(url.searchParams.get('muc_id'),40), tid=chuoi(url.searchParams.get('tai_san_id'),40), lai=url.searchParams.get('lai')==='1'; const goc=env.APP_BASE_URL||url.origin;
      const ds=(await env.DB.prepare(`SELECT t.id,t.ten,t.media_url,t.phan_tich,t.muc_id,m.tieu_de,sp.ten sp_ten,sp.dong,sp.quy_trinh FROM tai_san t LEFT JOIN muc_noi_dung m ON m.id=t.muc_id LEFT JOIN san_pham sp ON sp.id=m.san_pham_id WHERE t.loai='FOOTAGE' AND t.media_type='VIDEO' AND (?='' OR t.muc_id=?) AND (?='' OR t.id=?) ORDER BY t.created_at LIMIT 60`).bind(mid||'',mid||'',tid||'',tid||'').all()).results;
      return json({ viec: ds.filter(t=>lai||!docJSON(t.phan_tich,{}).timeline).map(t=>({ id:t.id, ten:t.ten, media_url:/^https?:/.test(t.media_url)?t.media_url:goc+t.media_url, dai:so(docJSON(t.phan_tich,{}).dai), chu_de:t.tieu_de||'', san_pham:t.sp_ten||'', dong:t.dong||'', quy_trinh:String(t.quy_trinh||'').split(/\r?\n/).map(x=>x.trim()).filter(Boolean).slice(0,20) })) }); }
    if(path==='/hub/doc-khung' && method==='POST'){ const t=await env.DB.prepare(`SELECT id,phan_tich FROM tai_san WHERE id=?`).bind(chuoi(body.tai_san_id,40)).first(); if(!t) return json({error:'Không có tài sản'},404);
      const tl=(Array.isArray(body.timeline)?body.timeline:[]).slice(0,120).map(x=>({ tu:+so(x.tu).toFixed(2), den:+so(x.den).toFixed(2), buoc:chuoi(x.buoc,80), hanh_dong:chuoi(x.hanh_dong,120), vat_lieu:chuoi(x.vat_lieu,120), co_canh:CO_CANH.includes(x.co_canh)?x.co_canh:null, tham_my:Math.max(0,Math.min(10,so(x.tham_my))), ro_net:Math.max(0,Math.min(10,so(x.ro_net))), co_nguoi:!!x.co_nguoi, mo_ta:chuoi(x.mo_ta,200) })).filter(x=>x.den>x.tu);
      if(!tl.length) return json({error:'Dòng thời gian rỗng'},400); const pt=docJSON(t.phan_tich,{})||{}; pt.timeline=tl; pt.vl={ model:chuoi(body.model,60), so_khung:so(body.so_khung), ms_khung:so(body.ms_khung), luc:nowISO(), may:chuoi(body.may,80) };
