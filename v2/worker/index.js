...
async function chayThayChu(env){ const c=await cfgThayChu(env); if(!c.bat) return {bo_qua:'chế độ AI thay chủ đang tắt'}; const kq=[];
  ...
  for(const v of ds.slice(0,20)){ try{
    ...
    else if(v.loai==='hoc_tiktok'){ const dong=chuoi(v.dong,80); if(dong&&!(await MAU().dsDongChuan(env)).some(x=>x.dong===dong)){ kq.push([v.kenh,'bỏ: dòng không có trong Bộ nhãn']); continue; } const x=await hangTaiTikTok(env, {link:v.kenh, toi_da:v.toi_da, dong, muc_dich:v.muc_dich}, AI_THAY_CHU); kq.push([v.kenh, x.ok?('học kênh · '+(x.trung?'đã có lệnh ':'')+x.lenh_id+' · Trạm tải'):('bỏ: '+x.loi)]); }
...
async function hangTaiTikTok(env, {link, toi_da, dong, muc_dich}, me){
  ...
  const tt=await docTramTrangThai(env); if(!tt||!tt.song) return {ok:false, ma:409, loi:'Trạm văn phòng đang im — TikTok tải qua phiên đăng nhập trên Trạm'};
  const cfg=await docCauHinh(env); const ds=((cfg.tai_tiktok||{}).hang||[]).filter(x=>x.kenh!==kenh); ds.push({kenh, toi_da:Math.max(1,Math.min(60,so(toi_da,20))), thu_muc:'D:\\may-dung\\thanh-pham\\tiktok\\'+kenh.slice(1), luc:nowISO(), boi:me.ho_ten, dong:chuoi(dong,80)||null, muc_dich:MUC_DICH.includes(chuoi(muc_dich,10))?chuoi(muc_dich,10):null});
  ...
  const r=await taoLenhTram(env,'chay_agent',{id:'content_os', viec:'tai_tiktok'}, me, 'tram'); await logAudit(env,me,'nạp kho thành phẩm TikTok','kho_thanh_pham',kenh,'tối đa '+ds[ds.length-1].toi_da+(dong?(' · '+dong):''));
...
async function handleApi(request, env){
  ...
  if((m=path.match(/^\/tai-san\/([^/]+)\/su-dung$/)) && method==='POST'){ if(!isStaff(me)) return json({error:'Không có quyền'},403); const t=await env.DB.prepare(`SELECT id,ten,loai FROM tai_san WHERE id=?`).bind(m[1]).first(); if(!t||t.loai!=='VIDEO_XUAT') return json({error:'Không có video'},404);
    const sd=['CHON','BO'].includes(body.su_dung)?body.su_dung:null; await env.DB.prepare(`UPDATE tai_san SET su_dung=? WHERE id=?`).bind(sd, t.id).run(); await logAudit(env,me,sd==='CHON'?'chọn dùng video':sd==='BO'?'loại video':'bỏ đánh dấu video','tai_san',t.id,t.ten); return json({ db: await bootstrap(env,me) }); }
