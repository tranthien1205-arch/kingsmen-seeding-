...
async function bootstrap(env, u){
  ...
  return {
    ...
    ai_nao: xem ? await (async()=>{ const th=thangHienTai(); return {
      ...
      do_chinh_xac_hinh: await (async()=>{ const ds=(await all(`SELECT dau_ra, nhan FROM mau_hoc_ai WHERE tinh_nang='nhan_khung'`)).map(x=>({c:docJSON(x.dau_ra,{}), n:docJSON(x.nhan,{})})); const dn=ds.filter(x=>x.c.nhom===x.n.nhom).length, dc=ds.filter(x=>x.c.nhom===x.n.nhom&&(x.c.buoc||null)===(x.n.buoc||null)&&(x.c.bai_test||null)===(x.n.bai_test||null)).length; return { so_nhan:ds.length, dung_nhom:dn, dung_chi_tiet:dc }; })(),
...
async function handleApi(request, env){
  ...
  if(path.startsWith('/hub/')){
    ...
    if(path==='/hub/thanh-pham' && method==='POST'){ const ten=chuoi(body.ten,200); const nid=chuoi(body.nguon_id,120)||ten; if(!ten) return json({error:'Thiếu tên'},400); const daCo=await env.DB.prepare(`SELECT id FROM kho_thanh_pham WHERE nguon_id=?`).bind(nid).first(); if(daCo&&!body.lam_lai) return json({ok:true, trung:true});
      if(daCo){ await env.DB.prepare(`DELETE FROM mau_hoc_ai WHERE doi_tuong='kho_thanh_pham' AND doi_tuong_id=?`).bind(daCo.id).run(); await env.DB.prepare(`DELETE FROM kho_thanh_pham WHERE id=?`).bind(daCo.id).run(); }   // học lại: thay bản cũ cùng mẫu của nó
  ...
  if((m=path.match(/^\/tai-san\/([^/]+)\/doan\/(\d+)$/)) && method==='POST'){ if(!isStaff(me)) return json({error:'Không có quyền'},403); const t=await env.DB.prepare(`SELECT t.id,t.ten,t.phan_tich,sp.dong,sp.quy_trinh FROM tai_san t LEFT JOIN muc_noi_dung mu ON mu.id=t.muc_id LEFT JOIN san_pham sp ON sp.id=mu.san_pham_id WHERE t.id=?`).bind(m[1]).first(); if(!t) return json({error:'Không có footage'},404);
    const pt=docJSON(t.phan_tich,{})||{}; const tl=Array.isArray(pt.timeline)?pt.timeline:[]; const i=so(m[2]); const d=tl[i]; if(!d) return json({error:'Không có đoạn '+i},404);
    const nhom=NHOM_CANH.includes(body.nhom)?body.nhom:d.nhom; const moi={ nhom, buoc:nhom==='THI_CONG'?(chuoi(body.buoc,80)||d.buoc||null):null, bai_test:nhom==='THU_NGHIEM'?(chuoi(body.bai_test,80)||d.bai_test||null):null };
    if(body.tham_my!=null) moi.tham_my=Math.max(0,Math.min(10,so(body.tham_my)));
    const cu={ nhom:d.nhom, buoc:d.buoc||null, bai_test:d.bai_test||null, tham_my:d.tham_my };
    tl[i]={ ...d, ...moi, tu_tin:1, can_xac_nhan:false, nguoi:me.ho_ten, nguoi_luc:nowISO() }; pt.timeline=tl; await env.DB.prepare(`UPDATE tai_san SET phan_tich=? WHERE id=?`).bind(JSON.stringify(pt), t.id).run();
    await ghiMauAI(env,{tinh_nang:'nhan_khung', doi_tuong:'tai_san', doi_tuong_id:t.id, dau_vao:{i, tu:d.tu, den:d.den, mo_ta:d.mo_ta, hanh_dong:d.hanh_dong, cu, tu_tin_may:d.tu_tin}, dau_ra:cu, nhan:moi, pham_vi:{dong:t.dong||null, muc_dich:null}});
    const dung=cu.nhom===moi.nhom&&(cu.buoc||null)===(moi.buoc||null)&&(cu.bai_test||null)===(moi.bai_test||null); await logAudit(env,me,dung?'xác nhận nhãn hình':'sửa nhãn hình','tai_san',t.id,'đoạn '+i+': '+(dung?'':(cu.nhom+'/'+(cu.buoc||'')+' → '))+moi.nhom+'/'+(moi.buoc||moi.bai_test||''));
