...
async function angleFootage(env, mucId, dinhDang, goc){ if(!mucId||dinhDang!=='VIDEO') return goc||''; const mm=await env.DB.prepare(`SELECT dong FROM muc_noi_dung WHERE id=?`).bind(mucId).first(); const kho=mm&&mm.dong?khoFootageId(mm.dong):'-';
