...
async function moTaKhungHinh(env, khungUrl, chuDe){ const key=env.ANTHROPIC_API_KEY; if(!key||!khungUrl||!/^\/media\//.test(khungUrl)||!env.MEDIA) return {mo_ta:'', loi:key?null:'chưa có ANTHROPIC_API_KEY'};
  try{ const obj=await env.MEDIA.get(khungUrl.slice(7)); if(!obj) return {mo_ta:'', loi:'không thấy khung'}; const buf=new Uint8Array(await obj.arrayBuffer()); let bin=''; for(let i=0;i<buf.length;i+=0x8000) bin+=String.fromCharCode(...buf.subarray(i,i+0x8000));
    const res=await fetch('https://api.anthropic.com/v1/messages',{method:'POST', headers:{'content-type':'application/json','x-api-key':key,'anthropic-version':'2023-06-01'}, body:JSON.stringify({ model:'claude-haiku-4-5-20251001', max_tokens:160,
      messages:[{role:'user', content:[{type:'image', source:{type:'base64', media_type:'image/jpeg', data:btoa(bin)}}, {type:'text', text:'Đây là một khung hình trong clip quay cho video marketing vật liệu xây dựng (chủ đề: '+String(chuDe||'').slice(0,120)+'). Chỉ dùng tiếng Việt có dấu (không chữ Hán). Viết MỘT câu dưới 30 chữ mô tả đúng thứ nhìn thấy (vật, người, hành động, bối cảnh, màu), rồi dấu | và 5–8 từ khoá cách nhau dấu phẩy. Không suy đoán thứ không thấy.'}]}] })});
    ...
    return {mo_ta: t.replace(/[　-鿿가-힯]+/g,' ').replace(/\s+/g,' ').trim().slice(0,300)};   /* bỏ chữ Hán/Hàn lọt vào (đo 24/09: "t施工") */ }catch(e){ return {mo_ta:'', loi:String(e.message||e).slice(0,120)}; } }
...
async function handleApi(request, env){
  ...
  if(path.startsWith('/hub/')){
    ...
    if(path==='/hub/tai-san' && method==='POST'){ const mucId=chuoi(body.muc_id,40); const muc=mucId?await env.DB.prepare(`SELECT id,tieu_de FROM muc_noi_dung WHERE id=?`).bind(mucId).first():null; if(!muc) return json({error:'Không có mục'},404);
      ...
      const mt2=await moTaKhungHinh(env, chuoi(body.khung_url,500), muc.tieu_de); const moTa=[mt2.mo_ta, chuoi(body.thu_muc,120)?('thư mục '+chuoi(body.thu_muc,120)):'', body.giay?('clip '+so(body.giay)+' giây'):'', 'nguồn Drive'].filter(Boolean).join(' · ');
      const id=uid('ts'); await env.DB.prepare(`INSERT INTO tai_san (id,loai,ten,mo_ta,media_url,media_type,muc_id,noi_dung_id,nguon,created_at,created_by_name) VALUES (?,?,?,?,?,?,?,?,?,?,?)`).bind(id, mt==='IMAGE'?'ANH':'FOOTAGE', ten, moTa.slice(0,500), media_url, mt, mucId, nd?nd.id:null, 'DRIVE', nowISO(), MAY('Máy (nạp Drive)').ho_ten).run();
