...
async function ensureSchema(env){
  ...
  for(const st of [`ALTER TABLE san_pham ADD COLUMN quy_trinh TEXT`,`ALTER TABLE san_pham ADD COLUMN bai_test TEXT`,`ALTER TABLE tai_san ADD COLUMN phan_tich TEXT`,`ALTER TABLE kho_thanh_pham ADD COLUMN luot_xem INTEGER`,`ALTER TABLE kho_thanh_pham ADD COLUMN luot_thich INTEGER`,`ALTER TABLE kho_thanh_pham ADD COLUMN ngay_dang TEXT`,`ALTER TABLE kho_thanh_pham ADD COLUMN link TEXT`,`ALTER TABLE kho_thanh_pham ADD COLUMN kenh TEXT`,`ALTER TABLE kho_thanh_pham ADD COLUMN doanh_thu REAL`,`ALTER TABLE kho_thanh_pham ADD COLUMN luot_ban INTEGER`,`ALTER TABLE kho_thanh_pham ADD COLUMN san_pham TEXT`,`ALTER TABLE kho_thanh_pham ADD COLUMN kich_ban TEXT`,`ALTER TABLE kho_thanh_pham ADD COLUMN dong TEXT`,`ALTER TABLE kho_thanh_pham ADD COLUMN muc_dich TEXT`,`ALTER TABLE mau_hoc_ai ADD COLUMN dong TEXT`,`ALTER TABLE mau_hoc_ai ADD COLUMN muc_dich TEXT`,`ALTER TABLE mo_hinh_phien_ban ADD COLUMN pham_vi TEXT DEFAULT 'chung'`,`ALTER TABLE bai_dang ADD COLUMN ma_theo_doi TEXT`,`ALTER TABLE tram_lenh ADD COLUMN may_id TEXT`,`ALTER TABLE ai_usage ADD COLUMN mo_hinh_id TEXT`,`ALTER TABLE ai_usage ADD COLUMN muc TEXT`,`ALTER TABLE mo_hinh_phien_ban ADD COLUMN model_id TEXT`,`ALTER TABLE lead_seeding ADD COLUMN loai TEXT`,`ALTER TABLE lead_seeding ADD COLUMN muc_do INTEGER`,`ALTER TABLE lead_seeding ADD COLUMN goi_y TEXT`,`ALTER TABLE mo_hinh ADD COLUMN base_url TEXT`,`ALTER TABLE mo_hinh ADD COLUMN khoa_env TEXT`]) try{ await env.DB.prepare(st).run(); }catch(e){}
...
function nhanGon(x){ if(!x||typeof x!=='object'||!NHOM_CANH.includes(x.nhom)) return null; return { nhom:x.nhom, buoc:chuoi(x.buoc,80)||null, bai_test:chuoi(x.bai_test,80)||null, chac:x.chac==null?null:+Math.max(0,Math.min(1,so(x.chac))).toFixed(2), model:chuoi(x.model,60)||null, ly_do:chuoi(x.ly_do,120)||null }; }
...
// ADR-016b — THẦY: Claude đọc một đoạn (≤ 3 dải hình, mỗi dải 3 khung liên tiếp) theo đúng tập nhãn đóng của sản phẩm. Nhãn thầy là nhãn "bạc":
// khớp với mô hình mở → dùng để dạy; bất đồng → vào hàng người gán. Chi phí ghi tính năng hoc_nhan_khung (chịu hạn mức ngân sách học).
async function thayDocDoan(env, x){ const key=env.ANTHROPIC_API_KEY; if(!key) return {ok:false, loi:'chưa có ANTHROPIC_API_KEY'};
  ...
  const cfg=(await docCauHinh(env)).ai||{}; const model=chuoi(cfg.thay_nhin_model,60)||'claude-opus-5'; const doiMoi=/^claude-(opus-5|fable-5|sonnet-5)/.test(model);   // đời 5: suy nghĩ thích ứng, effort, dự phòng khi bị từ chối
  if(so(cfg.ngan_sach_thay_usd)>0){ const da=await env.DB.prepare(`SELECT COALESCE(SUM(chi_phi_usd),0) usd FROM ai_usage WHERE thang=? AND tinh_nang IN ('hoc_nhan_khung','hoc_doc_loi')`).bind(thangHienTai()).first()||{}; if(so(da.usd)>=so(cfg.ngan_sach_thay_usd)) return {ok:false, vuot_ngan_sach:true, loi:'Hết ngân sách thầy tháng này ('+so(da.usd).toFixed(2)+'/'+so(cfg.ngan_sach_thay_usd)+' USD)'}; }
  ...
  const loi='Bạn là kỹ thuật viên thi công vật liệu xây dựng kiêm biên tập video của Kingsmen. '+anh.length+' ảnh là các DẢI của MỘT đoạn video (mỗi dải 3 khung liên tiếp, trái → phải, cách nhau 0,5 giây)'+(x.san_pham?(' — sản phẩm "'+chuoi(x.san_pham,80)+'"'):'')+'.\n'+(x.ngu_canh?('Ngữ cảnh: '+chuoi(x.ngu_canh,900)+'\n'):'')+
    'NHÓM CẢNH (chọn đúng một mã):\n'+NHOM_CANH.map(k=>'- '+k+': '+TEN_NHOM_CANH[k]).join('\n')+'\n'+
    'BƯỚC THI CÔNG (chỉ khi THI_CONG, chép đúng nguyên văn một dòng, không rõ thì null):\n'+(qt.length?qt.map((b,i)=>(i+1)+'. '+b).join('\n'):'(chưa khai — null)')+'\n'+
    'BÀI TEST (chỉ khi THU_NGHIEM'+(bt.length?', chép đúng nguyên văn một dòng':', mô tả ngắn')+', không rõ thì null):\n'+(bt.length?bt.map((b,i)=>(i+1)+'. '+b).join('\n'):'(chưa khai)')+'\n'+
    'THẨM MỸ (chỉ khi thấy bề mặt/mạch hoàn thiện, không thì -1): 0–3 bẩn, lem, chưa xong · 4–6 đang thi công · 7–8 xong nhưng chưa sạch/chưa đều · 9–10 đều, sạch, đáng làm cảnh chốt.\n'+
    'chac: bạn chắc bao nhiêu (0–1) về nhóm + bước/bài test. Nhìn không rõ, hoặc có thể là hai nhóm, thì ghi dưới 0,6 — đừng ghi 1 theo thói quen.\n'+
    (Array.isArray(x.vi_du)&&x.vi_du.length?('Người của Kingsmen đã sửa những lần trước — làm theo đúng cách gán và cách tả này:\n'+x.vi_du.slice(0,12).map(v=>'- '+chuoi(v,200)).join('\n')+'\n'):'')+
    'Chỉ trả MỘT JSON, không giải thích ngoài JSON: {"nhom":"…","buoc":"…"|null,"bai_test":"…"|null,"tham_my":số,"chac":số,"mo_ta":"≤ 20 chữ tiếng Việt có dấu, đúng thứ nhìn thấy","ly_do":"≤ 15 chữ vì sao chọn nhóm này"}';
  const t0=Date.now(); let res, j; try{ res=await fetch('https://api.anthropic.com/v1/messages',{method:'POST', headers:{'content-type':'application/json','x-api-key':key,'anthropic-version':'2023-06-01', ...(doiMoi?{'anthropic-beta':'server-side-fallback-2026-07-01'}:{})}, body:JSON.stringify({ model, max_tokens:doiMoi?4000:300, ...(doiMoi?{ output_config:{effort:chuoi(cfg.thay_nhin_effort,10)||'medium'}, fallbacks:'default' }:{}), messages:[{role:'user', content:[...anh, {type:'text', text:loi}]}] })}); j=await res.json().catch(()=>({})); }catch(e){ return {ok:false, loi:String(e.message||e).slice(0,120)}; }
  if(j&&j.stop_reason==='refusal') return {ok:false, loi:'thầy từ chối đọc đoạn này'};
  const vao=so(j.usage&&j.usage.input_tokens), ra=so(j.usage&&j.usage.output_tokens);
  await ghiAIUsage(env,{provider:'anthropic', model:chuoi(j&&j.model,60)||model, tinh_nang:'hoc_nhan_khung', tokens_vao:vao, tokens_ra:ra, ok:!!res.ok, ms:Date.now()-t0, loi:res.ok?null:String((j.error&&j.error.message)||res.status).slice(0,200), muc:'API'});
  if(!res.ok) return {ok:false, loi:String((j.error&&j.error.message)||res.status).slice(0,120)};
  ...
  const tm=Number(o.tham_my); return {ok:true, model:chuoi(j&&j.model,60)||model, nhan:{ nhom, buoc, bai_test:test, tham_my:Number.isFinite(tm)&&tm>=0?Math.min(10,tm):null, chac:Math.max(0,Math.min(1,so(o.chac))), mo_ta:chuoi(o.mo_ta,200), ly_do:chuoi(o.ly_do,120) }, vao, ra };
}
async function phutNguoiHomNay(env){ const tu=new Date(Date.parse(ngayVN()+'T00:00:00Z')-7*36e5).toISOString(); const ds=(await env.DB.prepare(`SELECT dau_vao FROM mau_hoc_ai WHERE tinh_nang IN ('nhan_khung','chat_luong_source') AND created_at>=?`).bind(tu).all()).results; const lo=(await env.DB.prepare(`SELECT nhan FROM mau_hoc_ai WHERE tinh_nang='doc_loi' AND cham_at>=? AND nhan LIKE '%"NGUOI"%'`).bind(tu).all()).results;
  return (ds.reduce((a,x)=>a+(so((docJSON(x.dau_vao,{})||{}).giay)||5),0)+lo.reduce((a,x)=>a+(so((docJSON(x.nhan,{})||{}).giay)||5),0))/60; }
...
async function hocNguongSource(env){ const ds=(await env.DB.prepare(`SELECT dau_vao, nhan, tap FROM mau_hoc_ai WHERE tinh_nang='chat_luong_source'`).all()).results.map(x=>({v:docJSON(x.dau_vao,{})||{}, n:docJSON(x.nhan,{})||{}, tap:x.tap})).filter(x=>x.v.so_do&&typeof x.n.dung==='boolean');
  if(ds.length<30) return {ok:false, loi:'cần ≥ 30 lần người quyết (có '+ds.length+')'}; const hoc=ds.filter(x=>x.tap!=='KIEM'), kiem=ds.filter(x=>x.tap==='KIEM'); const chamTren=(tap,ng)=>tap.filter(x=>luatSource(ng,x.v.so_do).dung===x.n.dung).length;
  let best=null; for(let net=0.40;net<=0.80;net+=0.02) for(let dong=0.10;dong<=0.60;dong+=0.05) for(let s0=0.05;s0<=0.30;s0+=0.05) for(let s1=0.78;s1<=0.99;s1+=0.03){ const ng={net:+net.toFixed(2),dong:+dong.toFixed(2),sang0:+s0.toFixed(2),sang1:+s1.toFixed(2)}; const d=chamTren(hoc,ng); if(!best||d>best.d) best={ng,d}; }
  const cfg=await docCauHinh(env); const hl=cfg.huan_luyen||{}; const cu=hl.source_nguong||{}; const tapDo=kiem.length>=8?kiem:hoc;
  const moi={...best.ng, khop_pct:Math.round(chamTren(tapDo,best.ng)/tapDo.length*100), khop_cu_pct:Math.round(chamTren(tapDo,cu)/tapDo.length*100), n:ds.length, n_kiem:kiem.length, luc:nowISO()};
  const row=await env.DB.prepare(`SELECT cau_hinh FROM module_config WHERE id='huan_luyen'`).first(); const o=row?docJSON(row.cau_hinh,{}):{}; o.source_hoc=moi;
...
// ADR-017 đợt B — K4 ĐỌC LỜI: Claude (Haiku, rẻ) gán nhóm cảnh / bước cho câu thoại video đã air theo lô 20 câu; người xác nhận chỗ thầy khác nhãn hình hoặc không chắc.
async function thayDocLoi(env, toiDa=60){ const cfg=await docCauHinh(env); const hl=cfg.huan_luyen||{}; const ai=cfg.ai||{}; if(hl.thay_loi===false||ai.thay_nhin===false) return {ok:false, tat:true}; const key=env.ANTHROPIC_API_KEY; if(!key) return {ok:false, loi:'chưa có ANTHROPIC_API_KEY'};
  ...
  const ds=(await env.DB.prepare(`SELECT id, dau_vao, dong FROM mau_hoc_ai WHERE tinh_nang='doc_loi' AND (dau_ra IS NULL OR dau_ra NOT LIKE '%"thay"%') ORDER BY created_at LIMIT ?`).bind(Math.max(1,Math.min(200,toiDa))).all()).results; if(!ds.length) return {ok:true, so:0};
  const spAll=(await env.DB.prepare(`SELECT dong, quy_trinh, bai_test FROM san_pham`).all()).results; const model=chuoi(hl.thay_loi_model,60)||'claude-haiku-4-5-20251001'; let xong=0, loi=null;
  const nhomTheoDong={}; for(const x of ds) (nhomTheoDong[x.dong||'']=nhomTheoDong[x.dong||'']||[]).push(x);
  for(const [dong, ds0] of Object.entries(nhomTheoDong)) for(let b=0;b<ds0.length;b+=20){ const lo=ds0.slice(b,b+20); const sp=spAll.find(x=>dong&&x.dong===dong&&(x.quy_trinh||x.bai_test))||{}; const qt=dsDongSP(sp.quy_trinh), bt=dsDongSP(sp.bai_test);
    const cau=lo.map((x,i)=>(i+1)+'. "'+String((docJSON(x.dau_vao,{})||{}).text||'').slice(0,300).replace(/"/g,"'")+'"').join('\n');
    const p='Bạn là biên tập viên video của Kingsmen (vật liệu xây dựng'+(dong?(', dòng '+dong):'')+'). Dưới đây là các câu thoại (máy nghe chép lại, có thể sai chữ) trong video đã đăng. Với MỖI câu, cho biết câu đó đang nói về nhóm cảnh nào để chọn hình minh hoạ đúng.\nNHÓM: '+NHOM_CANH.map(k=>k+' ('+TEN_NHOM_CANH[k]+')').join('; ')+'.\nBƯỚC THI CÔNG (chỉ khi THI_CONG, chép đúng một dòng hoặc null): '+(qt.length?qt.join(' | '):'(chưa khai — null)')+'.\nBÀI TEST (chỉ khi THU_NGHIEM, chép đúng một dòng hoặc null): '+(bt.length?bt.join(' | '):'(chưa khai — null)')+'.\nNếu câu nghe sai tới mức không hiểu thì nghe_sai=true. chac 0–1, không chắc thì ghi dưới 0,6.\nCÂU:\n'+cau+'\nChỉ trả MỘT mảng JSON, mỗi phần tử {"i":số thứ tự,"nhom":"…","buoc":"…"|null,"bai_test":"…"|null,"chac":số,"nghe_sai":true|false}.';
    ...
    const txt=((j.content||[]).find(c=>c.type==='text')||{}).text||''; let arr=[]; try{ arr=JSON.parse(txt.slice(txt.indexOf('['), txt.lastIndexOf(']')+1)); }catch(e){ arr=[]; }
    for(const o of (Array.isArray(arr)?arr:[])){ const x=lo[so(o.i)-1]; if(!x||!NHOM_CANH.includes(o.nhom)) continue; const th={ nhom:o.nhom, buoc:o.nhom==='THI_CONG'&&qt.includes(o.buoc)?o.buoc:null, bai_test:o.nhom==='THU_NGHIEM'&&o.bai_test?(bt.length?(bt.includes(o.bai_test)?o.bai_test:null):chuoi(o.bai_test,80)):null, chac:+Math.max(0,Math.min(1,so(o.chac))).toFixed(2), nghe_sai:!!o.nghe_sai, model };
      await env.DB.prepare(`UPDATE mau_hoc_ai SET dau_ra=? WHERE id=?`).bind(JSON.stringify({thay:th}), x.id).run(); xong++; } }
  return {ok:true, so:xong, loi}; }
// ===== KHO MẪU — một chỗ xem mọi nhãn (nguồn sự thật vẫn là dòng thời gian / mẫu học; đây chỉ là góc nhìn đọc ra) =====
// Trạng thái: VANG người đã gán · BAC thầy + mô hình mở khớp (dùng để dạy) · LECH hai bên bất đồng (chờ người) · THAY chỉ thầy · MO chỉ mô hình mở
//             KHONG_RO người báo hình không rõ · NGHE_SAI thầy báo câu nghe sai · CHO_THAY câu chưa có thầy đọc · LUAT (source) chỉ luật máy
async function khoMau(env, q){ const kn=['K1','K2','K4'].includes(q.get('kn'))?q.get('kn'):'K1'; const tt=chuoi(q.get('tt'),12), dong=chuoi(q.get('dong'),80), nguon=chuoi(q.get('nguon'),12), tim=String(q.get('q')||'').toLowerCase().slice(0,80);
  const trang=Math.max(1,so(q.get('trang'))||1), n=Math.max(10,Math.min(60,so(q.get('n'))||30)); const rows=[]; const nh=o=>o&&o.nhom?{nhom:o.nhom, buoc:o.buoc||null, bai_test:o.bai_test||null}:null; const cung=(a,b)=>a&&b&&a.nhom===b.nhom&&(a.buoc||null)===(b.buoc||null);
  if(kn==='K1'||kn==='K2'){ const ft=(await env.DB.prepare(`SELECT t.id,t.ten,t.media_url,t.phan_tich,t.created_at,sp.dong,sp.quy_trinh FROM tai_san t LEFT JOIN muc_noi_dung mu ON mu.id=t.muc_id LEFT JOIN san_pham sp ON sp.id=mu.san_pham_id WHERE t.phan_tich LIKE '%"timeline"%'`).all()).results.map(r=>({...r, nguon:'FOOTAGE'}));
    const qtDong={}; for(const r of (await env.DB.prepare(`SELECT dong, quy_trinh FROM san_pham WHERE quy_trinh IS NOT NULL AND quy_trinh<>''`).all()).results) if(r.dong&&!qtDong[r.dong]) qtDong[r.dong]=r.quy_trinh;
    const th=kn==='K1'?(await env.DB.prepare(`SELECT id,ten,link,phan_tich,created_at,dong FROM kho_thanh_pham WHERE phan_tich LIKE '%"timeline"%'`).all()).results.map(r=>({...r, nguon:'THANH_PHAM'})):[];
    const ng=((await docCauHinh(env)).huan_luyen||{}).source_nguong||{};
    for(const r of [...ft, ...th]){ const pt=docJSON(r.phan_tich,{})||{}; (pt.timeline||[]).forEach((d,i)=>{ let x;
      if(kn==='K1'){ const mo=nh(d.mo)||(!d.thay&&!d.nguoi?nh(d):null), th2=nh(d.thay); const t=d.khong_ro?'KHONG_RO':d.nguoi?'VANG':(th2&&mo)?(cung(th2,mo)?'BAC':'LECH'):th2?'THAY':'MO';
        x={ k:r.nguon+':'+r.id+':'+i, kn, nguon:r.nguon, id:r.id, i, ten:r.ten, dong:r.dong||null, luc:r.created_at, tu:d.tu, den:d.den, khung_url:d.khung_url||(Array.isArray(pt.khung)&&pt.khung[0])||((pt.shots||[]).find(s=>s.khung_url&&(d.tu+d.den)/2>=s.t0&&(d.tu+d.den)/2<s.t1)||{}).khung_url||null, media_url:r.nguon==='FOOTAGE'?r.media_url:null, link:r.link||null,
          quy_trinh:dsDongSP(r.quy_trinh||qtDong[r.dong]), tt:t, mo, thay:th2?{...th2, chac:d.thay.chac, ly_do:d.thay.ly_do||null, model:d.thay.model||null}:null, nguoi:d.nguoi?{...nh(d), mo_ta:d.mo_ta_nguoi||null, ai:d.nguoi, luc:d.nguoi_luc||null}:null, phan:d.phan||null, mo_ta:d.mo_ta_nguoi||d.mo_ta||'' }; }
      else { if(r.nguon!=='FOOTAGE') return; const sd=soDoSource(pt,d); if(!sd) return; const may=luatSource(ng,sd); x={ k:'S:'+r.id+':'+i, kn, nguon:r.nguon, id:r.id, i, ten:r.ten, dong:r.dong||null, luc:r.created_at, tu:d.tu, den:d.den, khung_url:d.khung_url||null, media_url:r.media_url, so_do:sd, may, nguoi:d.source_nguoi||null, tt:d.source_nguoi?'VANG':'LUAT', mo_ta:d.mo_ta||'' }; }
      rows.push(x); }); } }
  else { const ds=(await env.DB.prepare(`SELECT m.id, m.dau_vao, m.dau_ra, m.nhan, m.dong, m.created_at, m.cham_at, m.cham_boi, k.ten, k.link FROM mau_hoc_ai m LEFT JOIN kho_thanh_pham k ON k.id=m.doi_tuong_id WHERE m.tinh_nang='doc_loi'`).all()).results;
    for(const x of ds){ const v=docJSON(x.dau_vao,{})||{}, t=(docJSON(x.dau_ra,{})||{}).thay||null, n0=docJSON(x.nhan,{})||{}; const nguoi=n0.nguon==='NGUOI'; const hinh=nguoi?(n0.hinh||null):nh(n0);
      const tt2=nguoi?'VANG':!t?'CHO_THAY':t.nghe_sai?'NGHE_SAI':(hinh&&hinh.nhom===t.nhom)?'BAC':'LECH';
      rows.push({ k:'L:'+x.id, kn, nguon:'THANH_PHAM', id:x.id, ten:x.ten||'', dong:x.dong||null, luc:x.created_at, text:v.text||'', link:x.link||null, tt:tt2, hinh, thay:t, nguoi:nguoi?{ ...nh(n0), nghe_sai:!!n0.nghe_sai, ai:x.cham_boi, luc:x.cham_at }:null, mo_ta:'' }); } }
  const dem={}; for(const r of rows) dem[r.tt]=(dem[r.tt]||0)+1;
  let loc=rows.filter(r=>(!tt||r.tt===tt)&&(!dong||(dong==='(chưa có)'?!r.dong:r.dong===dong))&&(!nguon||r.nguon===nguon)&&(!tim||[r.ten,r.mo_ta,r.text,r.nguoi&&r.nguoi.mo_ta,r.thay&&r.thay.ly_do].some(v=>String(v||'').toLowerCase().includes(tim))));
  loc.sort((a,b)=>String(b.luc||'').localeCompare(String(a.luc||''))||String(a.ten).localeCompare(String(b.ten))||(so(a.tu)-so(b.tu)));
  const dongs=[...new Set(rows.map(r=>r.dong||'(chưa có)'))];
  return { kn, dem, tong:rows.length, loc:loc.length, trang, so_trang:Math.max(1,Math.ceil(loc.length/n)), hang:loc.slice((trang-1)*n, trang*n), dongs }; }
async function dsDongSanPham(env){ const m={}; const cong=(d,k,v=1)=>{ const key=d||'(chưa có)'; m[key]=m[key]||{dong:key, san_pham:0, video:0, footage:0, mau:0}; m[key][k]+=v; };
  ...
  for(const r of (await env.DB.prepare(`SELECT dong, COUNT(*) n FROM mau_hoc_ai GROUP BY dong`).all()).results) cong(r.dong,'mau',r.n);
...
async function banHuanLuyen(env){ const cfg=await docCauHinh(env); const hl=cfg.huan_luyen||{}; const ai=cfg.ai||{};
  ...
  { const ftM=ft.map(r=>docJSON(r.phan_tich,{})||{}); let nDoan=0; for(const pt of ftM) for(const d of (pt.timeline||[])) if(soDoSource(pt,d)) nDoan++;
    const sv=(await env.DB.prepare(`SELECT dau_vao, nhan FROM mau_hoc_ai WHERE tinh_nang='chat_luong_source'`).all()).results.map(x=>({v:docJSON(x.dau_vao,{})||{}, n:docJSON(x.nhan,{})||{}})); const hocS=hl.source_hoc||null; const ng=hl.source_nguong||{};
  ...
  { const lv=(await env.DB.prepare(`SELECT dau_ra, nhan, dong FROM mau_hoc_ai WHERE tinh_nang='doc_loi'`).all()).results.map(x=>({t:(docJSON(x.dau_ra,{})||{}).thay||null, n:docJSON(x.nhan,{})||{}, dong:x.dong}));
...
async function bootstrap(env, u){
  ...
  return {
    ...
    ai_nao: xem ? await (async()=>{ const th=thangHienTai(); return {
      ...
      do_chinh_xac_hinh: await (async()=>{ const ds=(await all(`SELECT dau_vao, dau_ra, nhan, dong, created_at FROM mau_hoc_ai WHERE tinh_nang='nhan_khung'`)).map(x=>({v:docJSON(x.dau_vao,{})||{}, c:docJSON(x.dau_ra,{})||{}, n:docJSON(x.nhan,{})||{}, dong:x.dong, luc:x.created_at}));
        const dN=x=>x.c.nhom===x.n.nhom, dC=x=>dN(x)&&(x.c.buoc||null)===(x.n.buoc||null)&&(x.c.bai_test||null)===(x.n.bai_test||null); const dem=(a)=>({ so:a.length, dung_nhom:a.filter(dN).length, dung_chi_tiet:a.filter(dC).length });
        const theo_nhom={}; for(const x of ds){ const k=x.n.nhom||'KHAC'; (theo_nhom[k]=theo_nhom[k]||{so:0,dung:0}).so++; if(dN(x)) theo_nhom[k].dung++; }
        const nh={}; for(const x of ds) if(!dN(x)){ const k=(x.c.nhom||'?')+'>'+(x.n.nhom||'?'); nh[k]=(nh[k]||0)+1; } const nham=Object.entries(nh).sort((a,b)=>b[1]-a[1]).slice(0,8).map(([k,so])=>({may:k.split('>')[0], nguoi:k.split('>')[1], so}));
        const coTin=ds.filter(x=>x.v.tu_tin_may!=null); const muc_tin=[['≥ 0,9',x=>x>=0.9],['0,6–0,9',x=>x>=0.6&&x<0.9],['< 0,6',x=>x<0.6]].map(([ten,fn])=>({ten, ...dem(coTin.filter(x=>fn(+x.v.tu_tin_may)))}));
        const demBen=(ben)=>{ const a=ds.filter(x=>x.v[ben]&&x.v[ben].nhom); return { so:a.length, dung_nhom:a.filter(x=>x.v[ben].nhom===x.n.nhom).length }; };
        return { mo:demBen('mo'), thay:demBen('thay'), so_nhan:ds.length, dung_nhom:ds.filter(dN).length, dung_chi_tiet:ds.filter(dC).length, ngau_nhien:dem(ds.filter(x=>x.v.ngau_nhien)), can_xac_nhan:dem(ds.filter(x=>x.v.can_xac_nhan_may)), theo_nguon:{ FOOTAGE:dem(ds.filter(x=>x.v.nguon!=='THANH_PHAM')), THANH_PHAM:dem(ds.filter(x=>x.v.nguon==='THANH_PHAM')) }, theo_nhom, nham, muc_tin }; })(),
...
async function handleApi(request, env){
  ...
  if(path.startsWith('/hub/')){
    ...
    if(path==='/hub/viec/doc_khung' && method==='GET'){ const mid=chuoi(url.searchParams.get('muc_id'),40), tid=chuoi(url.searchParams.get('tai_san_id'),40), lai=url.searchParams.get('lai')==='1'; const goc=env.APP_BASE_URL||url.origin;
      ...
      const suaNguoi=(await env.DB.prepare(`SELECT dong, dau_vao, nhan FROM mau_hoc_ai WHERE tinh_nang='nhan_khung' ORDER BY created_at DESC LIMIT 60`).all()).results.map(x=>({dong:x.dong, dv:docJSON(x.dau_vao,{}), nh:docJSON(x.nhan,{})}));
      const viDu=(dong)=>[...suaNguoi.filter(x=>dong&&x.dong===dong), ...suaNguoi.filter(x=>!dong||x.dong!==dong)].slice(0,10).map(x=>'Cảnh "'+String(x.nh.mo_ta||x.dv.mo_ta||'').slice(0,80)+'" là '+x.nh.nhom+(x.nh.buoc?(' / '+x.nh.buoc):'')+(x.nh.bai_test?(' / '+x.nh.bai_test):'')+((x.dv.cu&&(x.dv.cu.nhom!==x.nh.nhom||x.dv.cu.buoc!==x.nh.buoc))?(' (máy từng đọc nhầm là '+x.dv.cu.nhom+(x.dv.cu.buoc?(' / '+x.dv.cu.buoc):'')+')'):''));
      const dsDong=(v)=>String(v||'').split(/\r?\n/).map(x=>x.trim()).filter(Boolean).slice(0,20);
      return json({ viec: ds.filter(t=>lai||!docJSON(t.phan_tich,{}).timeline).map(t=>({ id:t.id, ten:t.ten, media_url:/^https?:/.test(t.media_url)?t.media_url:goc+t.media_url, dai:so(docJSON(t.phan_tich,{}).dai), chu_de:t.tieu_de||'', san_pham:t.sp_ten||'', dong:t.dong||'', quy_trinh:dsDong(t.quy_trinh), bai_test:dsDong(t.bai_test), vi_du:viDu(t.dong) })) }); }
    ...
    if(path==='/hub/thay-doc' && method==='POST'){ const cfg=(await docCauHinh(env)).ai||{}; if(cfg.thay_nhin===false) return json({ok:false, tat:true, loi:'Thầy (Claude) đang tắt ở Cấu hình'});
      const ds=(Array.isArray(body.doan)?body.doan:[]).slice(0,30); const kq=new Array(ds.length); let k=0;
      const dongLo=chuoi(body.dong||body.san_pham,80); const suaNg=(await env.DB.prepare(`SELECT dong, dau_vao, nhan FROM mau_hoc_ai WHERE tinh_nang='nhan_khung' ORDER BY created_at DESC LIMIT 80`).all()).results.map(x=>({dong:x.dong, dv:docJSON(x.dau_vao,{})||{}, nh:docJSON(x.nhan,{})||{}})).filter(x=>x.nh.nhom);
      const viDuLo=[...suaNg.filter(x=>dongLo&&x.dong&&dongLo.toLowerCase().includes(String(x.dong).toLowerCase())), ...suaNg.filter(x=>!(dongLo&&x.dong&&dongLo.toLowerCase().includes(String(x.dong).toLowerCase())))].slice(0,12).map(x=>'Cảnh "'+String(x.nh.mo_ta||x.dv.mo_ta||'').slice(0,90)+'" là '+x.nh.nhom+(x.nh.buoc?(' / '+x.nh.buoc):'')+(x.nh.bai_test?(' / '+x.nh.bai_test):'')+((x.dv.cu&&x.dv.cu.nhom&&(x.dv.cu.nhom!==x.nh.nhom||(x.dv.cu.buoc||null)!==(x.nh.buoc||null)))?(' (máy từng đọc nhầm là '+x.dv.cu.nhom+(x.dv.cu.buoc?(' / '+x.dv.cu.buoc):'')+')'):''));
      const chay=async()=>{ while(k<ds.length){ const i=k++; kq[i]=await thayDocDoan(env,{ anh:ds[i].anh, quy_trinh:body.quy_trinh, bai_test:body.bai_test, san_pham:body.san_pham, ngu_canh:body.ngu_canh, vi_du:viDuLo }).catch(e=>({ok:false, loi:String(e.message||e).slice(0,100)})); if(kq[i].vuot_ngan_sach){ for(let q=k;q<ds.length;q++) kq[q]={ok:false, loi:kq[i].loi, vuot_ngan_sach:true}; k=ds.length; } } };
      await Promise.all([chay(),chay(),chay(),chay()]); return json({ ok:true, kq }); }
    if(path==='/hub/doc-khung' && method==='POST'){ const t=await env.DB.prepare(`SELECT id,phan_tich FROM tai_san WHERE id=?`).bind(chuoi(body.tai_san_id,40)).first(); if(!t) return json({error:'Không có tài sản'},404);
      const tl=tlSach(body.timeline);
      if(!tl.length) return json({error:'Dòng thời gian rỗng'},400); const pt=docJSON(t.phan_tich,{})||{}; pt.timeline=tl; pt.vl={ model:chuoi(body.model,60), so_khung:so(body.so_khung), ms_khung:so(body.ms_khung), luc:nowISO(), may:chuoi(body.may,80), tong_quan:chuoi(body.tong_quan,200)||null };
      await env.DB.prepare(`UPDATE tai_san SET phan_tich=? WHERE id=?`).bind(JSON.stringify(pt), t.id).run(); return json({ ok:true, so_doan:tl.length }); }
    if(path==='/hub/phan-tich' && method==='POST'){ const t=await env.DB.prepare(`SELECT * FROM tai_san WHERE id=?`).bind(chuoi(body.tai_san_id,40)).first(); if(!t) return json({error:'Không có tài sản'},404); const khungDs=(Array.isArray(body.khung_urls)?body.khung_urls:[]).map(x=>chuoi(x,500)).filter(Boolean).slice(0,3);
      ...
      const pt=phanTichSach(body.phan_tich, khungDs.length?khungDs:(cu.khung||[]), coCanh); if(pt&&cu.timeline){ pt.timeline=cu.timeline; pt.vl=cu.vl; } await env.DB.prepare(`UPDATE tai_san SET phan_tich=?, mo_ta=? WHERE id=?`).bind(JSON.stringify(pt), moTa, t.id).run(); return json({ ok:true, co_canh:pt.co_canh }); }
    ...
    if(path==='/hub/thanh-pham' && method==='POST'){ const ten=chuoi(body.ten,200); const nid=chuoi(body.nguon_id,120)||ten; if(!ten) return json({error:'Thiếu tên'},400); const daCo=await env.DB.prepare(`SELECT id FROM kho_thanh_pham WHERE nguon_id=?`).bind(nid).first(); if(daCo&&!body.lam_lai) return json({ok:true, trung:true});
  ...
  if((m=path.match(/^\/kho-thanh-pham\/([^/]+)$/)) && (method==='PATCH'||method==='DELETE')){ if(!canGat(me)) return json({error:'Chỉ Trưởng MKT/Admin'},403); const tp=await env.DB.prepare(`SELECT id,ten FROM kho_thanh_pham WHERE id=?`).bind(m[1]).first(); if(!tp) return json({error:'Không có video này trong kho'},404);
    if(method==='DELETE'){ await env.DB.prepare(`DELETE FROM mau_hoc_ai WHERE doi_tuong='kho_thanh_pham' AND doi_tuong_id=?`).bind(tp.id).run(); await env.DB.prepare(`DELETE FROM kho_thanh_pham WHERE id=?`).bind(tp.id).run(); await logAudit(env,me,'loại video khỏi kho thành phẩm','kho_thanh_pham',tp.id,tp.ten); return json({ db: await bootstrap(env,me) }); }
  ...
  if((m=path.match(/^\/(tai-san|kho-thanh-pham)\/([^/]+)\/doan\/(\d+)$/)) && method==='POST'){ if(!isStaff(me)) return json({error:'Không có quyền'},403);
    const tp=m[1]==='kho-thanh-pham'; const bang=tp?'kho_thanh_pham':'tai_san';
    const t=tp ? await env.DB.prepare(`SELECT id,ten,phan_tich,dong FROM kho_thanh_pham WHERE id=?`).bind(m[2]).first()
               : await env.DB.prepare(`SELECT t.id,t.ten,t.phan_tich,sp.dong FROM tai_san t LEFT JOIN muc_noi_dung mu ON mu.id=t.muc_id LEFT JOIN san_pham sp ON sp.id=mu.san_pham_id WHERE t.id=?`).bind(m[2]).first();
    if(!t) return json({error:tp?'Không có video thành phẩm':'Không có footage'},404);
    const pt=docJSON(t.phan_tich,{})||{}; const tl=Array.isArray(pt.timeline)?pt.timeline:[]; const i=so(m[3]); const d=tl[i]; if(!d) return json({error:'Không có đoạn '+i},404);
    if(body.khong_ro){ tl[i]={ ...d, khong_ro:true, can_xac_nhan:false, nguoi:me.ho_ten, nguoi_luc:nowISO() }; pt.timeline=tl; await env.DB.prepare(`UPDATE ${bang} SET phan_tich=? WHERE id=?`).bind(JSON.stringify(pt), t.id).run(); return json({ ok:true, bo_qua:true }); }   // hình không đủ rõ để gán — không thành mẫu
    const nhom=NHOM_CANH.includes(body.nhom)?body.nhom:d.nhom; const moi={ nhom, buoc:nhom==='THI_CONG'?(chuoi(body.buoc,80)||(d.nhom==='THI_CONG'?d.buoc:null)||null):null, bai_test:nhom==='THU_NGHIEM'?(chuoi(body.bai_test,80)||(d.nhom==='THU_NGHIEM'?d.bai_test:null)||null):null };
    if(body.tham_my!=null&&body.tham_my!=='') moi.tham_my=Math.max(0,Math.min(10,so(body.tham_my)));
    const nhanGon2=(p)=>{ const n=NHOM_CANH.includes(p&&p.nhom)?p.nhom:null; if(!n) return null; return { nhom:n, buoc:n==='THI_CONG'?(chuoi(p.buoc,80)||null):null, bai_test:n==='THU_NGHIEM'?(chuoi(p.bai_test,80)||null):null, mo_ta:chuoi(p&&p.mo_ta,200)||null }; };
    // (25/09) một dải 3 khung có nhiều nội dung → người gán TỪNG khung; nhãn cả đoạn = nhóm xuất hiện nhiều nhất
    const phan=Array.isArray(body.phan)?body.phan.slice(0,3).map(nhanGon2):null;
    if(phan&&phan.filter(Boolean).length){ const dem={}; phan.filter(Boolean).forEach(p=>{ dem[p.nhom]=(dem[p.nhom]||0)+1; }); const chinh=phan.find(p=>p&&dem[p.nhom]===Math.max(...Object.values(dem))); Object.assign(moi,{ nhom:chinh.nhom, buoc:chinh.buoc, bai_test:chinh.bai_test }); }
    const moTaNguoi=chuoi(body.mo_ta,200)||null; if(moTaNguoi) moi.mo_ta=moTaNguoi;
    // bước / bài test mới → thêm vào quy trình chuẩn của dòng (khi người chọn)
    let themVao=null; if(body.them_buoc&&t.dong&&(moi.buoc||moi.bai_test)){ const cot=moi.buoc?'quy_trinh':'bai_test'; const gt=moi.buoc||moi.bai_test; const sp=await env.DB.prepare(`SELECT id, ${cot} v FROM san_pham WHERE dong=? ORDER BY id LIMIT 1`).bind(t.dong).first();
      if(sp){ const ds=dsDongSP(sp.v); if(!ds.some(x=>x.toLowerCase()===gt.toLowerCase())){ await env.DB.prepare(`UPDATE san_pham SET ${cot}=? WHERE id=?`).bind([...ds, gt].join('\n'), sp.id).run(); await logAudit(env,me,'thêm '+(cot==='quy_trinh'?'bước thi công':'bài test'),'san_pham',sp.id,t.dong+': '+gt); } themVao={ dong:t.dong, cot, gia_tri:gt }; } else themVao={ loi:'Dòng '+t.dong+' chưa có sản phẩm trong Danh mục' }; }
    const cu={ nhom:d.nhom, buoc:d.buoc||null, bai_test:d.bai_test||null, tham_my:d.tham_my };
    const daGan=!!d.nguoi;   // gán lại đoạn đã có nhãn người: sửa nhãn, không đếm là lần đo máy
    tl[i]={ ...d, ...moi, mo_ta_nguoi:moTaNguoi||d.mo_ta_nguoi||null, phan:phan&&phan.filter(Boolean).length?phan.map((p,k)=>p?{...p, k}:null):(d.phan||null), tu_tin:1, can_xac_nhan:false, khong_ro:false, nguoi:me.ho_ten, nguoi_luc:nowISO(), may:d.may||(daGan?null:cu) }; pt.timeline=tl; await env.DB.prepare(`UPDATE ${bang} SET phan_tich=? WHERE id=?`).bind(JSON.stringify(pt), t.id).run();
    ...
    if(phan&&phan.filter(Boolean).length>1) for(const [k,p] of phan.entries()){ if(!p) continue; await ghiMauAI(env,{tinh_nang:'nhan_khung', doi_tuong:bang, doi_tuong_id:t.id, dau_vao:{i, k, tu:d.tu, den:d.den, mo_ta:d.mo_ta, cu:d.may||cu, khung_url:d.khung_url||null, khung_k:k, nguon:tp?'THANH_PHAM':'FOOTAGE', phan:true, giay:Math.max(0,Math.min(120,so(body.giay)))||null}, dau_ra:d.may||cu, nhan:{nhom:p.nhom, buoc:p.buoc, bai_test:p.bai_test, mo_ta:p.mo_ta}, pham_vi:{dong:t.dong||null, muc_dich:null}}); }
    else await ghiMauAI(env,{tinh_nang:'nhan_khung', doi_tuong:bang, doi_tuong_id:t.id, dau_vao:{i, tu:d.tu, den:d.den, mo_ta:d.mo_ta, hanh_dong:d.hanh_dong, cu:cuMay, tu_tin_may:d.may?null:d.tu_tin, can_xac_nhan_may:!!d.can_xac_nhan, ngau_nhien:!!body.ngau_nhien, giay:Math.max(0,Math.min(120,so(body.giay)))||null, khung_url:d.khung_url||null, nguon:tp?'THANH_PHAM':'FOOTAGE', mo:d.mo||null, thay:d.thay||null, nguon_nhan:d.nguon_nhan||null}, dau_ra:cuMay, nhan:moi, pham_vi:{dong:t.dong||null, muc_dich:null}});
    ...
    if(body.nhe) return json({ ok:true, dung, them_vao:themVao });   // màn gán nhãn nhanh: không tải lại cả db mỗi phím
  ...
  if(path==='/dong-san-pham/doi-ten' && method==='POST'){ if(!canGat(me)) return json({error:'Chỉ Trưởng MKT/Admin'},403); const tu=chuoi(body.tu,80), sang=chuoi(body.sang,80); if(!tu||!sang||tu===sang) return json({error:'Cần tên cũ và tên mới khác nhau'},400);
    const n={}; for(const [bang,cot] of [['san_pham','dong'],['kho_thanh_pham','dong'],['mau_hoc_ai','dong']]){ const r=await env.DB.prepare(`UPDATE ${bang} SET ${cot}=? WHERE ${cot}=?`).bind(sang,tu).run(); n[bang]=(r.meta&&r.meta.changes)||0; }
  ...
  if(path==='/hop-viec/dem' && method==='GET'){ if(!isStaff(me)) return json({error:'Không có quyền'},403);
    ...
    for(const r of ft){ const pt=docJSON(r.phan_tich,{})||{}; for(const d of (pt.timeline||[])){ if(!d.nguoi&&!d.khong_ro&&d.thay&&d.can_xac_nhan){ k1++; if(!d.kiem_ngau_nhien) k1_can++; } if(!d.source_nguoi&&soDoSource(pt,d)) k2++; } }
    for(const r of th){ const pt=docJSON(r.phan_tich,{})||{}; for(const d of (pt.timeline||[])) if(!d.nguoi&&!d.khong_ro&&d.thay&&d.can_xac_nhan){ k1++; if(!d.kiem_ngau_nhien) k1_can++; } }
  ...
  if(path==='/source/hang' && method==='GET'){ if(!isStaff(me)) return json({error:'Không có quyền'},403); const hl=(await docCauHinh(env)).huan_luyen||{}; const ng=hl.source_nguong||{}; const tr=await hetTran(env,url);
    ...
    const bo=new Set(String(url.searchParams.get('bo')||'').split(',').filter(Boolean)); const ds=[];
    for(const r of ft){ const pt=docJSON(r.phan_tich,{})||{}; (pt.timeline||[]).forEach((d,i)=>{ if(d.source_nguoi) return; const k='S:'+r.id+':'+i; if(bo.has(k)) return; const sd=soDoSource(pt,d); if(!sd) return; ds.push({ k, id:r.id, i, ten:r.ten, dong:r.dong||null, tu:d.tu, den:d.den, nhom:d.nhom||null, mo_ta:d.mo_ta||'', khung_url:d.khung_url||(Array.isArray(pt.khung)&&pt.khung[0])||null, media_url:r.media_url, so_do:sd, may:luatSource(ng,sd), bien:bienSource(ng,sd) }); }); }
    const gan=ds.slice().sort((a,b)=>a.bien-b.bien); const nn=ds.slice(); for(let j=nn.length-1;j>0;j--){ const q=Math.floor(Math.random()*(j+1)); [nn[j],nn[q]]=[nn[q],nn[j]]; }
    const ra=[], da=new Set(); while(ra.length<30&&(gan.length||nn.length)){ const lay=(ra.length%4===3&&nn.length)?{...nn.shift(), ngau_nhien:true}:gan.shift()||{...nn.shift(), ngau_nhien:true}; if(lay&&!da.has(lay.k)){ da.add(lay.k); ra.push(lay); } }
    return json({ hang:tr.het?[]:ra, het_tran:tr.het, phut_hom_nay:tr.phut, tran_phut:tr.tran, con_lai:ds.length, nguong:ng, hoc:hl.source_hoc||null }); }
  if((m=path.match(/^\/tai-san\/([^/]+)\/doan\/(\d+)\/source$/)) && method==='POST'){ if(!isStaff(me)) return json({error:'Không có quyền'},403); const t=await env.DB.prepare(`SELECT t.id,t.phan_tich,sp.dong FROM tai_san t LEFT JOIN muc_noi_dung mu ON mu.id=t.muc_id LEFT JOIN san_pham sp ON sp.id=mu.san_pham_id WHERE t.id=?`).bind(m[1]).first(); if(!t) return json({error:'Không có footage'},404);
    const pt=docJSON(t.phan_tich,{})||{}; const tl=Array.isArray(pt.timeline)?pt.timeline:[]; const i=so(m[2]); const d=tl[i]; if(!d) return json({error:'Không có đoạn '+i},404); const sd=soDoSource(pt,d); const ng=((await docCauHinh(env)).huan_luyen||{}).source_nguong||{}; const may=luatSource(ng,sd);
    const LY=['mờ','rung','tối','cháy sáng','che khuất','bố cục xấu','khác']; const nhan={ dung:!!body.dung, ly_do:(Array.isArray(body.ly_do)?body.ly_do:[]).filter(x=>LY.includes(x)).slice(0,4) };
    tl[i]={ ...d, source_nguoi:{...nhan, nguoi:me.ho_ten, luc:nowISO()} }; pt.timeline=tl; await env.DB.prepare(`UPDATE tai_san SET phan_tich=? WHERE id=?`).bind(JSON.stringify(pt), t.id).run();
    await env.DB.prepare(`DELETE FROM mau_hoc_ai WHERE tinh_nang='chat_luong_source' AND doi_tuong_id=? AND dau_vao LIKE ?`).bind(t.id, '{"i":'+i+',%').run();
    await ghiMauAI(env,{tinh_nang:'chat_luong_source', doi_tuong:'tai_san', doi_tuong_id:t.id, dau_vao:{i, so_do:sd, may, ngau_nhien:!!body.ngau_nhien, giay:Math.max(0,Math.min(120,so(body.giay)))||null, khung_url:d.khung_url||null}, dau_ra:may, nhan, pham_vi:{dong:t.dong||null, muc_dich:null}});
    const n=so((await env.DB.prepare(`SELECT COUNT(*) n FROM mau_hoc_ai WHERE tinh_nang='chat_luong_source'`).first()||{}).n); let hoc=null; if(n>=30&&n%10===0) hoc=await hocNguongSource(env);
    return json({ ok:true, dung:!!may&&may.dung===nhan.dung, hoc }); }
  if(path==='/source/hoc' && method==='POST'){ if(!laGat(me)) return json({error:'Chỉ Trưởng MKT/Admin'},403); return json(await hocNguongSource(env)); }
  ...
  if(path==='/doc-loi/hang' && method==='GET'){ if(!isStaff(me)) return json({error:'Không có quyền'},403); const tr=await hetTran(env,url); const locDong=chuoi(url.searchParams.get('dong'),80); const bo=new Set(String(url.searchParams.get('bo')||'').split(',').filter(Boolean));
    const ds=(await env.DB.prepare(`SELECT m.id, m.dau_vao, m.dau_ra, m.nhan, m.dong, m.doi_tuong_id, k.ten, k.link, k.phan_tich FROM mau_hoc_ai m LEFT JOIN kho_thanh_pham k ON k.id=m.doi_tuong_id WHERE m.tinh_nang='doc_loi' AND m.dau_ra LIKE '%"thay"%' AND (m.nhan IS NULL OR m.nhan NOT LIKE '%"NGUOI"%') ORDER BY m.created_at DESC LIMIT 400`).all()).results;
    const sp=(await env.DB.prepare(`SELECT dong, quy_trinh, bai_test FROM san_pham`).all()).results;
    const tt=ds.filter(x=>(!locDong||x.dong===locDong)&&!bo.has('L:'+x.id)).map(x=>{ const v=docJSON(x.dau_vao,{})||{}, t=(docJSON(x.dau_ra,{})||{}).thay||{}, n=docJSON(x.nhan,{})||{}; const pt=docJSON(x.phan_tich,{})||{}; const sh=(pt.shots||[]).find(y=>y.loi&&String(y.loi).trim()===String(v.text||'').trim()); const s0=sp.find(y=>x.dong&&y.dong===x.dong&&(y.quy_trinh||y.bai_test))||{};
      const hinh=n.nhom?{nhom:n.nhom, buoc:n.buoc||null, bai_test:n.bai_test||null}:null; const lech=!!(hinh&&hinh.nhom!==t.nhom);
      return { k:'L:'+x.id, id:x.id, text:v.text||'', ten:x.ten||'', link:x.link||null, dong:x.dong||null, khung_url:sh&&sh.khung_url||null, thay:t, hinh, lech, uu:(t.nghe_sai?10:lech?1:(so(t.chac)<0.7?2:9)), quy_trinh:dsDongSP(s0.quy_trinh), bai_test_ds:dsDongSP(s0.bai_test) }; });
    const kho=tt.filter(x=>x.uu<9).sort((a,b)=>a.uu-b.uu||so(a.thay.chac)-so(b.thay.chac)), de=tt.filter(x=>x.uu===9), sai=tt.filter(x=>x.uu===10);   // 24/09: 39/40 câu bản nghe cũ bị thầy báo nghe sai — không bắt người bấm lại
    for(let j=sai.length-1;j>0;j--){ const q=Math.floor(Math.random()*(j+1)); [sai[j],sai[q]]=[sai[q],sai[j]]; } for(let j=de.length-1;j>0;j--){ const q=Math.floor(Math.random()*(j+1)); [de[j],de[q]]=[de[q],de[j]]; }
    const ra=[]; while(ra.length<30&&(kho.length||de.length||(sai.length&&ra.length%8===7))){ if(sai.length&&ra.length%8===7) ra.push({...sai.shift(), ngau_nhien:true}); else if(de.length&&(ra.length%4===3||!kho.length)) ra.push({...de.shift(), ngau_nhien:true}); else if(kho.length) ra.push(kho.shift()); else break; }
    return json({ hang:tr.het?[]:ra, het_tran:tr.het, phut_hom_nay:tr.phut, tran_phut:tr.tran, con_lai:tt.length-sai.length, thay_nghe_sai:sai.length, cho_thay:so((await env.DB.prepare(`SELECT COUNT(*) n FROM mau_hoc_ai WHERE tinh_nang='doc_loi' AND (dau_ra IS NULL OR dau_ra NOT LIKE '%"thay"%')`).first()||{}).n) }); }
  if((m=path.match(/^\/mau\/([^/]+)\/doc-loi$/)) && method==='POST'){ if(!isStaff(me)) return json({error:'Không có quyền'},403); const x=await env.DB.prepare(`SELECT id, dau_ra, nhan FROM mau_hoc_ai WHERE id=? AND tinh_nang='doc_loi'`).bind(m[1]).first(); if(!x) return json({error:'Không có câu'},404);
    const t=(docJSON(x.dau_ra,{})||{}).thay||{}; const cu=docJSON(x.nhan,{})||{}; const giay=Math.max(0,Math.min(120,so(body.giay)))||null;
    const nhan=body.nghe_sai?{ nghe_sai:true, nguon:'NGUOI', hinh:cu.nguon==='NGUOI'?cu.hinh:{nhom:cu.nhom||null, buoc:cu.buoc||null}, giay, ngau_nhien:!!body.ngau_nhien }
      :{ nhom:NHOM_CANH.includes(body.nhom)?body.nhom:t.nhom, buoc:body.nhom==='THI_CONG'?(chuoi(body.buoc,80)||null):null, bai_test:body.nhom==='THU_NGHIEM'?(chuoi(body.bai_test,80)||null):null, nguon:'NGUOI', hinh:cu.nguon==='NGUOI'?cu.hinh:{nhom:cu.nhom||null, buoc:cu.buoc||null}, giay, ngau_nhien:!!body.ngau_nhien };
    await env.DB.prepare(`UPDATE mau_hoc_ai SET nhan=?, cham_at=?, cham_boi=? WHERE id=?`).bind(JSON.stringify(nhan), nowISO(), me.ho_ten, x.id).run();
    return json({ ok:true, dung:!body.nghe_sai&&t.nhom===nhan.nhom }); }
  ...
  if(path==='/nhan-hinh/hang' && method==='GET'){ if(!isStaff(me)) return json({error:'Không có quyền'},403);
    ...
    const sp=(await env.DB.prepare(`SELECT ten,dong,quy_trinh,bai_test FROM san_pham`).all()).results;
    const spDong=(dong)=>sp.find(x=>dong&&x.dong===dong&&(x.quy_trinh||x.bai_test))||null;
    const ft=(await env.DB.prepare(`SELECT t.id,t.ten,t.media_url,t.phan_tich,sp.dong,sp.quy_trinh,sp.bai_test FROM tai_san t LEFT JOIN muc_noi_dung mu ON mu.id=t.muc_id LEFT JOIN san_pham sp ON sp.id=mu.san_pham_id WHERE t.phan_tich LIKE '%"timeline"%' ORDER BY t.created_at DESC LIMIT 300`).all()).results;
    const th=(await env.DB.prepare(`SELECT id,ten,link,dong,luot_xem,phan_tich FROM kho_thanh_pham WHERE phan_tich LIKE '%"timeline"%' ORDER BY created_at DESC LIMIT 300`).all()).results;
    const ds=[]; let tong=0, daGan=0;
    const them=(nguon,row,dong,qt,bt,anhCua)=>{ const pt=docJSON(row.phan_tich,{})||{}; (Array.isArray(pt.timeline)?pt.timeline:[]).forEach((d,i)=>{ tong++; if(d.nguoi){ daGan++; return; } if(!d.thay||!d.can_xac_nhan) return; if(locDong&&dong!==locDong) return; const k=nguon+':'+row.id+':'+i; if(bo.has(k)) return;   // thầy chủ đạo: chỉ đoạn thầy chưa chắc / mẫu kiểm ngẫu nhiên
      ds.push({ k, nguon, ngau_nhien:!!d.kiem_ngau_nhien, ly_do_hoi:d.kiem_ngau_nhien?'KIEM':'THAY_CHUA_CHAC', id:row.id, i, ten:row.ten, dong:dong||null, tu:d.tu, den:d.den, nhom:d.nhom, buoc:d.buoc||null, bai_test:d.bai_test||null, tham_my:d.tham_my, tu_tin:d.tu_tin, can_xac_nhan:!!d.can_xac_nhan, mo_ta:d.mo_ta||'', hanh_dong:d.hanh_dong||'', mo:d.mo||null, thay:d.thay||null, tl_video:(pt.timeline||[]).slice(0,80).map(y=>[y.tu,y.den,y.nhom||null,y.nguoi?1:0]), dai_video:so(pt.dai)||null, khung_url:d.khung_url||anhCua(d, pt)||null, media_url:nguon==='FOOTAGE'?row.media_url:null, link:row.link||null, quy_trinh:dsDongSP(qt), bai_test_ds:dsDongSP(bt) }); }); };
    for(const r of ft) them('FOOTAGE', r, r.dong, r.quy_trinh, r.bai_test, (d,pt)=>Array.isArray(pt.khung)&&pt.khung.length?pt.khung[Math.min(pt.khung.length-1, Math.floor(pt.khung.length*((d.tu+d.den)/2)/Math.max(1,so(pt.dai)||d.den)))]:null);
    for(const r of th){ const s0=spDong(r.dong); them('THANH_PHAM', r, r.dong, s0&&s0.quy_trinh, s0&&s0.bai_test, (d,pt)=>{ const g=(d.tu+d.den)/2; const sh=(pt.shots||[]).find(x=>g>=x.t0&&g<x.t1&&x.khung_url)||(pt.shots||[]).filter(x=>x.khung_url).sort((a,b)=>Math.abs((a.t0+a.t1)/2-g)-Math.abs((b.t0+b.t1)/2-g))[0]; return sh?sh.khung_url:null; }); }
    const coAnh=ds.filter(x=>x.khung_url||x.media_url);
...
export default {
  async scheduled(controller, env, ctx){ ctx.waitUntil((async()=>{ const e=await napKhoa(env); await dieuPhoi(e); await tuHoc(e).catch(()=>{}); await thayDocLoi(e, 40).catch(()=>{}); })().catch(()=>{})); },
