...
export default async function nap({ app, goiApp, lenh, dir, log }) {
  ...
  for (const f of chon) {
    try {
      ...
      if (la) {
        ...
        const d = thoiLuong(ra); spawnSync("ffmpeg", ["-hide_banner", "-loglevel", "error", "-y", "-ss", (d / 2).toFixed(2), "-i", ra, "-frames:v", "1", "-vf", "scale=512:-2", khung], { encoding: "utf8" });
      } else {
      ...
      const mediaUrl = await up(ra, la ? "video/mp4" : "image/jpeg"); const khungUrl = existsSync(khung) ? await up(khung, "image/jpeg") : "";
      let x; for (let k = 0; k < 3; k++) { x = await goiApp("/hub/tai-san", { method: "POST", body: JSON.stringify({ muc_id: mucId, ten: f.ten, thu_muc: f.thu_muc || "", media_url: mediaUrl, media_type: la ? "VIDEO" : "IMAGE", khung_url: khungUrl, nguon: "DRIVE", drive_id: f.id, giay: la ? +thoiLuong(ra).toFixed(1) : null }) }); if (x.ok || (x.status && x.status < 500)) break; await new Promise((q) => setTimeout(q, 4000 * (k + 1))); }
