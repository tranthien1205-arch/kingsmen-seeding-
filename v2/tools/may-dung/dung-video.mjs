...
export default async function dung({ app, goiApp, lenh, dir, log, may, script }) {
  ...
  let goiF = ""; try { goiF = join(dir, "dung", id + "-goi-capcut.zip"); rmSync(goiF, { force: true }); const z = spawnSync(process.platform === "win32" && existsSync("C:\\Windows\\System32\\tar.exe") ? "C:\\Windows\\System32\\tar.exe" : "tar", ["-a", "-c", "-f", id + "-goi-capcut.zip", "-C", TH, "canh", "giong", "phu-de.srt", "ban-nhap.mp4", "DOC-TOI.txt", ...(nhacF ? ["nhac.mp3"] : []), ...(ttsMo ? ["giong-mo"] : [])], { cwd: join(dir, "dung"), encoding: "utf8", timeout: 300000 }); if (z.status !== 0 || !existsSync(goiF)) { log("  không nén được gói:", String(z.stderr || z.error || "").slice(0, 120)); goiF = ""; } } catch (e) { goiF = ""; }
