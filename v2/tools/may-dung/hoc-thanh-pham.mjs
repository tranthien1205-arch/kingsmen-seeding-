  ...
  for (const v of video) {
    try {
      ...
      for (let i = 0; i < shots.length; i++) {
        ...
        if (i < 40) { spawnSync("ffmpeg", ["-hide_banner", "-loglevel", "error", "-y", "-ss", giua.toFixed(2), "-i", f, "-frames:v", "1", "-vf", "scale=512:-2", kf], { encoding: "utf8", timeout: 60000, windowsHide: true }); if (existsSync(kf)) s.khung_url = await up(kf, "image/jpeg"); }
