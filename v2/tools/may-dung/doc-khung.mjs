...
//   5. TỰ BIẾT KHÔNG CHẮC: tu_tin tự khai KHÔNG đáng tin (đo 24/09: luôn 1,0 kể cả khi sai) → can_xac_nhan khi đoạn ngắn < 2 dải, bị Viterbi sửa, hoặc tu_tin < 0,6 (người xác nhận ở màn Huấn luyện → nhãn vàng).
...
function loiNhac({ dsBuoc, dsTest, san_pham, chu_de, vi_du, tong_quan, ngu_canh, dai }) {
  return "Bạn là kỹ thuật viên thi công vật liệu xây dựng kiêm biên tập video. Ảnh là DẢI 3 khung liên tiếp (trái → phải, cách nhau 0,5 giây) trong clip quay cho video" + (san_pham ? " về \"" + san_pham + "\"" : "") + (chu_de ? " (chủ đề: " + chu_de + ")" : "") + ". Nhìn chuyển động giữa 3 khung để biết đang làm gì.\n" +
    (ngu_canh ? "Ngữ cảnh: " + String(ngu_canh).slice(0, 900) + "\n" : "") + (tong_quan ? "Tổng quan cả clip: " + tong_quan + "\n" : "") +
...
async function tongQuan(file, dai, thu, ctx) { const o = join(thu, "luoi.jpg"); const fps = Math.max(0.05, 6 / Math.max(1, dai));
  ...
  const x = await hoiVL(o, "Ảnh là lưới 6 khung rải đều của MỘT clip quay thi công vật liệu" + (ctx.san_pham ? " \"" + ctx.san_pham + "\"" : "") + ". " + (ctx.ngu_canh ? "Ngữ cảnh: " + String(ctx.ngu_canh).slice(0, 600) + ". " : "") + "Tóm tắt clip trong ≤ 25 chữ tiếng Việt: đang ở giai đoạn nào (bối cảnh, vấn đề, sản phẩm, thi công bước nào, bài test, hoàn thiện), làm gì.", { type: "object", required: ["tom_tat"], properties: { tom_tat: { type: "string" } } });
...
export async function docKhung(file, ctx = {}) {
  ...
  const p = spawnSync("ffmpeg", ["-hide_banner", "-loglevel", "error", "-i", file, "-vf", "fps=" + fps.toFixed(3) + ",scale=448:-2,tile=3x1", "-q:v", "4", join(thu, "d%03d.jpg")], { encoding: "utf8", timeout: 300000 });
  ...
  const nhan = viterbi(dai3, dsBuoc); dai3.forEach((d, i) => { d.anh = ds[i]; if (nhan[i] !== d.k) { const [n, b, t] = nhan[i].split("|"); d.nhom = n; d.buoc = b || null; d.bai_test = t || null; d.k = nhan[i]; d.tu_tin = Math.min(d.tu_tin, 0.5); d._sua = true; } });
  const doan = []; for (const g of dai3) { const cu = doan[doan.length - 1]; if (cu && cu.k === g.k && cu.co_canh === g.co_canh) { cu.den = +(g.t + cua).toFixed(2); cu._n++; cu._anh.push(g.anh); cu._sua = cu._sua || !!g._sua; cu._tm.push(g.tham_my); cu.ro_net += g.ro_net; cu.tu_tin += g.tu_tin; cu.co_nguoi = cu.co_nguoi || g.co_nguoi; }
