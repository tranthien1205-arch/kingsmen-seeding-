...
test('kỹ năng: 4 thẻ với trạng thái; tự học tạo lệnh huấn luyện khi đủ mẫu, không trùng; phiên bản mang ví dụ; bật cần điểm ≥ ngưỡng, tắt về API', async () => {
  ...
  const p70 = await gui(70); b = (await api('/bootstrap')).j.db.ai_nao; let k = b.ky_nang.find(x => x.id === 'chon_canh'); assert.equal(k.trang_thai, 'MOI'); assert.equal(k.ban_moi.diem, 70); assert.equal(k.ban_moi.vi_du.length, 1); assert.equal(k.ban_moi.vi_du[0].cu, ts[1]);
