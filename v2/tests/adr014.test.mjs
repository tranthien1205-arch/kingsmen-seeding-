...
test('014a: quy trình thi công theo sản phẩm → việc đọc khung; lưu dòng thời gian làm sạch; không mất khi phân tích lại; giao máy mạnh có mô hình', async () => {
  ...
  let pt = JSON.parse(DB.raw.prepare(`SELECT phan_tich FROM tai_san WHERE id=?`).get(ts).phan_tich); assert.equal(pt.timeline, undefined, 'ADR-018: nhãn không nằm trong phan_tich'); const md = () => DB.raw.prepare(`SELECT * FROM mau_doan WHERE doi_tuong_id=? ORDER BY i`).all(ts); const mo1 = JSON.parse(md()[1].nhan_mo); assert.equal(mo1.co_canh, null, 'cỡ cảnh lạ bị bỏ'); assert.equal(mo1.tham_my, 10, 'thẩm mỹ kẹp 0–10'); assert.equal(JSON.parse(md()[0].nhan_mo).buoc, 'Bơm keo vào ron'); assert.deepEqual(JSON.parse(md()[0].nhan_mo).dung_cu, [], 'vật liệu/dụng cụ tự do của mô hình mở không có trong bộ nhãn vẫn giữ ở vat_lieu'); assert.equal(md()[0].trang_thai, 'MO'); assert.equal(pt.vl.model, 'qwen2.5vl:7b'); assert.equal(pt.doan.length, 1);
  await manh('/hub/phan-tich', 'POST', { tai_san_id: ts, phan_tich: { dai: 12, doan: [{ t: 0, net: .9, dong: .1, sang: .5 }, { t: .5, net: .9, dong: .1, sang: .5 }] } });
  pt = JSON.parse(DB.raw.prepare(`SELECT phan_tich FROM tai_san WHERE id=?`).get(ts).phan_tich); assert.equal(pt.doan.length, 2); assert.equal(md().length, 2, 'dòng thời gian không mất'); assert.ok(JSON.parse(md()[0].so_do).net > .85, 'số đo cập nhật theo phân tích mới');
