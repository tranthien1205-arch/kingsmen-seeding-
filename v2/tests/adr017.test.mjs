...
test('017a: bàn huấn luyện, trần thầy, trần phút, thời gian chặng', async () => {
  ...
  const tl = (n) => Array.from({ length: n }, (_, i) => ({ tu: i * 2, den: i * 2 + 2, nhom: 'THI_CONG', mo: { nhom: i % 2 ? 'HOAN_THIEN' : 'THI_CONG' }, thay: { nhom: 'THI_CONG', chac: 0.9 }, can_xac_nhan: !!(i % 2), khung_url: '/media/media/k' + i + '.jpg' }));
